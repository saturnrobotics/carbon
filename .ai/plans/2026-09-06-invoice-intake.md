...
7. Treat overlapping multi-file Gmail candidate bundles as payment-scoped evidence until primary selection. Preserve all provenance and require review before paid parsing. Deduplicate across payments only when verified complete source sets establish one document or a native invoice already establishes the association. Resume untouched payment-only shells without consolidating ambiguous candidates. Canonical readiness and final approval recheck approved primary-hash ownership under the company approval lock; supporting candidates cannot claim an invoice or silently relink grouped payments.
...
- Release review found and fixed evidence registration during Processing retaining an invalid active attempt. Two PostgreSQL regressions first reproduced the stuck state, then verified NeedsReview, preserved human edits/raw evidence/billing, no stale readiness recovery, and no second paid call. The complete invoice-intake suite passes 71 tests. Internal native payload builders are excluded from MCP discovery and execution; generated metadata was refreshed.
- Learned-repeat browser verification: the normal worker plus canonical ERP validator used a deterministic synthetic provider, then a second receipt reached persisted Ready with saved supplier/item matches and changed quantity, price, total, date and reference. One-click approval created another Draft without new masters. Four actual private attachment copies completed and protected metadata was recorded.
...
- The first successful live extraction required removing numeric and collection bounds from Google's constrained decoder schema. Authoritative application validation retains those limits, including page/line bounds, and records charges for rejected output. The measured input usage exceeded the previous estimate margin; admission now adds an explicit response-schema allowance. A synthetic receipt completed with measured usage and a reviewable line; this is a provider smoke test, not the full quality gate.
- Final source-coverage review now requires a saved primary file and explicit reasons for other distinct files, retains completed-source provenance across failed retries, and invalidates Ready when a new file arrives. Browser selection/save/reload checks pass. The integrated suite passes 78 jobs tests and 28 ERP tests; both scoped typechecks pass.
- Live evaluation checkpoints retain initial machine observations. Resumed held-out receipts must still be canonically Ready with unchanged supplier/item/unit/conversion selections; a failed resumed candidate cannot reuse an old passing report.
