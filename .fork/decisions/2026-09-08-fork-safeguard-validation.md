...
The full ERP unit suite currently reports 982 passing tests and two failing localization checks. Both test files and the relevant implementation are unchanged from the merged upstream revision. One check rejects permitted breadcrumb `msg` descriptors; the other labels text inside `<Trans>` as untranslated and also finds a real untranslated Tax Status label. These are reported as failures, not waived or marked successful.

Disposable verification has applied all 1,000 migrations on fresh and upgrade paths. Full schema fingerprints, DB types, and backup artifacts converge. Swagger differs between independently booted PostgREST instances in the inferred primary-key annotation for the `partners` view. The schema gate retains that failure rather than erasing semantic metadata during comparison.
