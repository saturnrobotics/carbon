# Carbon knowledge source outbox (plan Task 15, Carbon half)

Context: the knowledge platform plan (`.fork/plans/2026-09-07-company-knowledge-platform.md`,
...
Decision: Carbon records knowledge events with reviewed `AFTER ROW` triggers on the
four tables the bounded knowledge reads project from — `receipt`, `receiptLine`,
`item`, `purchaseOrder` — into `public."knowledgeSourceOutbox"` (migration
...
- The dedupe identity is `UNIQUE (companyId, source, entityType, entityId,
  sourceVersion, eventType)`, the same shape the knowledge outbox and worker use.
...
- `knowledge.events.server.ts` claims with `FOR UPDATE SKIP LOCKED`, a
...
- `knowledge.outbox.integration.test.ts`: a rolled-back posting leaves no row; a
...
- `pnpm db:check:datasets` 4/4, `pnpm db:check:backups` restorable,
  `@carbon/knowledge` indexing 5/5, `knowledge-worker` 27/27, Kanban `test-api`
  34 passed / 1 skipped.
...
`knowledge.outbox` (the worker's adapter), and the periodic reconciliation sweep.
