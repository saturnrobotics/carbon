...
**Date:** 2026-09-11
**Scope:** plan Task 22 (`.fork/plans/2026-09-07-company-knowledge-platform.md`),
...
`.fork/plans/2026-09-11-knowledge-authorization.md` §2.
...
| 4 MCP over the reviewed handlers | nothing; `@modelcontextprotocol/sdk` exists only in `apps/erp` | `packages/knowledge/src/mcp/` (surface, delegating transport, deployment gate) + `apps/knowledge-query/src/mcp.ts` and `apps/knowledge-actions/src/mcp.ts`, each a ~10-line wiring of its own route table |
| 5 Disabled until auth + parity pass | n/a | `isMcpEnabled` requires the exact flag AND a release profile past `manual-v1`; `release.py` independently refuses the variable; documented in `contrib/deploying/knowledge/README.md` |
...
- **No new dependency.** The transport is plain JSON-RPC 2.0 over HTTP
  ...
  not resolvable from `packages/knowledge`; adding a production dependency was
...
- **A single-implementation refactor in the actions app.** Both command
  branches of `apps/knowledge-actions/src/index.ts` became named route
...
## Verification (local, synthetic; no containers were needed and none were created — the developer's stack and the shared `knowledge-manual-local-*` project were untouched)
...
- **`apps/knowledge-actions/src/mcp.ts` mounts the ticket command only.** The
