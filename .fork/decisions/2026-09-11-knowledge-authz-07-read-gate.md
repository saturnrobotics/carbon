# Knowledge authorization Task 07: the Carbon canonical read gate

Implements Task 07 of `.fork/plans/2026-09-11-knowledge-authorization.md` on top
...
- **One allowlist, owned by the module.** `KNOWLEDGE_OPERATIONS` in
  `apps/erp/app/modules/knowledge/knowledge.server.ts` maps each published
  operation to the workforce capability it requires. The former
  `KNOWLEDGE_READ_OPERATIONS` (unreferenced) and the gate's private
  `WORKFORCE_CAPABILITIES` are gone; `assertWorkforceAuthorization` reads the
  map through `knowledgeCapabilityFor`, and `isKnowledgeOperation` (by module,
  ...
  rather than disclosed until someone lists it. `knowledge.gate.test.ts` pins
  that every generated `knowledge_*` operation has a capability and that every
...
- **Pricing is its own operation.** `getItemSupplierPricing(itemId, supplierId?)`
  ...
  `knowledge.read.pricing`, permission `purchasing:view` (the `supplierPart` and
...
- **Disclosure.** `isDisclosedOperation` / `DISCLOSED_OPERATIONS` in
  `operations.server.ts` withhold the knowledge module from `search_tools`
...
- `pnpm run generate:mcp && pnpm --silent check:manifest`: 1563 operations
  ...
  (`knowledge_getItemSupplierPricing`, READ, `purchasing:view`).
- `pnpm --dir apps/erp exec vitest run "app/routes/api+/v1+/lib" app/modules/knowledge test/mcp-tool-permissions.test.ts test/mcp-tool-metadata.test.ts`
  ...
  `knowledge.read.integration.test.ts` are skipped (below).
...
- The `describe.skipIf` block in `knowledge.read.integration.test.ts` (pricing
  ...
  `KNOWLEDGE_READ_TEST_SUPABASE_URL`, `_ANON_KEY`, `_SERVICE_ROLE_KEY`,
  `_JWT_SECRET` taken from that slot's `.env.local` and
  `KNOWLEDGE_READ_TEST_DISPOSABLE=1`, then `crbn down --purge`.
