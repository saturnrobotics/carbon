# Knowledge intake review interface (plan Task 14)

Concise record of the decisions behind `apps/knowledge` intake and review.
...
- **The item association lives on the review, not as an entity link.** It is
  ...
  cannot insert `knowledge.entity` rows and no Carbon entity projection exists
...
- **Lingui without `@carbon/locale`.** The portal uses `@lingui/react/macro`
  and a `knowledge` catalog in the root `lingui.config.js`, with a small local
...
- `pnpm --filter knowledge test`, `--filter knowledge-worker test`,
  `--filter knowledge-query test`, `--filter @carbon/knowledge test`
- `pnpm exec turbo run typecheck` for the four knowledge packages
- `pnpm --filter knowledge test:e2e -- intake` against the local Docker stack
...
- A verified `knowledge.entityLink` for the associated item (see above).
