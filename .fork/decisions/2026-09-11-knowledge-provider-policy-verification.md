# Knowledge provider-eligibility policy verification

**Date:** 2026-09-11
**Scope:** Task 10 of the knowledge authorization program (platform plan §1.9.1).
...
- `knowledge.source."providerPolicy"` (jsonb, default `{}`) is the eligibility
...
- `packages/knowledge/src/provider-policy.ts` owns `providerEligible`,
...
- `createProviderDisclosure` in `apps/knowledge-query/src/query.server.ts` is the
...
- Release fence: `apps/knowledge-query/src/release-fence.test.ts` fails if the
...
- `pnpm --filter @carbon/knowledge test provider-policy` — 9 passed.
- `pnpm --filter @carbon/knowledge test` — 26 files, 121 passed.
- `pnpm --filter knowledge-query test` — 9 files, 29 passed (includes the fence
  and `provider-disclosure.test.ts`).
- Negative proof of the fence: hiding `provider-policy.test.ts` and, separately,
  removing the `@carbon/knowledge test` line from the workflow each made the fence
...
the `knowledge-check` workflow. Vector search and synthesis remain disabled.
