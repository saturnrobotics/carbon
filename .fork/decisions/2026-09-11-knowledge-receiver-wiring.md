# ERP knowledge receiver: deployment wiring and error semantics

Task 05 of the knowledge authorization program. The ERP API's delegated
...
- The deployment contract gains two optional ERP-only inputs handled by a
  dedicated `knowledge_receiver.py` module in the same shape as the payment
  sync credentials: `KNOWLEDGE_RECEIVER_AUDIENCE` (config) and
  `KNOWLEDGE_TRUSTED_CALLERS_JSON` (secret, file-mounted, placeholder
...
- `python -m unittest test_knowledge_receiver test_prepare_release test_render
...
Not covered: a live deployment, enrollment (Task 01), and the knowledge
