# Knowledge bounded routing, synthesis and the portal interface (plan Task 18)

**Date:** 2026-09-11
**Scope:** Task 18 of `.fork/plans/2026-09-07-company-knowledge-platform.md`
(§1.7 budgets, §1.9 invariants, A05/A08/A09/A14).
**Base:** `feat/knowledge-12-source-adapters` at `ab3c5da5ef`, merged with
`feat/knowledge-16-retrieval-gaps`, `feat/knowledge-authz-07-read-gate` and
`feat/knowledge-14-intake-review-ui`.
...
Conflicts hand-resolved: `packages/knowledge/src/sources/http.server.ts`
(Task 12's status-aware `boundedCall` now reads a 403 body for the structured
step-up code with the shared `readBounded`), `packages/knowledge/package.json`
...
## Verification (local, synthetic; containers `knowledge-t18-test` on 59980 and `knowledge-t18-cache` on 59983, removed afterwards)
...
- **The Docker browser e2e** (`apps/knowledge/tests/query.spec.ts`) was written
  ...
  `knowledge-manual-local` stack held all four for the whole session. Its
