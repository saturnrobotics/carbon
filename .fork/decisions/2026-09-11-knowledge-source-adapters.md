...
**Date:** 2026-09-11
**Scope:** plan Task 12 (`.fork/plans/2026-09-07-company-knowledge-platform.md`)
...
`knowledgeSourceOutbox`; and the periodic sweep for trigger-silenced reloads.
...
- **Machine admission is one module.** `verifyMachineRequest` moved to
  `@carbon/knowledge/machine-identity.server`; the worker's `machine-auth.ts`
  re-exports it, and Carbon's new route uses the same registration shape
  (`KNOWLEDGE_MACHINE_CALLERS_JSON`) with the same refusal of forwarded employee
  evidence.
- **Carbon's change feed is a route, not a database.** `POST
  /api/v1/knowledge/source-changes` (`apps/erp/app/routes/api+/v1+/`) is
  ...
  workforce identity. Its service module `knowledge.changes.server.ts` is not a
...
- **A claim carries the projection Carbon shows now.** Reads are batched per
  ...
  and tombstones).
- **Entity upserts do not enter the knowledge outbox.** `confirmOutboxApplied`
...
- **Reconciliation is a merge join, not a mark-and-sweep.** The sweep walks
  ...
  own range are tombstoned. State lives in `knowledge.source.cursor.carbonSweep`
...
## Verification (local, synthetic containers `knowledge-t12-*` and the
