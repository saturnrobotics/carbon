# Portal authorization Task 03: required assurance for delegated access

Implements Task 03 of `.fork/plans/2026-09-11-portal-authorization.md` on top
...
The portal services never compute a verdict: an IAP assertion proves
...
`packages/portal/src/step-up.ts` names the denial once for every hop.
...
with two-factor authentication. The page links to `PORTAL_CARBON_LOGIN_URL`
...
- `release.py` rejects any web environment key outside its required set, so
  `PORTAL_CARBON_LOGIN_URL` cannot be deployed until the receiver deployment
...
`pnpm --filter @carbon/portal test` (28 files, 178 tests),
`pnpm --filter @carbon/auth test workforce` (13 tests),
`pnpm --dir apps/erp exec vitest run "app/routes/api+/v1+/lib"` (57 tests),
`pnpm --filter portal test` (32 tests), `pnpm --filter portal-query test`,
...
`turbo typecheck` for `@carbon/portal`, `@carbon/auth`, `erp`, `portal`,
`portal-query`, `portal-worker`.
