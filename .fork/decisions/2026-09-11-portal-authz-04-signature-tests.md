# Portal authorization Task 04: real-signature verifier proof and caller registry validation

Implements Task 04 of `.fork/plans/2026-09-11-portal-authorization.md` on
...
`pnpm --filter @carbon/portal callers:validate <file>`: the receiver audience
...
`contrib/deploying/portal/callers.example.json` is a synthetic two-caller
registry validated by the `fork-checks` node job on every change to
`contrib/deploying/**`, the validator or `identity.server.ts`.

Verification on the branch: `pnpm --filter @carbon/portal test` (27 files,
152 tests), `pnpm exec turbo run typecheck --filter=@carbon/portal`, strict
