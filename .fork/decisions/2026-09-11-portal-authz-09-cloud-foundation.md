# Portal authorization Task 09: cloud identity foundation completion

Context: `.fork/plans/2026-09-11-portal-authorization.md` Task 09 (read from
...
- **State**: `main.tf` declares a partial `backend "gcs" {}`; bucket and
  `prefix=portal/<environment>` come from `-backend-config` (README "State
...
- **Web entry**: `google_cloud_run_v2_service.web` is a Terraform-owned shell
  ...
  observed service onto each v1 `replace` and refuses to promote `portal-web`
...
- **Audiences**: outputs `service_audiences` (IAP audience
  `/projects/<number>/locations/<region>/services/portal-web`; deterministic
...
- **Private source path**: mirrors Kanban — bidirectional VPC peering to
  ...
  `portal-source-database-client`, and an egress deny for everything else. No
  Cloud NAT. `release.py` stamps the tag only on units holding a database
  credential (query, ingest, schema, retention). A `portal-source-database-ca`
...
```text
terraform -chdir=contrib/deploying/portal init -backend=false   exit 0
terraform -chdir=contrib/deploying/portal validate               Success, no warnings
terraform -chdir=contrib/deploying/portal fmt -check             exit 0
python3 -m unittest discover -s contrib/deploying/portal -p 'test_infrastructure.py'   18 tests OK
python3 -m unittest discover -s contrib/deploying/portal -p 'test_release.py'          15 tests OK
python3 -m unittest discover -s contrib/deploying/portal -p 'test_*.py'                45 tests OK
...
`portal-web`, and dropping the `var.private_source_cidrs` references were
...
- `portal-actions` receives its §1.4 grant although it is not a manual-v1
