# Portal Google Drive connector

**Date:** 2026-09-11
**Scope:** Task 19 of the company portal platform plan (§1.6, §1.8, §1.9, D1–D4).
...
| --- | --- | --- |
| 1. Explicit enrollment, read-only connector, Secret Manager reference | Missing: a `drive` source row was the whole record; no scope, no credential reference, no live-check scope | `portal."driveEnrollment"` (migration `20260911230000_drive-connector.sql`): corpus, shared-drive id, root folders, `oauthScope` CHECK-constrained to `drive.readonly`, `credentialSecretRef` CHECK-constrained to a Secret Manager version path, reader live-check scope, `domainWideDelegation` default false (the worker refuses `true`), notification channel id + token hash, reconciliation cadence, last sync outcome. Runtime roles cannot insert it; the ingest role may update sync outcome columns only |
...
| Verify: `apps/portal/tests/drive-source.spec.ts` | Missing | Written against a new loopback fixture `apps/portal-worker/src/test/local-drive.ts`; see "Not run" |
...
## Verification (local, synthetic containers `portal-t19-test` / `portal-t19-cache`, no dev stack)

| Command | Result |
| --- | --- |
| `pnpm --filter @carbon/portal test drive` | 2 files, 10 passed |
| `pnpm --filter @carbon/portal test:integration drive` | 3 files, 17 passed (`drive.integration.test.ts` is 7 of them: enrollment read, scoped listing + no-op reconciliation, publication visibility, same PDF in two drives, parent-folder revocation of descendants + warm cache + candidate batch, delete/move/inaccessible shortcut + outbox re-arm, failed page changes nothing) |
| `pnpm --filter portal-worker test` | 15 files, 47 passed |
| `pnpm --filter portal-query test` | 11 files, 33 passed |
| `pnpm --filter portal test` | 11 files, 31 passed |
| `pnpm --filter @carbon/portal test` (whole package) | 29 files, 163 passed |
| `pnpm --filter @carbon/portal test:integration` (whole package) | 39 files, 191 passed |
| `python3 packages/portal/scripts/test_schema.py` | 20 passed (forced RLS and audit columns on the two new tables) |
| `pnpm --filter portal exec playwright test --list drive-source` | 3 tests listed in 1 file |
| `pnpm exec biome check --error-on-warnings <24 changed files>` | clean |
| `pnpm exec turbo run typecheck --filter=@carbon/portal --filter=portal-worker --filter=portal-query --filter=portal` | clean |
| `pnpm --filter @carbon/portal migrate:test` + `generate:types` | applied `20260911230000_drive-connector.sql`; 19 tables generated, `database.types.ts` gained the two tables only |
| `pnpm --filter portal test:e2e -- drive-source` | not run (see below) |

## Not run

- `pnpm --filter portal test:e2e -- drive-source`: the harness binds the fixed
  ...
  another agent's running `portal-manual-local-*` stack during this task and must
  ...
  `corepack pnpm --filter portal-worker exec tsx src/test/local-drive.ts` started
  with `PORTAL_E2E_SYNTHETIC_FIXTURES=1` and `PORTAL_E2E_DATABASE_URL`.
...
Registering `routes/settings.sources.tsx` in `apps/portal/app/routes.ts`
...
`contrib/deploying/portal/test_images.py` lists `settings.sources` among the
...
(`packages/portal/src/sources/drive-deployment.ts`) reads
`PORTAL_DRIVE_ENABLED=true`. Route config is a BUILD-time artifact, so a
...
deferred configuration) and already pins `portal-web` to `manual-v1`; a new
...
- `pnpm --filter portal build` was already FAILING on this branch, and the
  ...
  with `PORTAL_DRIVE_ENABLED=true` still fails. The fix is to move the
...
The first finding above is closed. `apps/portal/app/modules/sources/sources.models.ts`
...
`apps/portal/app/modules/**` found no other component reaching a `.server`
...
only import, and `portal-check.yml`'s build step now runs
`PORTAL_DRIVE_ENABLED=true pnpm --filter portal build` ahead of the turbo
...
`apps/portal` is not in `@carbon/checks`'s `TYPESCRIPT_ROOTS`, and adding it
...
Also worth recording: `compose.local.yaml` sets `PORTAL_DRIVE_ENABLED` as a
...
argument: `Dockerfile.web`'s `e2e` stage declares `ARG PORTAL_DRIVE_ENABLED`
...
building the release target WITH `--build-arg PORTAL_DRIVE_ENABLED=true` still
...
no `portal.read` operation, so it answers no Drive route either. The Drive
fixture therefore runs as its own `drive` Compose service on its own two ports,
and the manual gateway forwards `/v1/drive/*` to it (`PORTAL_E2E_DRIVE_FIXTURE_URL`,
...
scoped to its own `sourceId` — with one exception: `portal."identityBinding"`
...
without dropping the guarantee. `apps/portal/tests/loopback.ts` now owns that
...
`PORTAL_E2E_PORT` / `PORTAL_WEB_ORIGIN`; a published port that differed
...
2. **Not fixed — hand-off.** The browser download of a just-published manual
   ...
   `apps/portal-worker/src/server.ts:394`, i.e. `getAuthorizedDocumentVersion`
