# Portal platform Tasks 01, 11 and 23: foundation polish

Context: `.fork/plans/2026-09-07-company-portal-platform.md` Task 01 step 5,
...
- **Base image pins**: `contrib/deploying/portal/base-images.json` is the one
...
- **Migration window**: every database unit's plan entry declares
  `migrations.minimum`/`maximum` as bare portal migration names, which
  ...
  only for `portal-schema`, and records the window in the manifest. The
...
Offline: `python3 -m unittest discover -s contrib/deploying/portal -p
'test_*.py'` (64 tests), `terraform validate`, `ruff check contrib/deploying`,
actionlint on `portal-check.yml`, `pnpm --filter @carbon/portal test`
(126 tests), `pnpm --filter portal-worker test`, the query probe tests,
