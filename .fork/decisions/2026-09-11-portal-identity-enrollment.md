...
Task 01 of `.fork/plans/2026-09-11-portal-authorization.md`. Base revision:
`saturn/main` at `7464d10347`. All identifiers below are synthetic.

Context: `portal."identityBinding"` had `FORCE ROW LEVEL SECURITY` with
...
- A dedicated NOLOGIN, NOSUPERUSER, NOBYPASSRLS, NOINHERIT function-owner role
  `portal_enrollment_owner` holds `SELECT, INSERT, UPDATE` on the table plus
  ...
  direct write.
- `portal.enroll_workforce_identity(issuer, subject, company, user, capabilities)`
  and `portal.unbind_workforce_identity(issuer, subject, company)` are
  `SECURITY DEFINER`, owned by that role, executable only by `portal_migrate`
...
- Eligibility (existing active user, current membership, active company) is
  read through `public.portal_resolve_workforce_identity`, the same
  ...
  `portal.actor_active`. The write is made first and the call raises (rolling
...
- Subjects must match `^accounts\.google\.com:[0-9]+$` and may never contain
  ...
  through unbind.
- The owner role needs `EXECUTE` on `portal.actor_id()` and
  `portal.actor_active(text)`: PostgreSQL checks helper execution for the
...
- `packages/portal/src/enrollment.server.ts` wraps both functions;
  `packages/portal/scripts/enroll-identity.ts` (`identity:enroll`) is the
  ...
  non-local `PORTAL_MIGRATION_DATABASE_URL` without `--allow-remote` plus an
...
Verification (disposable container `portal.disposable=true` on a loopback
port; no developer database touched):

- `pnpm --filter @carbon/portal migrate:test` → applied, then `unchanged: true`.
- `python3 packages/portal/scripts/test_enrollment.py` → 11 passed through
  real roles (`portal_read`/`ingest`/`review`/`actions`/`maintenance`, `anon`,
  `authenticated` denied with 42501; `portal_migrate` enrolls, re-enrolls as a
...
- `pnpm --filter @carbon/portal test:integration policies enrollment` → 28 passed.
- `pnpm --filter @carbon/portal test enrollment enroll-identity` → 46 passed.
- `contrib/deploying/portal/local-stack-fixture.sql` applied twice: same
  binding ids, `version` stays 1, `portal_read` resolves the bob subject.
...
- `pnpm exec turbo run typecheck --filter=@carbon/portal --filter=portal-worker`.

Not run: the full Docker browser workflow (`local-stack.sh test`); the running
`portal-manual-local` stack belongs to someone else and was not touched. The
