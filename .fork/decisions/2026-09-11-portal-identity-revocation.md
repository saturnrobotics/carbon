...
Task 02 of `.fork/plans/2026-09-11-portal-authorization.md`. Base: branch
`feat/knowledge-authz-01-enrollment` at `9869cb58c1`. All identifiers below are
synthetic.

Context: `portal."identityBinding"."revocationVersion"` is folded into every
principal's `policyVersion` and into the answer-cache policy snapshot, but after
Task 01 only `portal.unbind_workforce_identity` advanced it. Deactivating a
...
- Carbon migration `20260911211525_knowledge-identity-revocation.sql` installs
  `public.portal_propagate_identity_revocation()` (SECURITY DEFINER, owned by
  `portal_enrollment_owner`, no PUBLIC or runtime-role EXECUTE) and two
  ...
  advances `revocationVersion` (and `version`, for `portal.check_version`)
...
- The guard is at fire time, not install time. The documented order applies
  Carbon migrations before the private portal migrations, so an install-time
  `IF EXISTS` on the portal schema would silently skip the triggers on every
  fresh install. The function returns before touching anything while
  `portal."identityBinding"` does not exist; the owner role is created here
  when absent (same NOLOGIN/NOBYPASSRLS shape as Task 01), and its grants and
  RLS policies still come only from the portal enrollment migration. A
...
- `packages/portal/scripts/revoke-identity.ts` (`identity:revoke`) revokes
  ...
  Listing a user's bindings runs under `SET ROLE portal_migrate` because the
...
- The disposable fixture (`scripts/setup-disposable.py`) applies the Carbon
  migration after the portal migrations and grants the vitest login
...
Verification (private disposable container `portal-authz02-test`, label
`portal.disposable=true`, loopback port 59970; the worktree's own
`crbn up --no-apps --minimal --no-portless` stack for the Carbon checks):

- `python3 packages/portal/scripts/test_revocation.py` -> 8 passed: owner,
  ...
  `portal_read`); the migration re-applies idempotently.
- `pnpm --filter @carbon/portal test:integration revocation policies enrollment`
...
- `pnpm --filter @carbon/portal test enrollment enroll-identity revoke-identity revocation`
  ...
  revocation paths).
- `pnpm --filter @carbon/portal migrate:test` -> `unchanged: true`.
...
- `pnpm exec turbo run typecheck --filter=@carbon/portal --filter=@carbon/database`
  ...
  `packages/portal/scripts/**` is outside Biome's configured includes
  (pre-existing, Task 01's CLI too), so the three scripts were formatted
  through Biome's stdin path and diff-verified instead.

Not done: a portal-side migration cannot install these triggers (the schema
...
Follow-up (same branch): the verification above ran the `@carbon/portal`
suites but not the `portal-query` integration pair, and two consequences of
the irreversible rule were found there afterwards. Both are test-side; the
trigger is unchanged.

- `apps/portal-query/src/local-security.integration.test.ts` restored
  ...
  re-enrolls explicitly (bumping `version`, which `portal.check_version`
