...
Implements Task 21 of `.fork/plans/2026-09-07-company-portal-platform.md` on top
of Task 18 (`feat/knowledge-18-routing-portal`), keeping the invariants in §2 of
`.fork/plans/2026-09-11-portal-authorization.md`. Local proof only: one
...
- **The proposal stays a proposal until it is complete.**
  `packages/portal/src/commands/procurement.ts` now owns the whole
...
- **The portal outbox event comes from the trigger, not from the command.**
  The earlier version inserted into `portal.outbox` and read `portal.source`
  — neither exists in Carbon's schema (they live in the portal database), so
  that path could only ever have failed against a real Carbon database. Task 15's
  `portalSourceOutbox` trigger on `purchaseOrder` already records the change in
...
- **The raw purchasing transaction is not a published operation.**
  ...
  leaving `portal_createProcurementDraft` — capability
  `carbon.procurement.draft`, permission `purchasing:create` — as the only
  exposure. `PORTAL_OPERATIONS` needed no change.
- **Scheduling persists a command reference, never a credential.**
  `apps/portal-actions/src/scheduled.ts` builds that reference and
...
- **Migration `20260912185440_knowledge-command-receipt-tenancy.sql`.** The
  ...
  foreign key, unlike its sibling `portalProcurementSchedule` and every other
...
- `pnpm run generate:types` — `portalCommandReceipt.version` added; no other
  table changed.
- `pnpm run generate:mcp && pnpm --silent check:manifest` — ok, 1562 operations
  ...
  blocked, the portal operation's response shape updated).
- `pnpm --filter @carbon/portal test procurement` — 32 passed.
- `pnpm --filter portal-actions exec vitest run src/procurement.test.ts src/scheduled.test.ts`
  — 23 passed.
...
- `pnpm --dir apps/erp exec vitest run app/modules/portal test/mcp-tool-permissions.test.ts`
...
- `pnpm db:check:backups` — restorable; `-- --stage` refreshed the 364-table
  manifest baseline.
- `pnpm exec turbo run typecheck --filter=erp --filter=@carbon/portal --filter=portal-actions --filter=@carbon/jobs`
...
- `packages/jobs/src/procurement-schedule/*` (the claim/lease worker) and the
  ...
  `packages/portal/scripts/setup-disposable.py`, which was not run here.
