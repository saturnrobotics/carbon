# Portal retrieval gaps: extension record, section expansion, filtered ANN

**Date:** 2026-09-11
**Scope:** Task 16 of the company portal platform plan (§1.7), remaining gaps.
...
- **Extension versions are recorded, not assumed.** Migration
  ...
  `portal."extensionVersion"` (owner-only, no row policies, listed in
  `PORTAL_METADATA_TABLES`) and records pgvector's `extversion` at migration
  time. `verifyPortalExtensions` runs after every migration pass: a missing or
...
- **One ACL predicate for every vector ranking.** `portal.search_allowed_documents`
...
- `pnpm --filter @carbon/portal migrate:test` — applied, `vector 0.8.0` recorded.
- `pnpm --filter @carbon/portal generate:types` — 17 tables.
- `pnpm --filter @carbon/portal test:integration retrieval` — 22 passed.
- `pnpm --filter @carbon/portal test retrieval recall` — 14 passed; full suite 130.
- `pnpm --filter portal-query test` — 29 passed.
- `python3 packages/portal/scripts/test_schema.py` — 20 passed.
...
- Root `pnpm run generate:types` not run: no Carbon development database was
  running, and the portal migration changes no public type.
