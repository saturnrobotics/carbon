...
`/api/propose-command` gateways, portal-query's proposal handler (board and
...
`portal-actions` service with its Dockerfile, and on the Kanban side the
...
- Per-command re-authorization is a named boundary: `authorization.ts` in
  `portal-actions` refuses machine identities and any principal without
  `kanban.ticket.create`, so a `portal.read` query identity has no invocation
...
- A service-boundary integration test starts the real Kanban backend (SQLite,
  ...
  `portal-actions` through it: initial column and actor, replay, four
  ...
  only when `PORTAL_KANBAN_BACKEND_DIR` is set.

## Deliberately not done

- portal-query does not route `/v1/propose-command` or `/v1/transcribe`, and
...
- The central `portal.command` row is not written by the action service.
...
`64c6e3f658e1` applied, container removed. Carbon: `@carbon/portal test
command` 12 passed; `portal-actions test` unit suites passed plus the
Kanban-backed integration suite 5 passed when pointed at the sibling checkout;
`portal test` passed; scoped typecheck and strict Biome on the changed paths
...
(`contrib/deploying/portal/test_images.py`) enforces, so
`apps/portal/app/routes.ts` never registers it and `routes/unavailable.tsx`
answers instead.
...
`PORTAL_ACTIONS_URL` nor `PORTAL_ACTIONS_AUDIENCE` on a `portal-web`
...
Fence proof on the release-shaped build (`pnpm --filter portal build`, no
...
`@carbon/portal`, which the portal needs in order to recognise and refuse a
