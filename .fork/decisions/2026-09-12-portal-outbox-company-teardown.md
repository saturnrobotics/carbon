# Portal source outbox and the company teardown cascade

Context: `20260911205347_knowledge-source-outbox.sql` (fork decision
`2026-09-11-portal-15-carbon-outbox.md`) attached an `AFTER ROW` trigger to
...
`portalSourceOutbox_companyId_fkey` — a constraint none of those suites
reference.
...
`portal_source_outbox_enqueue()` with the same body plus one precondition
...
- Regression pinned red→green in `portal.outbox.integration.test.ts`: a bare
...
- ERP invoice integration 43/43 (was 30 failed / 13 passed), jobs invoice and
  payment-sync integration 101/101, outbox 6/6, `@carbon/portal` indexing 5/5,
  `portal-worker` 27/27.
- `db:check:datasets` 4/4, `db:check:backups` restorable, scoped typecheck for
  `erp`, `@carbon/database` and `@carbon/portal` clean.
...
Not done here: the consumer that carries these rows into `portal.outbox` and
