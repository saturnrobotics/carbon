# Knowledge connected workflow: verification and remaining gates
...
2. The final workforce browser test ignored `KNOWLEDGE_E2E_PRESERVE_FIXTURE` and
...
| --- | --- |
| Frozen pnpm install; config package build | Passed with pinned pnpm 10.33.4. The first knowledge test attempt could not load an unbuilt `@carbon/config`; building it resolved setup, without dependency changes. |
...
| Knowledge / query / portal / worker unit tests | 456 / 85 / 76 / 89 passed; no skipped tests in these four runs. |
| Scoped typecheck | Five tasks passed for knowledge, query, worker, shared knowledge and their task dependency. Portal typecheck repeated after its test change. |
...
38 knowledge migrations in the same database. Enrollment succeeded against
...
The query handler used real Redis and a separate `knowledge_read` login that was
...
[local verification](../playbooks/knowledge-local-verification.md) and
[Carbon connection](../playbooks/knowledge-carbon-connected.md). Raw logs and
