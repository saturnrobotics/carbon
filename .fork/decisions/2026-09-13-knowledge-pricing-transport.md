...
Implements Task 05 of `.fork/plans/2026-09-13-knowledge-erp-join-repairs.md`.
Base: `saturn/main` at `eb5da21766`. Local proof only; no cloud environment was
touched, and no authorization rule changed.

## What was found

Carbon published `knowledge_getItemSupplierPricing` with a capability
(`knowledge.read.pricing`), a permission (`purchasing:view`), a manifest entry
and a committed digest line. The knowledge platform's source transport keeps an
...
`packages/knowledge` or `apps/knowledge*` referenced the operation. Every side
...
- **Registered, not widened.** One entry
  (`/api/v1/knowledge/getItemSupplierPricing`) joins the allowlist. No other
  path, no change to the transport's blanket `knowledge.read` check, and no
...
- **Pricing is a Carbon adapter method, not a source-contract capability.**
  ...
  `knowledge.read.integration.test.ts` pins that on the manifest's response
...
- **The capability is re-read before the call, and can only refuse.** The
  transport's existing check is `knowledge.read`, which does not imply
  `knowledge.read.pricing`; Carbon's gate stays the authority. Refusing locally
...
`packages/knowledge/src/sources/carbon-operations.test.ts` asserts that every
knowledge operation Carbon publishes is either registered in the transport or
...
One exclusion is recorded today — `knowledge_createProcurementDraft`, a write
whose review boundary is the portal and whose route is the actions service's
own, not this read transport.

`apps/erp/app/modules/knowledge/AGENTS.md` gains the matching rule, since the
...
- No consumer calls `getSupplierPricing` yet. The query service's source wiring
  (`apps/knowledge-query/src/index.ts`, `query.server.ts`) is Tasks 03 and 04 and
  belongs to another change; this task is the transport path and the gap class.
- The four row-level-security cases in `knowledge.read.integration.test.ts`
  remain skipped — they need a disposable Supabase stack, as recorded in
  `.fork/decisions/2026-09-11-knowledge-authz-07-read-gate.md`. Unchanged here.
