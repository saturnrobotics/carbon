...
Context → The knowledge portal's Drive settings page took its eligibility label and its zod contract from `sources.service.ts`, the same module that forwards the loader's request. That service imports `intake.service.ts`, which imports `services/identity.server`. `pnpm --filter knowledge build` was green, so the leak looked absent.
...
Applies to → Every React Router app in this repo (`knowledge`, `erp`, `mes`, `academy`, `starter`); any `{module}.service.ts` that exports a schema, label, or formatter a component renders; and any feature whose routes are contributed conditionally at build time.
