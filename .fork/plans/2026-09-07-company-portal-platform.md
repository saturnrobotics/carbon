# Company portal platform — execution plan
...
[current program status](../decisions/2026-09-12-portal-program-status.md)
...
**Suggested implementation branch:** feat/company-portal-platform, split into the independently reviewable tasks below.

This plan covers a fast, mostly read-only natural-language interface to operational
records and documents, streamlined portal intake, Google workforce login, scoped
...
projects, and put the portal workload in a separate production project. Keep
nonproduction separate from production. The portal workload may contain several
...
This does not block the portal service or the existing Google credential SSO.
...
|---|---|---|---|
| portal-web | portal-query | portal-web's exact IAP audience | query |
| portal-web | portal-actions | portal-web's exact IAP audience | propose/execute registered user commands |
| portal-query | Carbon or Kanban read API | portal-web's exact IAP audience | registered reads only |
| portal-actions | Carbon or Kanban command API | portal-web's exact IAP audience | exact registered command |
...
|---|---|---|
| portal-web | apps/portal | IAP browser backend; can invoke approved query/action APIs; no source DB credentials |
| portal-query | apps/portal-query | Read-only index role, read source APIs, model access; cannot invoke business write APIs |
| portal-ingest | apps/portal-worker | Connector credentials and index-write permissions; no purchasing/ticket execution |
| portal-parser | apps/portal-worker, separate entrypoint/image | One staged document and bounded outputs; no source credentials or business DB access |
| portal-actions | apps/portal-actions | Narrow command APIs; no broad document connector credentials |
| portal-schema | packages/portal migration entrypoint | Migration-only role; never supplied to an application |
...
Share pure schemas and algorithms through @carbon/portal; do not make the
...
portal workload when provisioning it; do not change the behavior of Carbon's
...
Portal intake accepts a PDF, source URL, scanned nameplate, photo, or existing
...
Publishing portal and posting an ERP financial/inventory transaction are
...
Eligibility is the per-source column `portal.source."providerPolicy"` (jsonb,
`NOT NULL DEFAULT '{}'`; `packages/portal/migrations/20260908000245_knowledge-foundation.sql:30`),
read as `allowedProviders` and `allowedClassifications` by `providerEligible`
(`packages/portal/src/provider-policy.ts:15`): a provider is admitted only when
...
re-read projection (`packages/portal/src/retrieval/lexical.server.ts:39`), and
...
(`apps/portal-query/src/query.server.ts:57`) is the only route from evidence to
...
deny. `executeReadQuery` (`packages/portal/src/query/answer.server.ts:111-119`)
...
(`packages/portal/src/indexing/indexer.server.ts:130`) throws before the first
...
`model` is wired (`apps/portal-query/src/index.ts:63`; worker `apps/portal-worker/src/index.ts:68`),
so no provider is reachable; `apps/portal-query/src/release-fence.test.ts`
fails unless `packages/portal/src/provider-policy.test.ts` and
`apps/portal-query/src/provider-disclosure.test.ts` are in the CI runtime
...
quantity, units, and date intent are resolved. Until then, retain a portal
...
[source registry decision](../decisions/2026-09-13-portal-query-source-registry.md).
...
2. Add a repeatable local container harness under `contrib/deploying/portal/`
...
3. Extend `apps/portal/tests/manual-workflow.spec.ts` and the worker integration
...
[`2026-09-11-portal-authorization.md`](2026-09-11-portal-authorization.md).
...
backlog: use the [program task map](../decisions/2026-09-12-portal-program-status.md)
...
- [ ] Task 10: Create the portal schema, roles, and policy tests.
...
- Create: packages/portal/package.json, tsconfig.json, vitest.config.ts
- Create: packages/portal/src/contracts.ts, contracts.test.ts, test/database.ts
- Create: packages/portal/src/test/synthetic.ts
- Create: apps/portal/package.json, apps/portal-query/package.json,
  apps/portal-worker/package.json, apps/portal-actions/package.json
- Create: apps/portal/playwright.config.ts and apps/portal/tests/setup.ts
- Create: apps/portal/app/root.tsx, routes.ts, entry.client.tsx, entry.server.tsx
- Create: apps/portal/react-router.config.ts, vite.config.ts, tsconfig.json
...
1. Create @carbon/portal as a server-safe pure contract/algorithm package. Keep
...
5. Name the app packages portal, portal-query, portal-worker, and
   portal-actions. Add typecheck/test/build scripts and portal's test:e2e
...
6. Make the portal browser skeleton runnable with existing layout/style
...
**Verify:**
~~~bash
pnpm --filter @carbon/portal test
pnpm exec turbo run typecheck --filter=@carbon/portal
~~~
...
- Create: contrib/deploying/portal/main.tf, variables.tf, outputs.tf
- Create: contrib/deploying/portal/identity.tf, networking.tf, services.tf
- Create: contrib/deploying/portal/storage.tf, redis.tf, releases.tf
- Create: contrib/deploying/portal/README.md, test_infrastructure.py
- Create: contrib/deploying/portal/Dockerfile.web, Dockerfile.query,
  Dockerfile.ingest, Dockerfile.parser, Dockerfile.actions, Dockerfile.schema
- Create: contrib/deploying/portal/cloudbuild.yaml, release.py, test_release.py
- Create: contrib/deploying/portal/probe/server.ts, Dockerfile.probe
- Create: .github/workflows/portal-check.yml
...
5. Configure one private GCS portal bucket with object-generation references,
...
python3 -m unittest discover -s contrib/deploying/portal -p 'test_release.py'
~~~
...
- Create: packages/portal/src/identity.server.ts, identity.test.ts
...
- Create: apps/portal/app/services/identity.server.ts
- Create: contrib/deploying/portal/callers.schema.json
...
- Create: apps/erp/app/modules/portal/portal.models.ts
- Create: apps/erp/app/modules/portal/portal.service.ts
- Create: apps/erp/app/modules/portal/portal.server.ts, index.ts
- Create: apps/erp/app/modules/portal/portal.read.integration.test.ts
...
pnpm --dir apps/erp exec vitest run app/routes/api+/v1+/lib app/modules/portal
...
- Create: packages/portal/src/migrations.server.ts
- Create: packages/portal/src/schema-contract.ts
- Create: packages/portal/src/schema.integration.test.ts
- Create: packages/portal/src/policies.integration.test.ts
- Create: packages/portal/migrations/ using the timestamped generator from Task 01.
- Copy from: packages/database/supabase/migrations for ID/audit/company conventions.

**Steps:**

1. Implement the private portal schema defined in section 4.2. It is not added
   to PostgREST's exposed schemas. Keep an independent migration ledger/owner and
   a database advisory lock so portal migrations do not replay Carbon migrations.
...
3. Create dedicated NOLOGIN policy roles and separately provisioned runtime login
   roles: portal_read, portal_ingest, portal_review, portal_actions,
   portal_migrate. Runtime roles are not owners, superusers, or BYPASSRLS roles.
...
**Verify:**
~~~bash
pnpm --filter @carbon/portal migration:new portal-foundation
pnpm --filter @carbon/portal migrate:test
pnpm --filter @carbon/portal test:integration schema policies
~~~
...
**Out of scope:** production schema changes, exposing portal through raw Data API,
...
- Create: packages/portal/scripts/generate-types.ts
- Generate: packages/portal/src/database.types.ts
...
pnpm exec turbo run typecheck --filter=@carbon/portal --filter=@carbon/database
pnpm --filter @carbon/portal test:integration compatibility
~~~
...
- Create: packages/portal/src/sources/carbon.server.ts, kanban.server.ts
- Create: packages/portal/src/sources/registry.ts, sources.test.ts
- Create: packages/portal/src/entities/resolve.ts, resolve.test.ts
- Create: apps/portal-query/src/server.ts, source-routes.ts
...
**Verify:**
~~~bash
pnpm --filter @carbon/portal test sources resolve
pnpm --filter @carbon/portal test:integration source-contract
~~~
...
- Create: packages/portal/src/intake/contracts.ts, intake.server.ts
- Create: packages/portal/src/intake/extraction.server.ts, reconciliation.ts
- Create: packages/portal/src/intake/intake.test.ts, preservation.test.ts
- Create: apps/portal-worker/src/ingest.ts, parser.ts, fetch-policy.ts
- Create: apps/portal-worker/src/fetch-policy.test.ts
...
**Verify:**
~~~bash
pnpm --filter @carbon/portal test intake preservation
pnpm --filter portal-worker test
~~~
...
- Create: apps/portal/app/routes/intake.tsx, intake.$id.tsx
- Create: apps/portal/app/modules/intake/intake.models.ts, intake.service.ts
- Create: apps/portal/app/modules/intake/ui/IntakeReview.tsx
- Create: apps/portal/app/modules/intake/ui/IntakeUpload.tsx
- Create: apps/portal/tests/intake.spec.ts
...
pnpm exec turbo run typecheck --filter=portal
pnpm --filter portal test:e2e -- intake
~~~
...
- Create: packages/portal/src/indexing/chunks.ts, indexer.server.ts
- Create: packages/portal/src/indexing/outbox.server.ts, indexing.test.ts
- Create: apps/portal-worker/src/inngest.ts, functions.ts
- Create: apps/erp/app/modules/portal/portal.events.server.ts
- Create: a Carbon migration from pnpm db:migrate:new portal-source-outbox.
- Create: apps/erp/app/modules/portal/portal.outbox.integration.test.ts
- Create: ../kanban/backend/migrations/versions/ using the existing revision generator
  for a portal-source-outbox revision.
- Modify: ../kanban/backend/app/models.py, services.py, main.py for transactional
  outbox insertion at source writes.
- Create: ../kanban/backend/app/portal_events.py and corresponding backend tests.
...
2. Embed changed content once, with model/dimension/version recorded. Publish a
   ...
   models in place.
3. Use existing Inngest v3 semantics with a separate portal client/application
   and function registration endpoint. Do not import the entire ERP function list
   into the portal worker or require ERP deployment for worker-only changes.
...
pnpm db:migrate
pnpm run generate:types
pnpm --dir apps/erp exec vitest run app/modules/portal/portal.outbox.integration.test.ts
pnpm --filter @carbon/portal test indexing
pnpm --filter portal-worker test
make -C ../kanban test-api
~~~
...
- Create: packages/portal/src/retrieval/lexical.server.ts, vector.server.ts
- Create: packages/portal/src/retrieval/fusion.ts, evidence.ts
- Create: packages/portal/src/retrieval/retrieval.integration.test.ts
- Create: packages/portal/src/retrieval/recall.test.ts
- Create: a generated portal migration for query indexes and search functions.
...
pnpm --filter @carbon/portal generate:types
pnpm run generate:types
pnpm --filter @carbon/portal test:integration retrieval
pnpm --filter @carbon/portal evaluate -- suite=recall
~~~
...
- Create: packages/portal/src/cache/keys.ts, cache.server.ts, epochs.server.ts
- Create: packages/portal/src/cache/cache.test.ts, revocation.integration.test.ts
- Create: apps/portal-query/src/cache.server.ts
- Create: apps/portal-worker/src/invalidation.ts
...
**Verify:**
~~~bash
pnpm --filter @carbon/portal test cache
...
- Create: packages/portal/src/query/router.ts, budgets.ts, answer.server.ts
- Create: packages/portal/src/query/query.test.ts, grounding.test.ts
- Modify: apps/portal/app/root.tsx
- Create: apps/portal/app/routes/_index.tsx, routes/api.query.ts
- Create: apps/portal/app/modules/portal/ui/QueryInput.tsx, EvidenceCard.tsx
- Create: apps/portal/tests/query.spec.ts
...
**Verify:**
~~~bash
pnpm --filter @carbon/portal test query grounding
pnpm exec turbo run typecheck --filter=portal --filter=portal-query
pnpm --filter portal test:e2e -- query
pnpm --filter @carbon/portal evaluate -- suite=interactive
~~~
...
- Create: packages/portal/src/sources/drive.server.ts, drive.test.ts
- Create: apps/portal-worker/src/drive-sync.ts, drive-permissions.ts
- Create: apps/portal/app/routes/settings.sources.tsx
- Create: apps/portal/tests/drive-source.spec.ts
- Copy from: apps/portal/app/modules/intake/ui/IntakeReview.tsx
...
**Verify:**
~~~bash
pnpm --filter @carbon/portal test drive
pnpm --filter portal-worker test
pnpm --filter portal test:e2e -- drive-source
~~~
...
- Create: packages/portal/src/commands/ticket.ts, command.test.ts
- Create: apps/portal-actions/src/server.ts, authorization.ts, ticket.ts
...
- Create: apps/portal/app/routes/api.transcribe.ts, api.commands.ts
- Create: apps/portal/tests/ticket-command.spec.ts
...
4. Add a source-local command receipt table with a unique workspace/actor/action/
   ...
   do not try to include the central portal command row in a distributed transaction.
...
~~~bash
make -C ../kanban test-api
pnpm --filter @carbon/portal test command
pnpm --filter portal-actions test
pnpm --filter portal test:e2e -- ticket-command
~~~
...
- Create: packages/portal/src/commands/procurement.ts, procurement.test.ts
- Create: apps/portal-actions/src/procurement.ts, scheduled.ts
- Create: apps/erp/app/modules/portal/portal.commands.server.ts
- Create: apps/erp/app/modules/portal/portal.commands.test.ts
...
- Create: a Carbon migration from pnpm db:migrate:new portal-command-receipts.
...
3. Keep incomplete data in a versioned portal proposal. Execution requires
...
pnpm run generate:types
pnpm run generate:mcp
pnpm --filter @carbon/portal test procurement
pnpm --filter portal-actions test
pnpm --dir apps/erp exec vitest run app/modules/portal/portal.commands.test.ts
...
- Create: packages/portal/src/sources/conformance.ts, conformance.test.ts
- Create: packages/portal/src/sources/engineering.example.ts, crm.example.ts
- Create: apps/portal-query/src/mcp.ts
- Create: apps/portal-actions/src/mcp.ts
- Create: packages/portal/src/mcp-parity.test.ts
...
**Verify:**
~~~bash
pnpm --filter @carbon/portal test conformance mcp-parity
...
- Create: packages/portal/src/telemetry.ts, redaction.test.ts
- Create: packages/portal/src/budgets.server.ts, retention.server.ts
- Create: apps/portal-worker/src/retention.ts, recovery.ts
- Create: contrib/deploying/portal/monitoring.tf, recovery.md
- Create: contrib/deploying/portal/verify_recovery.py
...
5. Back up canonical portal records, ACLs, source bindings, and object references.
...
**Verify:**
~~~bash
pnpm --filter @carbon/portal test redaction budgets retention
pnpm --filter portal-worker test
...
- Create: packages/portal/evaluations/questions.jsonl, attacks.jsonl
- Create: packages/portal/scripts/evaluate.ts, verify-security.ts
- Create: packages/portal/scripts/verify-deployment.ts
- Create: apps/portal/tests/workforce-sso.spec.ts, authorization.spec.ts
- Create: contrib/deploying/portal/ROLLOUT.md
...
**Verify:**
~~~bash
pnpm --filter @carbon/portal evaluate -- suite=all
pnpm --filter @carbon/portal verify:security
pnpm --filter @carbon/portal verify:deployment
...
### 4.2 Portal schema contract

The SQL implementation lives in independently generated migrations in
packages/portal/migrations/. Use the private portal schema, not the
...
each role/table. portal_read has SELECT only on published permitted document
and entity projections; it has no writes, no source secrets, and no command-table
mutation. portal_ingest can update source-derived records for its configured
sources but cannot execute business commands. portal_review requires a verified
review/publish grant and cannot edit connector credentials. portal_actions can
persist command state and audit only. Only portal_migrate owns schema/DDL.
