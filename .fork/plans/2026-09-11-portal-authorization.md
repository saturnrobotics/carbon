# Portal platform authorization program
...
of `2026-09-07-company-portal-platform.md`. That plan's §1.2 (managed Google
...
Every read feature of the portal platform beyond manual-v1 depends on this
...
and credentials belong in `contrib/deploying/portal/.local/`,
...
|---|---|---|
| Two-assertion contract (§1.4) | `packages/portal/src/identity.server.ts:258-363` verifies the service ID token against the receiver audience and the IAP assertion against the caller's registered `sourceIapAudience`, using `google-auth-library` with key caching and single-flight refresh (`:184-249`). Spoofable identity headers are rejected (`:19-24`). Unit suite `identity.test.ts` covers wrong audience, unregistered subject, expiry, future-dated, overlong lifetime, missing access level, forged headers, fresh-header construction. | No test drives `GoogleWorkforceTokenVerifier` against a real signature; every assertion-level test injects a stub. `contrib/deploying/portal/callers.schema.json` is validated by nothing. A failed workforce auth in ERP throws `Error`, surfaced as HTTP 500 rather than 401 (`apps/erp/app/routes/api+/v1+/lib/authenticate.server.ts:75-106`). |
| Canonical user resolution | `portal_resolve_workforce_identity` (`packages/database/supabase/migrations/20260908005300_…sql:35-88`) keys on `(issuer, subject, companyId)`; no email column is read. Unknown subject ⇒ unauthorized. Store implementations in `packages/auth/src/services/workforce.server.ts:67-88`, `packages/portal/src/identity-store.server.ts`, and a remote store that re-checks issuer/subject/company (`identity.server.ts:486-546`). | **No way to create a binding.** `portal."identityBinding"` has `FORCE ROW LEVEL SECURITY` with INSERT/UPDATE/DELETE `false` for every role (`packages/portal/migrations/20260908000245_knowledge-foundation.sql:397-408`); the only writers are the local e2e fixture and the README's hand-run SQL. |
| Active state, membership, revocation | Binding, user, membership and company active flags are all required (`identity.server.ts:330-338`; resolver `:55-56`). `revocationVersion` exists (foundation `:47`), is folded into `policyVersion` (`identity.server.ts:351`) and the cache epoch (`packages/portal/src/cache/epochs.server.ts:14,32`). | **Nothing increments `revocationVersion`.** No route, job, RPC or runbook step; it is a constant `1`. No trigger ties Carbon user deactivation or membership removal to it. |
...
| Logging | `identity.server.ts` has no log calls; telemetry is a positive allowlist (`packages/portal/src/telemetry.ts:41-80`); forwarded headers are rebuilt from scratch (`identity.server.ts:418-450`). | — |
| Carbon canonical read API | `workforce` auth kind (`authenticate.server.ts:75-106`); `assertWorkforceAuthorization` applies caller `allowedOperations` ∩ capabilities AND the user's fresh permissions containing `companyId` (`base.server.ts:70-98`); portal module operations are reachable only on the workforce kind (`base.server.ts:106-109`). Six reads plus one write in `apps/erp/app/modules/portal/portal.service.ts`; every read is user-scoped and `.eq("companyId", …)` from context; validators are `.strict()`. Registry, generator module list, permission overrides and manifest (1562 tools, 16 modules) are wired. | `PORTAL_READ_OPERATIONS` (`portal.server.ts:1-8`) is an unreferenced duplicate of `WORKFORCE_CAPABILITIES` (`base.server.ts:60-68`). Price fields are excluded by a string constant with no separate permission and no test. No test that api-key, oauth or session callers get `NOT_FOUND`; no `callOperation` (MCP path) test for a portal op; no cross-company middleware test; no restricted-field assertion. Portal tools are disclosed in MCP `search_tools` and the public OpenAPI document. ERP has no deployment wiring for `PORTAL_TRUSTED_CALLERS_JSON` (`packages/auth/src/services/workforce.server.ts:90-105` throws without it; only `contrib/deploying/portal/release.py:19-21` sets it, for the portal services). No `AGENTS.md` for the module. |
| Kanban authorization | Code-complete: workforce path with two assertions and a caller registry (`../kanban/backend/app/security.py:49-68`, `workforce.py:67-100`); no implicit admission — unbound subjects get 403 (`authorization.py:37-80`); schema for `external_identity_bindings`, `users.is_active`, `board_memberships` with view/create/update, `command_receipts` (`migrations/versions/64c6e3f658e1_…py`); board filtering in every list, ticket, column, capture, portal, product and command handler; `actor_id` stamped on activities (`services.py:190-211`); `tests/test_authorization.py` and `test_workforce.py`. | **Not deployed.** No deploy artifact sets `KANBAN_WORKFORCE_REQUIRED`, `KANBAN_SERVICE_AUDIENCE`, `KANBAN_SOURCE_IAP_AUDIENCE`, `KANBAN_WORKFORCE_CALLERS`; `deploy/gcp-setup.sh` has no IAP integration. The legacy `KANBAN_API_TOKEN` + forwarded-email path is the default mode. The migration backfills board memberships but deliberately creates no identity bindings, so after cutover nobody can sign in until bindings are provisioned, and there is no provisioning path. `authorization.py:74-75` still selects a hard-coded demo user when no binding and no authenticated email exist (test mode only). `POST /api/users` creates principals without bindings. `product.py:513,853` take `author_id`/`uploader_id` from the payload rather than the principal. |
| Cloud identity foundation | Terraform declares seven service accounts with no keys, a conditioned release-controller role, secret containers with a per-identity access matrix, least-privilege invokers for parser and retention jobs, a private bucket, dedicated Redis, monitoring, and IAP on the synthetic probe with a Workspace-group binding (`contrib/deploying/portal/identity.tf`, `services.tf`, `storage.tf`, `redis.tf`, `monitoring.tf`). `test_infrastructure.py` is static substring assertion. | No backend/state configuration. IAP exists only on the probe; no IAP client/brand resource; per-service audiences exist only as release-controller env vars. No Cloud Run IAM invoker grants for web→query, web→actions, ingest→query. `var.private_source_cidrs` is declared and unused: no firewall, NAT or peering toward the Carbon VM. No cloud environment has ever been provisioned (`contrib/deploying/portal/README.md:174`). |
...
3. Read credentials cannot write. `portal_read` stays `NOLOGIN NOSUPERUSER NOBYPASSRLS NOINHERIT`; new functions name their callers (`20260908012959_knowledge-function-execution-boundary.sql` revoked `PUBLIC EXECUTE` by default).
...
8. No task resets a developer database. Schema work runs in disposable databases (`packages/portal/scripts/setup-disposable.py`, `pnpm --filter @carbon/portal migrate:test`).
...
- Create: `packages/portal/migrations/<generated>_workforce-identity-enrollment.sql` via `pnpm --filter @carbon/portal migration:new workforce-identity-enrollment`
- Create: `packages/portal/scripts/enroll-identity.ts`, `enroll-identity.test.ts`
- Create: `packages/portal/src/enrollment.server.ts`, `enrollment.test.ts`
- Modify: `packages/portal/src/policies.integration.test.ts` — enrollment role cases
- Modify: `packages/portal/package.json` — script `identity:enroll`
- Modify: `contrib/deploying/portal/README.md` §"Database and library enrollment" — replace the hand-run SQL with the script
- Modify: `contrib/deploying/portal/local-stack-fixture.sql` — call the function instead of raw INSERT
...
1. Add a NOLOGIN, NOBYPASSRLS function-owner role `portal_enrollment_owner`
   with explicit `INSERT, UPDATE` on `portal."identityBinding"` and its own
   RLS policies scoped to that role. Do not widen the existing `false` policies for
   runtime roles.
2. Create `portal.enroll_workforce_identity(p_issuer text, p_subject text,
   p_company_id text, p_user_id text, p_capabilities text[])` as `SECURITY
   DEFINER` owned by that role, `SET search_path = pg_catalog, public, portal`.
   ...
   grant only `portal_migrate` and `service_role`.
3. Create `portal.unbind_workforce_identity(p_issuer, p_subject, p_company_id)`
...
4. Write `enrollment.server.ts` with typed wrappers over the two functions using
   the `portal_migrate` connection, and `scripts/enroll-identity.ts`, a CLI that
...
5. Policy tests through real roles: `portal_read` cannot execute either
   function; `portal_migrate` can; an inactive user or missing membership is
...
**Verify:**
```bash
pnpm --filter @carbon/portal migrate:test
pnpm --filter @carbon/portal test:integration policies enrollment
pnpm --filter @carbon/portal test enrollment enroll-identity
...
- Create: `packages/database/supabase/migrations/<generated>_portal-identity-revocation.sql` via `pnpm db:migrate:new portal-identity-revocation`
- Create: `packages/portal/scripts/revoke-identity.ts`, `revoke-identity.test.ts`
- Modify: `packages/portal/src/cache/revocation.integration.test.ts` — trigger cases
- Modify: `apps/erp/app/modules/portal/portal.read.integration.test.ts` — revoked principal cases
- Modify: `contrib/deploying/portal/README.md` — "Emergency disablement" section
...
1. In the Carbon migration, add `AFTER UPDATE OF active ON public."user"` and
   ...
   `portal."identityBinding"` for that user (and company, for the membership
   case). Guard with `IF EXISTS` on the portal schema so the migration applies on
   installs without the portal platform.
...
pnpm --filter @carbon/portal test:integration revocation
pnpm --dir apps/erp exec vitest run app/modules/portal/portal.read.integration.test.ts
...
- Modify: `packages/portal/src/identity.server.ts` — `Principal.assurance`, caller `assurance` config
- Modify: `contrib/deploying/portal/callers.schema.json` — `assurance` object
- Modify: `packages/auth/src/services/workforce.server.ts` — MFA check after actor resolution
- Modify: `packages/auth/src/services/workforce.server.test.ts`, `packages/portal/src/identity.test.ts`
...
- Modify: `apps/portal/app/routes/unavailable.tsx` or a new `step-up.tsx` — user-facing message
...
3. In `assertWorkforceAuthorization`, deny when `assurance.required && !satisfied`
   ...
   sign-in with two-factor" plus a link to the ERP login; the portal-web route
...
**Verify:**
```bash
pnpm --filter @carbon/portal test identity
...
- Create: `packages/portal/src/identity.signature.test.ts`
- Create: `packages/portal/scripts/validate-callers.ts`
- Modify: `packages/portal/package.json` — script `callers:validate`
- Modify: `.github/workflows/fork-checks.yml` — run the validator against `contrib/deploying/portal/callers.example.json`
- Create: `contrib/deploying/portal/callers.example.json` (synthetic)
- Modify: `contrib/deploying/portal/README.md:103-112`
...
**Verify:**
```bash
pnpm --filter @carbon/portal test identity.signature validate-callers
pnpm --filter @carbon/portal callers:validate contrib/deploying/portal/callers.example.json
...
- Modify: `contrib/deploying/gcp-tailscale/config.example.json`, `secrets.example.json` — `PORTAL_TRUSTED_CALLERS_JSON` (secret), `PORTAL_RECEIVER_AUDIENCE`
...
- Modify: `apps/erp/app/routes/api+/v1+/lib/portal.http.test.ts`
- Modify: `contrib/deploying/gcp-tailscale/README.md` — portal receiver section
...
2. In `authenticate.server.ts`, catch the workforce path's `Error` and return the
   ...
   `PORTAL_TRUSTED_CALLERS_JSON` is unset. Nothing about the failure reason is
   exposed.
...
- Delete: the `PORTAL_READ_OPERATIONS` export in `apps/erp/app/modules/portal/portal.server.ts` (or make `WORKFORCE_CAPABILITIES` derive from it; one list, not two)
- Modify: `apps/erp/app/modules/portal/portal.service.ts`, `portal.models.ts` — add `getItemSupplierPricing` with its own capability `portal.read.pricing` and permission `purchasing:view`
- Modify: `scripts/lib/service-metadata.ts` — permission override for the pricing read
- Modify: `apps/erp/app/routes/api+/mcp+/lib/catalog-search.ts` and `apps/erp/app/routes/api+/v1+/openapi[.]json.ts` — exclude `portal_*` from MCP discovery and the public OpenAPI document (they are unusable there; today they are disclosed)
- Create: `apps/erp/app/routes/api+/v1+/lib/portal.gate.test.ts`
- Modify: `apps/erp/app/modules/portal/portal.read.integration.test.ts`
- Create: `apps/erp/app/modules/portal/AGENTS.md` via `.claude/skills/create-agents-md/SKILL.md`
...
1. Collapse the two allowlists into one exported constant that both the gate and
   the module consume; add a test that every `portal_*` operation in the
...
2. Pricing as a separate operation: `getItemSupplierPricing(itemId, supplierId?)`
   ...
   capability `portal.read.pricing`, permission `purchasing:view`. Existing reads
...
3. Gate tests: api-key, oauth and session callers receive `NOT_FOUND` for every
   portal operation; `callOperation` (the MCP/agent path) for a portal op
   ...
   `portal_createProcurementDraft` over real HTTP through `$.ts` gets 403; the
   pricing op requires its capability.
4. Disclosure: filter `portal_*` out of `search_tools`/`describe_tool` results
...
# Expected: one allowlist; eight portal operations pinned; non-workforce callers
# blocked on every path; no price key in read outputs; portal absent from
...
**Depends on:** 01 (pattern), 09 (IAP audience for `portal-web`)
...
- Modify: `deploy/gcp-setup.sh`, `deploy/deploy.sh`, `deploy/README.md` — provision `KANBAN_SERVICE_AUDIENCE`, `KANBAN_SOURCE_IAP_AUDIENCE`, `KANBAN_WORKFORCE_CALLERS`, `KANBAN_WORKFORCE_MACHINE_CALLERS` as secrets/config; IAP-fronted ingress for the web app; the Cloud Run invoker grant for `portal-query` and `portal-actions` service accounts
...
- Modify: `contrib/deploying/portal/main.tf` — GCS backend with per-environment prefix, read from `-backend-config`
- Modify: `contrib/deploying/portal/identity.tf` — `google_iap_client` + brand (or documented manual brand), Cloud Run IAM `run.invoker` grants: web→query, web→actions, ingest→query, and scheduler→retention already present
- Modify: `contrib/deploying/portal/services.tf` — IAP on the real `portal-web` service entry (controller-owned template, Terraform-owned IAP binding and IAM), not only the probe
- Modify: `contrib/deploying/portal/networking.tf` — use `var.private_source_cidrs`: firewall egress allow to the Carbon VM's private listener, Cloud NAT or VPC peering as the private path, TLS material reference for the Postgres listener
- Modify: `contrib/deploying/portal/outputs.tf` — per-service audiences
- Modify: `contrib/deploying/portal/release.py` — read audiences from Terraform outputs instead of free env vars
- Modify: `contrib/deploying/portal/test_infrastructure.py` — assertions for the invoker grants, IAP on web, unused-variable check; add `terraform validate` in `portal-check.yml` (already there) plus `terraform plan -refresh=false` with a synthetic tfvars file in CI where credentials allow, else keep static
...
python3 -m unittest discover -s contrib/deploying/portal -p 'test_infrastructure.py'
...
- Audit then modify as needed: `packages/portal/src/retrieval/*.server.ts`, `packages/portal/src/query/*.server.ts`, the schema fields that mark a source or document provider-eligible
- Create: `packages/portal/src/provider-policy.test.ts`
- Modify: `apps/portal-query/src/*` route tests
...
**Verify:**
```bash
pnpm --filter @carbon/portal test provider-policy
...
- Create: `.fork/decisions/<date>-portal-production-verification.md` (evidence, synthetic identifiers only)
- Modify: `contrib/deploying/portal/README.md` §"Release validation" — mark verified boundaries
...
| --- | --- | --- |
| 01 | PR #15; enrollment command and owner-role tests in [enrollment record](../decisions/2026-09-11-portal-identity-enrollment.md). PR #50/#56 repaired the real Carbon identifier-helper grants and fixture. | Exercise the integrated enrollment path on the selected Carbon-connected environment; production enrollment belongs to 11. |
| 02 | PR #24 through #30; [revocation record](../decisions/2026-09-11-portal-identity-revocation.md). Trunk tests require explicit re-enrollment after reactivation and compare monotonic version deltas. | Measure deployed propagation under 11; do not mistake restored user activity for restored admission. |
| 03 | PR #22 through #30; [assurance record](../decisions/2026-09-11-portal-authz-03-assurance.md). | Prove the selected Workspace assurance policy and access level under 11. Carbon-session evidence is not forwarded; `carbon-mfa` continues to deny when required. |
| 04 | PR #14; [real-signature and registry checks](../decisions/2026-09-11-portal-authz-04-signature-tests.md). | Real cloud caller/issuer/audience configuration remains 11; fixture signatures are not Google sign-in proof. |
| 05 | PR #20; [receiver wiring record](../decisions/2026-09-11-portal-receiver-wiring.md), with denial handling repaired in #49/#55/#56. | Live receiver configuration under 11; reproduce the outstanding non-API-key bearer and step-up classification findings before claiming complete error semantics. |
| 06 | PR #12; [session and entry-path checks](../decisions/2026-09-11-portal-authz-06-session-tests.md). | Real Google session/cookie and entry-path exercise under 11. |
| 07 | PR #27 through #30; [read-gate record](../decisions/2026-09-11-portal-authz-07-read-gate.md). PR #52/#56 added pricing transport and published-operation drift coverage. | Exercise current canonical permissions and receipt/manual resolution through the real receiver; production proof remains 11. |
| 08 | Kanban PR #2 is indexed in the [program record](../decisions/2026-09-12-portal-program-status.md). | Verify that repository's current merged revision and perform its workforce deployment/cutover; Carbon CI does not prove this. |
| 09 | PR #13; [foundation record](../decisions/2026-09-11-portal-authz-09-cloud-foundation.md) records Terraform validation and mutation tests. | No cloud apply is recorded. Database CA mounting and runtime IAM/networking must be verified before or during 11. |
| 10 | PR #11; [provider-policy checks](../decisions/2026-09-11-portal-provider-policy-verification.md). | Keep vector search and synthesis disabled pending their separate release decision and applicable acceptance. |
