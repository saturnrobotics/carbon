# Knowledge manual workflow and recovery verification
...
1. Read `.fork/agent-policy.md` and the current knowledge program status.
...
3. Run `corepack pnpm install --frozen-lockfile` and
   `corepack pnpm --filter @carbon/config build`. The knowledge test configuration
...
4. Export every stack variable from the "Running a second stack" example in
   `contrib/deploying/knowledge/README.md`, changing the name/tag/ports to unused
...
Manual interaction uses the loopback fixture's `knowledge_e2e_actor` cookie
...
```bash
KNOWLEDGE_E2E_PRESERVE_FIXTURE=0 contrib/deploying/knowledge/local-stack.sh test
