# Verify portal reads against an isolated Carbon runtime
...
Co-locate the current portal schema with Carbon for this exercise:
`PORTAL_MIGRATION_DATABASE_URL=<owned-url> pnpm --filter @carbon/portal exec
...
`portal_read` for the query runtime. Enroll the synthetic IAP subject through
`portal.enroll_workforce_identity`, using the migration connection; prove the
...
`/api/v1/portal/*` HTTP routes. Capture statuses and assert bodies; successful
...
1,016 Carbon migrations and all 38 portal migrations, successfully enrolled
...
revocation. The query used a separate `portal_read` login, actual Redis and
