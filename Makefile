...
.PHONY: help guard-clean guard-gh feature push-feature feature-pr merge-pr \
	sync-upstream upstream-pr production-tag production-status
...
feature: guard-clean ## Create feature/NAME from the latest origin/saturn/main.
	...
	@! git show-ref --verify --quiet "refs/heads/feature/$(NAME)" || { \
		echo "Local branch feature/$(NAME) already exists." >&2; exit 1; \
	}
	@! git ls-remote --exit-code --heads "$(FORK_REMOTE)" "feature/$(NAME)" >/dev/null 2>&1 || { \
		echo "Remote branch feature/$(NAME) already exists." >&2; exit 1; \
	}
	@git switch --create "feature/$(NAME)" "$(FORK_REMOTE)/$(PROD_BRANCH)"
	@echo "Created feature/$(NAME) from $(FORK_REMOTE)/$(PROD_BRANCH)."

push-feature: guard-clean ## Push the current feature/* branch to the Saturn fork.
	@branch="$$(git branch --show-current)"; \
	[[ "$$branch" == feature/* ]] || [[ "$$branch" == feat/* ]] || { echo "Current branch must be feature/*." >&2; exit 1; }; \
	git push --set-upstream "$(FORK_REMOTE)" "$$branch"

feature-pr: guard-clean guard-gh ## Open a PR from the current feature/* branch to saturn/main.
	@branch="$$(git branch --show-current)"; \
	[[ "$$branch" == feature/* ]] || [[ "$$branch" == feat/* ]] || { echo "Current branch must be feature/*." >&2; exit 1; }; \
	git push --set-upstream "$(FORK_REMOTE)" "$$branch"; \
...
production-tag: guard-clean guard-gh ## Tag the latest deployed commit; use TAG=prod-YYYY.MM.DD.N.
	...
	@deployed_sha="$$(gh run list --repo "$(FORK_REPO)" --workflow "$(PROD_WORKFLOW)" \
		--branch "$(PROD_BRANCH)" --status success --limit 1 --json headSha --jq '.[0].headSha')"; \
