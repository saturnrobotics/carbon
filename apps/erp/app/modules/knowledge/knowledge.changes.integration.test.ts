...
const url = process.env.KNOWLEDGE_OUTBOX_TEST_DATABASE_URL;
...
beforeAll(() => {
  ...
  if (!["localhost", "127.0.0.1", "[::1]"].includes(new URL(url).hostname)) {
    throw new Error(
      "Set KNOWLEDGE_OUTBOX_TEST_DATABASE_URL to an isolated local database"
    );
  }
...
async function fixture(run: (f: Fixture) => Promise<void>) {
  ...
  try {
    ...
    const company = await db
      .insertInto("company")
      .values({ name: "Knowledge Changes Fixture", baseCurrencyCode: "USD" })
      .returning("id")
...
describe.skipIf(!url)("knowledge source changes feed", () => {
  it("claims events with the projection Carbon shows now, lands a line on its receipt, and hides unposted rows", async () =>
    fixture(async (f) => {
      ...
      await db
        ...
        .where("companyId", "=", f.companyId)
        .execute();
      const page = await claimKnowledgeSourceChanges(db, {
      ...
      const other = await claimKnowledgeSourceChanges(db, {
        companyId: f.companyId,
        workerId: "worker-b",
        limit: 100
      });
      expect(other.items).toEqual([]);
      const ack = await acknowledgeKnowledgeSourceChanges(db, {
        companyId: f.companyId,
        workerId: "worker-b",
        eventIds: page.items.map((c) => c.id)
      });
      expect(ack.acknowledged).toEqual([]);
      const mine = await acknowledgeKnowledgeSourceChanges(db, {
        companyId: f.companyId,
        workerId: "worker-a",
        eventIds: page.items.map((c) => c.id)
      });
      ...
      await db
        ...
        .where("companyId", "=", f.companyId)
        .execute();
      const gone = await claimKnowledgeSourceChanges(db, {
        companyId: f.companyId,
        workerId: "worker-a",
        limit: 100
      });
  ...
  it("lists versions in byte order with the trigger's version format and projects a batch", async () =>
    fixture(async (f) => {
      await postReceipt(f);
      const versions = await listKnowledgeSourceEntityVersions(db, {
        companyId: f.companyId,
        entityType: "receipt",
        limit: 100
      });
      ...
      const items = await listKnowledgeSourceEntityVersions(db, {
        companyId: f.companyId,
        entityType: "item",
        cursor: f.itemId,
        limit: 100
      ...
      const projections = await getKnowledgeSourceEntityProjections(db, {
        companyId: f.companyId,
        entityType: "item",
      ...
      `.execute(db);
      if (foreign.rows[0]) {
        const leak = await getKnowledgeSourceEntityProjections(db, {
          companyId: f.companyId,
          entityType: "item",
