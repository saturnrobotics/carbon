...
} from "@carbon/knowledge/sources/carbon.server";
import type { SourceChange } from "@carbon/knowledge/sources/contract";
import { now } from "@internationalized/date";
import { type Kysely, sql } from "kysely";
import { z } from "zod";
import {
  type ClaimedKnowledgeEvent,
  claimPendingKnowledgeEvents,
  KNOWLEDGE_EVENT_CLAIM_LIMIT
} from "./knowledge.events.server";

/**
 * The source-changes surface the knowledge worker pulls from Carbon.
 *
 ...
 * machine-authorized `api/v1/knowledge/source-changes` route. Every reader is
...
export const sourceChangesRequestValidator = z.discriminatedUnion("action", [
  z
    .object({
      ...
      sourceId: identifier,
      workerId,
      limit: z.number().int().min(1).max(KNOWLEDGE_EVENT_CLAIM_LIMIT)
    })
    .strict(),
  z
    .object({
      ...
      sourceId: identifier,
      workerId,
      eventIds: z.array(identifier).min(1).max(KNOWLEDGE_EVENT_CLAIM_LIMIT)
    })
    .strict(),
  z
    .object({
 ...
 * outbox trigger records (`knowledge-source-outbox` migration), so a sweep
...
function receiptIdOf(event: ClaimedKnowledgeEvent): string | null {
 ...
 */
export async function claimKnowledgeSourceChanges(
  db: Db,
  input: { companyId: string; workerId: string; limit: number }
) {
  const observedAt = now("UTC").toAbsoluteString();
  const events = await claimPendingKnowledgeEvents(
    db,
    input.companyId,
    input.limit,
 ...
 */
export async function acknowledgeKnowledgeSourceChanges(
  db: Db,
 ...
 */
export async function listKnowledgeSourceEntityVersions(
  db: Db,
  input: {
    companyId: string;
    entityType: CarbonEntityType;
...
export async function getKnowledgeSourceEntityProjections(
  db: Db,
  input: {
    companyId: string;
    entityType: CarbonEntityType;
...
/** The knowledge source row this feed serves must exist, be Carbon, and be active. */
export async function isActiveCarbonKnowledgeSource(
  db: Db,
  companyId: string,
  sourceId: string
): Promise<boolean> {
