...
async function fixture(run: (f: Fixture) => Promise<void>) {
  ...
  try {
    ...
    const company = await db
      .insertInto("company")
      .values({ name: "Knowledge Outbox Fixture", baseCurrencyCode: "USD" })
      .returning("id")
...
describe.skipIf(!url)("knowledge source outbox", () => {
  ...
  it("dedupes a replayed write and a duplicate event identity", async () =>
    fixture(async (f) => {
      ...
      const duplicate = () =>
        db
          .insertInto("knowledgeSourceOutbox")
          .values({
  ...
  it("leases claims and refuses acknowledgement from a lost lease", async () =>
    fixture(async (f) => {
      await postReceipt(db, f);
      const claimedByA = await claimPendingKnowledgeEvents(
        db,
        f.companyId,
        10,
        {
      ...
      expect(
        await claimPendingKnowledgeEvents(db, f.companyId, 10, {
          workerId: "worker-b"
        })
      ).toEqual([]);
      await expect(
        acknowledgeKnowledgeEvent(db, {
          companyId: f.companyId,
          eventId,
          workerId: "worker-b"
        })
      ...
      await db
        ...
        .where("id", "=", eventId)
        .execute();
      const claimedByB = await claimPendingKnowledgeEvents(
        db,
        f.companyId,
        10,
        {
      ...
      await acknowledgeKnowledgeEvent(db, {
        companyId: f.companyId,
        eventId,
        workerId: "worker-b"
      });
  ...
  it("claims tombstones before upserts and announces lines only through a posted receipt", async () =>
    fixture(async (f) => {
      ...
      const claimed = await claimPendingKnowledgeEvents(db, f.companyId, 2, {
        workerId: "worker-a"
      });
