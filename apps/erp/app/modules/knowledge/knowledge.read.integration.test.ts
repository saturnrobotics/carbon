...
import { KNOWLEDGE_OPERATIONS } from "./knowledge.server";
import { getItemSupplierPricing, resolveItems } from "./knowledge.service";
...
describe("knowledge read outputs", () => {
  ...
  const identityReads = Object.keys(KNOWLEDGE_OPERATIONS).filter(
    (name) =>
      KNOWLEDGE_OPERATIONS[name as keyof typeof KNOWLEDGE_OPERATIONS] ===
      "knowledge.read"
  );
  ...
  it("discloses money only through the pricing read, and only the agreed fields", () => {
    expect(responseFields("knowledge_getItemSupplierPricing").sort()).toEqual([
      "currencyCode",
      "supplierId",
      "supplierUnitPrice",
      "unitOfMeasureCode",
...
function disposableStack(): DisposableStack | null {
  const url = process.env.KNOWLEDGE_READ_TEST_SUPABASE_URL;
  const anonKey = process.env.KNOWLEDGE_READ_TEST_ANON_KEY;
  const serviceRoleKey = process.env.KNOWLEDGE_READ_TEST_SERVICE_ROLE_KEY;
  const jwtSecret = process.env.KNOWLEDGE_READ_TEST_JWT_SECRET;
  if (!url || !anonKey || !serviceRoleKey || !jwtSecret) return null;
  const { hostname } = new URL(url);
  if (
    process.env.KNOWLEDGE_READ_TEST_DISPOSABLE !== "1" ||
    !["localhost", "127.0.0.1", "[::1]"].includes(hostname)
  ) {
    throw new Error(
      "Set KNOWLEDGE_READ_TEST_SUPABASE_URL to a local stack and KNOWLEDGE_READ_TEST_DISPOSABLE=1"
    );
  }
...
describe.skipIf(!stack)("knowledge reads under row-level security", () => {
  ...
  beforeAll(async () => {
    ...
    const companyId = await insert("company", {
      name: "Knowledge Read Fixture",
      baseCurrencyCode: "USD"
    });
    const otherCompanyId = await insert("company", {
      name: "Knowledge Read Fixture (other)",
