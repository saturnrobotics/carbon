...
export async function resolveItems(
  ...
  const term = portalItemSearch.parse(search);
  const boundedLimit = portalPageLimit.parse(limit);
  return await client
    .from("item")
...
export async function getItemIdentity(
  ...
  return await client
    ...
    .select(ITEM_IDENTITY_FIELDS)
    .eq("companyId", companyId)
    .eq("id", portalIdentifier.parse(itemId))
    .maybeSingle();
}
...
export async function getRecentReceipts(
  ...
  return await client
    ...
    .limit(portalPageLimit.parse(limit));
}
...
export async function getRecentReceiptItems(
  ...
  const validatedItemIds = itemIds
    ? z.array(portalIdentifier).min(1).max(50).parse(itemIds)
    : undefined;
...
export async function getDocumentReferences(
  ...
  const prefix = `${companyId}/parts/${portalIdentifier.parse(itemId)}`;
 ...
 * portal read that discloses money: it is a separate operation with its own
 * capability (`portal.read.pricing`) and gates on purchasing view, so the
...
export async function getItemSupplierPricing(
  ...
  let partsQuery = client
    ...
    .eq("companyId", companyId)
    .eq("itemId", portalIdentifier.parse(itemId))
    .eq("active", true)
  ...
  if (supplierId) {
    partsQuery = partsQuery.eq(
      "supplierId",
      portalIdentifier.parse(supplierId)
    );
  }
...
export async function getPurchaseStatus(
  ...
  return await client
    ...
    .eq("companyId", companyId)
    .or(
      `id.eq.${portalIdentifier.parse(purchaseOrderId)},purchaseOrderId.eq.${portalIdentifier.parse(purchaseOrderId)}`
    )
    .maybeSingle();
}
