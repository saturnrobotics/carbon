...
| `portalCommandReceipt` | Written by `createProcurementDraft` inside its transaction: actor, company, action, idempotency key, payload hash and `version`, plus the PO it created. Its uniqueness on `(companyId, actorId, action, idempotencyKey)` is the concurrency authority — a retry that loses the race rolls back and replays the committed order |
...
- `createProcurementDraft` / `resolveProcurementDraft` — the portal command's Draft-PO transaction and its read-only preflight (`procurementDraftInputValidator` + `PROCUREMENT_DRAFT_PAYLOAD_VERSION` in `purchasing.models.ts`); proven against the real schema by `procurement-draft.integration.test.ts`
