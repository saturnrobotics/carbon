...
vi.mock(
  "~/modules/knowledge/knowledge.changes.server",
  async (importOriginal) => ({
    ...(await importOriginal<
      typeof import("~/modules/knowledge/knowledge.changes.server")
    >()),
    claimKnowledgeSourceChanges: mocks.claim,
    acknowledgeKnowledgeSourceChanges: mocks.acknowledge,
    listKnowledgeSourceEntityVersions: mocks.versions,
    getKnowledgeSourceEntityProjections: mocks.projections,
    isActiveCarbonKnowledgeSource: mocks.activeSource
  })
);
vi.mock(
  "@carbon/knowledge/machine-identity.server",
  async (importOriginal) => ({
    ...(await importOriginal<
      typeof import("@carbon/knowledge/machine-identity.server")
    >()),
...
import { action, loader } from "../knowledge.source-changes";
...
function post(body: unknown, headers: Record<string, string> = {}) {
  return action({
    request: new Request(
      "https://erp.example/api/v1/knowledge/source-changes",
      {
        method: "POST",
        headers: {
...
beforeEach(() => {
  process.env.KNOWLEDGE_MACHINE_CALLERS_JSON = configuration;
...
afterEach(() => {
  delete process.env.KNOWLEDGE_MACHINE_CALLERS_JSON;
});

describe("knowledge source-changes route", () => {
