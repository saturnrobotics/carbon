...
// The portal source-table triggers, for the suites that prove what
// portal_source_outbox_enqueue() records.
//
// 20260913060708_knowledge-source-outbox-triggers-off.sql detaches all four:
// the portal platform ships as a read-only pilot that does not consume the
...
// right: portal.outbox.integration, portal.changes.integration and
 ...
 * `portal_source_outbox_enqueue()`, not in the trigger definitions.
 */
const OUTBOX_TRIGGERS = [
  { name: "portal_source_outbox_receipt_trigger", table: "receipt" },
  {
    name: "portal_source_outbox_receipt_line_trigger",
    table: "receiptLine"
  },
  { name: "portal_source_outbox_item_trigger", table: "item" },
  {
    name: "portal_source_outbox_purchase_order_trigger",
    table: "purchaseOrder"
  }
] as const;
...
function testDatabaseUrl(): string | null {
  const url =
    process.env.PORTAL_OUTBOX_TEST_DATABASE_URL ??
    ...
    for (const trigger of OUTBOX_TRIGGERS) {
      ...
      await sql`
        CREATE TRIGGER ${sql.id(trigger.name)}
          ...
          FOR EACH ROW EXECUTE FUNCTION portal_source_outbox_enqueue()
      `.execute(db);
