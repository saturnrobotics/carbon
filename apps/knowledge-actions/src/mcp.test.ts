...
const environment = {
  ...
  KNOWLEDGE_CARBON_SOURCE_URL: "https://erp.example.com",
  KNOWLEDGE_IDENTITY_RESOLVER_AUDIENCE: "resolver-audience",
  KNOWLEDGE_IDENTITY_RESOLVER_URL: "https://query.example.com",
  KNOWLEDGE_KANBAN_SOURCE_AUDIENCE: "kanban-audience",
  KNOWLEDGE_KANBAN_SOURCE_ID: "kanban",
  KNOWLEDGE_KANBAN_SOURCE_URL: "https://kanban.example.com",
...
describe("the command service's MCP mount", () => {
  it("is 404 by default, including with the flag alone", async () => {
    for (const extra of [{}, { KNOWLEDGE_MCP_ENABLED: "true" }]) {
  ...
  it("exposes only the reviewed ticket command, and runs it through the HTTP handler", async () => {
    ...
    ).toEqual(["knowledge_create_ticket"]);
    const called = await (
      await handler(
        rpc("tools/call", {
