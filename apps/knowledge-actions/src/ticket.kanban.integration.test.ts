 ...
 *
 * It runs only when `KNOWLEDGE_KANBAN_BACKEND_DIR` names a checkout of the
...
const backendDirectory = process.env.KNOWLEDGE_KANBAN_BACKEND_DIR;
...
describe.skipIf(!backendDirectory)(
  "knowledge-actions against a real Kanban backend",
  () => {
    ...
    beforeAll(async () => {
      ...
      directory = mkdtempSync(join(tmpdir(), "knowledge-actions-kanban-"));
