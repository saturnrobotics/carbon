import type { CarbonChangeFeed } from "@carbon/knowledge/sources/carbon.server";
...
vi.mock("@carbon/knowledge/sources/carbon.server", async (importOriginal) => {
  const actual =
    await importOriginal<
      typeof import("@carbon/knowledge/sources/carbon.server")
    >();
  return {
    ...actual,
...
describe("Carbon change consumer", () => {
  ...
  it("registers only when the Carbon source is configured", () => {
    ...
    expect(() =>
      readCarbonSourceConfiguration({
        KNOWLEDGE_CARBON_SOURCE_JSON: '{"sourceId":"x","origin":"not a url"}'
      })
    ).toThrow();
  });
});
