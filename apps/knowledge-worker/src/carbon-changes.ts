...
/**
 * Carbon → knowledge change consumer (the worker half of plan Task 15).
 *
 * `knowledge-carbon-changes` pulls leased events from Carbon's source-changes
 * route, projects them into `knowledge.entity` (and invalidation events into
 * the knowledge outbox), and acknowledges only what was committed. Every step
 ...
 *
 * `knowledge-carbon-reconcile` is the periodic sweep for what the trigger
...
export function readCarbonSourceConfiguration(
  ...
  const raw = environment.KNOWLEDGE_CARBON_SOURCE_JSON?.trim();
  if (!raw) return null;
...
export async function sweepCarbonEntities(
  ...
  const knowledgeEntityType = CARBON_KNOWLEDGE_ENTITY_TYPES[state.entityType];
  ...
  if (stale.length) {
    ...
    if (projections.status !== "unavailable") {
      ...
      const changes: SourceChange[] = stale.map((entityId) => ({
        ...
        target: { entityType: knowledgeEntityType, entityId },
...
export function createCarbonChangeFunction(runtime: CarbonChangeRuntime) {
  return knowledgeInngest.createFunction(
    {
      id: "knowledge-carbon-changes",
      retries: 3,
      concurrency: [{ limit: 1, key: "event.data.companyId" }]
    },
    [{ event: "knowledge/carbon.changes" }, { cron: "*/1 * * * *" }],
    async ({ event, step }) => {
...
export function createCarbonReconciliationFunction(
  ...
  return knowledgeInngest.createFunction(
    {
      id: "knowledge-carbon-reconcile",
      retries: 2,
      concurrency: [{ limit: 1, key: "event.data.companyId" }]
    },
    [{ event: "knowledge/carbon.reconcile" }, { cron: "*/15 * * * *" }],
    async ({ event, step }) => {
      const requested = requestedCompany(event);
      const companies = requested
        ? runtime.companies.filter((entry) => entry.companyId === requested)
