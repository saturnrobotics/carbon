...
function dependencies(
  ...
  const read = fakePool((text) =>
    text.includes("FROM knowledge.document d JOIN knowledge.source")
      ? { rows: overrides.visible ? [{}] : [] }
      : text.includes('FROM knowledge."driveEnrollment" e')
        ? { rows: [] }
        : { rows: [] }
  );
  const ingest = fakePool((text) =>
    text.includes('FROM knowledge."driveItem"')
      ? { rows: overrides.item ? [overrides.item] : [] }
      : text.includes('FROM knowledge."driveEnrollment"')
        ? { rows: overrides.channelMatches ? [{}] : [] }
        : { rows: [] }
  );
  const review = fakePool((text) =>
    text.includes("UPDATE knowledge.source")
