...
async function verifyReader(
  ...
  const identity = await dependencies.verifyHuman(request, "knowledge.read");
  if (!identity.principal.capabilities.includes("knowledge.read"))
...
export async function handleDriveRoute(
  ...
  if (access && request.method === "POST") {
    ...
    const visible = await withKnowledgeTransaction(
      ...
      async (client) =>
        (
          await client.query(
            `SELECT 1 FROM knowledge.document d JOIN knowledge.source s ON s."companyId"=d."companyId" AND s.id=d."sourceId"
  ...
  if (sync && request.method === "POST") {
    ...
    // UPDATE policy on knowledge.source is the admin check, and the write it
    // performs is the audit mark of who asked.
    const admitted = await withKnowledgeTransaction(
      ...
      async (client) =>
        (
          await client.query(
            `UPDATE knowledge.source SET "updatedBy"=$3,"updatedAt"=now(),version=version+1
