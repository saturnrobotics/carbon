 ...
 * real Redis are covered by packages/knowledge cache/revocation.integration.
 */
function recordingPool(options: { tombstoned: ReadonlySet<string> }) {
  ...
  const client = {
    async query(text: string, values?: unknown[]) {
      ...
      if (text.includes("UPDATE knowledge.source"))
        return {
      ...
      if (text.includes("SELECT 1 FROM knowledge.document"))
        return {
...
describe("outbox invalidation batch", () => {
  it("advances epochs before confirming or acknowledging any event", async () => {
    ...
    const tombstoneConfirm = statements.findIndex(
      (s, index) =>
        index > firstAck && s.includes("SELECT 1 FROM knowledge.document")
    );
    ...
    expect(
      statements.filter((s) => s.includes("SELECT 1 FROM knowledge.document"))
    ).toHaveLength(2);
  });
  it("defers an unconfirmed tombstone without blocking the rest of the batch", async () => {
    ...
    expect(statements.some((s) => s.includes("UPDATE knowledge.source"))).toBe(
      true
    );
    expect(
