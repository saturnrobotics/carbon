...
} from "@carbon/knowledge/cache/epochs.server";
import {
  type DatabasePrincipal,
  withKnowledgeTransaction
} from "@carbon/knowledge/database.server";
import {
  acknowledgeOutbox,
  claimOutbox,
...
export async function applyOutboxInvalidation(
  ...
  if (plan.length)
    await withKnowledgeTransaction(pool, principal, "write", (client) =>
...
/**
 * Cache-invalidation consumer of the knowledge outbox. It leases only the
...
export function createOutboxInvalidationFunction(runtime: {
  ...
  return knowledgeInngest.createFunction(
    {
      id: "knowledge-outbox-invalidation",
      retries: 3,
