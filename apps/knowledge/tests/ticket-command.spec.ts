 ...
 * `apps/knowledge/app/routes.ts` is the production route manifest and names
 * none of `api.commands`, `api.propose-command` or `api.transcribe`;
 * `test_production_web_route_manifest_excludes_deferred_routes` in
 * `contrib/deploying/knowledge/test_images.py` pins that. A React Router route
 ...
 * `apps/knowledge-actions/src/ticket.kanban.integration.test.ts` and the Kanban
