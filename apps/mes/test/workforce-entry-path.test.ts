...
const REDIRECT_TO = "/x/operations?tab=1";
...
beforeEach(() => {
  ...
  harness.memberships.mockResolvedValue({
...
describe("MES workforce entry path", () => {
  ...
  it("routes a user with a verified TOTP factor to /mfa before any full session exists", async () => {
    ...
    const pending = cookies.find((cookie) => cookie.startsWith("carbon-mes="));
    expect(pending).toBeDefined();
    expect(pending).not.toMatch(/Domain=/);
    expect(pending).toMatch(/Secure/);

    // The pending cookie carries the first factor and nothing readable as a
    // full session.
  ...
  it("expires the legacy parent-domain cookie on every callback response", async () => {
    ...
    expect(full.headers.get("Location")).toBe("/x");
    expect(
      setCookies(full).filter((cookie) => legacy.test(cookie))
    ).toHaveLength(1);
    expect(
      setCookies(full).some((cookie) => cookie.startsWith("carbon-mes="))
    ).toBe(true);
  });
});
