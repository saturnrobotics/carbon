...
import { requestBoundary } from "@carbon/portal/query/request-boundary.server";
...
export const serviceName = "portal-actions";

const REQUIRED_ACTIONS_CONFIGURATION = [
  "PORTAL_CARBON_SOURCE_AUDIENCE",
  "PORTAL_CARBON_SOURCE_URL",
  "PORTAL_IDENTITY_RESOLVER_AUDIENCE",
  "PORTAL_IDENTITY_RESOLVER_URL",
  "PORTAL_KANBAN_SOURCE_AUDIENCE",
  "PORTAL_KANBAN_SOURCE_ID",
  "PORTAL_KANBAN_SOURCE_URL",
...
export function createHandler(environment: NodeJS.ProcessEnv = process.env) {
  const callers = environment.PORTAL_TRUSTED_CALLERS_JSON;
  const resolverUrl = environment.PORTAL_IDENTITY_RESOLVER_URL;
  const resolverAudience = environment.PORTAL_IDENTITY_RESOLVER_AUDIENCE;
  const sourceId = environment.PORTAL_KANBAN_SOURCE_ID;
  const sourceUrl = environment.PORTAL_KANBAN_SOURCE_URL;
  const sourceAudience = environment.PORTAL_KANBAN_SOURCE_AUDIENCE;
  const carbonUrl = environment.PORTAL_CARBON_SOURCE_URL;
  const carbonAudience = environment.PORTAL_CARBON_SOURCE_AUDIENCE;
  ...
  const procurementRoute = async (request: Request) => {
    ...
    return handleProcurementCommand(request, {
      ...
      verifyWorkforce: (incoming) =>
        verifyWorkforceRequest({
          request: incoming,
          operation: "portal_createProcurementDraft",
          configuration,
          tokenVerifier: verifier,
          identityStore: createRemoteWorkforceIdentityStore({
            request: incoming,
