...
} from "@carbon/portal/cache/isolation.server";
import { createRedisCache } from "@carbon/portal/cache/redis.server";
import { createTelemetry, type Telemetry } from "@carbon/portal/telemetry";
...
export function startCacheIsolationProbe(
  ...
  const url = environment.PORTAL_REDIS_URL?.trim();
  const store =
