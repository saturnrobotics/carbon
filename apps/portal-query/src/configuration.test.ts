...
const rejected: [reason: string, value: string][] = [
  ...
  [
    ...
    JSON.stringify({
      version: 1,
      sources: [
        { ...registry.sources[0], origin: "https://erp.example/portal" }
      ]
    })
  ],
  [
...
describe("source registry configuration", () => {
  it("reaches both read paths when a registry is configured", () => {
    createHandler({
      ...environment,
      PORTAL_SOURCES_JSON: JSON.stringify(registry)
    });
  ...
  it.each(
    ...
    const broken = { ...environment, PORTAL_SOURCES_JSON: value };
  ...
  it("accepts a registry holding every registered connector kind", () => {
    ...
    expect(
      readSourceRegistryConfiguration({
        ...environment,
        PORTAL_SOURCES_JSON: JSON.stringify(every)
      })
    ).toEqual(every);
  });
});
