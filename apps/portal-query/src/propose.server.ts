...
import { durableBudget } from "@carbon/portal/budgets.server";
    ...
    async (client) =>
      (
        await client.query<{ id: string; kind: string }>(
          "SELECT id,kind FROM portal.source WHERE \"companyId\"=$1 AND status='active' ORDER BY id LIMIT 5",
      ...
      const identity = await (
        ...
        ((incoming) =>
          verifyWorkforceRequest({
            ...options,
            request: incoming,
            operation: "portal.propose-command"
          }))
      )(request);
