...
import { admitReadRequest, durableBudget } from "@carbon/portal/budgets.server";
...
import { assertProviderCandidates } from "@carbon/portal/provider-policy";
import {
  executeReadQuery,
  type QueryResult,
  queryResultSchema,
  type ReadDependencies
} from "@carbon/portal/query";
import {
  conservativeTokenCount,
  QUERY_BUDGETS
} from "@carbon/portal/query/budgets";
import type { ConversationState } from "@carbon/portal/query/conversation";
import {
  createVertexEmbedder,
  type EmbeddingConfiguration
} from "@carbon/portal/query/embedding.server";
import { requestTelemetry } from "@carbon/portal/query/request-boundary.server";
import { routeQuery } from "@carbon/portal/query/router";
import {
  acceptsQueryStream,
...
} from "@carbon/portal/query/vertex.server";
import { assembleEvidence } from "@carbon/portal/retrieval/evidence";
import { reciprocalRankFusion } from "@carbon/portal/retrieval/fusion";
import {
  lexicalSearch,
  type RetrievedChunk
} from "@carbon/portal/retrieval/lexical.server";
import { loadParentSections } from "@carbon/portal/retrieval/sections.server";
import { vectorSearchApproximate } from "@carbon/portal/retrieval/vector.server";
import type { SourceRegistryConfiguration } from "@carbon/portal/sources/registry.server";
import {
  StepUpRequiredError,
  stepUpRequiredResponse
} from "@carbon/portal/step-up";
    ...
    try {
      ...
      if (
        !(await trace.measure("policy", () =>
          admitReadRequest(options.pool, principal, "portal.query")
        ))
      )
        return Response.json(
      ...
      if (options.sources && route.structured) {
        ...
        if (structured) {
          ...
          if (
            ...
            !current.capabilities.includes("portal.read")
          )
        ...
        async (client) =>
          (
            await client.query<{ id: string }>(
              `SELECT id FROM portal.source WHERE "companyId"=$1 AND status='active' AND ($2::text IS NULL OR id=$2) AND ($3::boolean=false OR kind='upload') ORDER BY id LIMIT $4`,
              [
                principal.companyId,
