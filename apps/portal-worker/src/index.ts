...
import { outboxBacklog } from "@carbon/portal/indexing/outbox.server";
...
export const serviceName = "portal-worker";
...
export function startServer(
  ...
  const parserLocation = environment.PORTAL_PARSER_LOCATION;
  ...
  if (dependencies && !schedulerConfiguration) {
    ...
    if (driveSources.length)
      dependencies.requestDriveSync = async (input) => {
        await portalInngest.send({
          name: "portal/drive.sync",
          data: input,
          id: `portal-drive-${input.sourceId}-${crypto.randomUUID()}`
        });
      };
  }
