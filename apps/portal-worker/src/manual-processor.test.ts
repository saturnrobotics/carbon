...
function runtime(inputRefs: unknown = []) {
  const query = vi.fn(async (sql: string) => ({
    rows: sql.includes("FROM portal.source")
      ? [{}]
      : sql.includes("FROM portal.intake")
