...
vi.mock("@carbon/portal/intake", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@carbon/portal/intake")>()),
...
function dependencies(
  ...
  return {
    verifyHuman: vi
      .fn()
      .mockResolvedValue(
        verifiedIdentity(["portal.intake.capture", "portal.intake.review"])
      ),
...
it("rejects a verified workforce identity without the route capability", async () => {
  const deps = dependencies({
    verifyHuman: vi.fn().mockResolvedValue(verifiedIdentity(["portal.read"]))
  });
