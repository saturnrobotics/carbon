...
import { writeWebResponse } from "@carbon/portal/query/request-boundary.server";
import { Pool } from "pg";
...
const callerId = "portal-e2e-loopback";
...
// owner function, the only writer of portal."identityBinding".
...
function rolePool(
  ...
  role: "portal_read" | "portal_review"
) {
  return new Pool({
    connectionString,
    options: `-c role=${role}`,
...
export async function startE2eGateway(
  ...
  const readPool = rolePool(connectionString, "portal_read");
  const reviewPool = rolePool(connectionString, "portal_review");
  ...
  await fixturePool.query(
    `DELETE FROM portal."grant" WHERE "companyId"=$1
  ...
  await fixturePool.query(
    "SELECT portal.enroll_workforce_identity($1::text,$2::text,$3::text,'bob',ARRAY['portal.read']::text[])",
  ...
  await bridge(workerServer, async (request) => {
    ...
    if (
      ...
      await fixturePool.query(
        `UPDATE portal.intake SET state='needs-review', extraction=$3::jsonb, version=version+1
      ...
      await fixturePool.query(
        `INSERT INTO portal.extraction ("companyId","createdBy","intakeId",generation,"providerProfile","sourceVersions",output,status)
  ...
  return {
    async close() {
      ...
      if (capturedIntakes.size) {
        ...
        await fixturePool.query(
          `DELETE FROM portal.audit WHERE "companyId"=$1 AND "requestId" LIKE 'intake-%' AND "targetRefs"::text LIKE '%e2e%'`,
          [companyId]
        );
        await fixturePool.query(
          `DELETE FROM portal.outbox WHERE "companyId"=$1 AND "entityId"=ANY($2::text[])`,
          [companyId, ids]
        );
        const documents = await fixturePool.query<{ id: string }>(
          `SELECT id FROM portal.document WHERE "companyId"=$1 AND "sourceItemId"=ANY($2::text[])`,
          [companyId, ids.map((id) => `intake:${id}`)]
        );
        const documentIds = documents.rows.map((document) => document.id);
        if (documentIds.length) {
          await fixturePool.query(
            `UPDATE portal.document SET "currentVersionId"=NULL,version=version+1 WHERE "companyId"=$1 AND id=ANY($2::text[])`,
            [companyId, documentIds]
          );
          await fixturePool.query(
            `DELETE FROM portal.chunk WHERE "companyId"=$1 AND "documentId"=ANY($2::text[])`,
            [companyId, documentIds]
          );
          await fixturePool.query(
            `DELETE FROM portal."documentVersion" WHERE "companyId"=$1 AND "documentId"=ANY($2::text[])`,
            [companyId, documentIds]
          );
          await fixturePool.query(
            `DELETE FROM portal."grant" WHERE "companyId"=$1 AND "documentId"=ANY($2::text[])`,
            [companyId, documentIds]
          );
          await fixturePool.query(
            `DELETE FROM portal.document WHERE "companyId"=$1 AND id=ANY($2::text[])`,
            [companyId, documentIds]
          );
        }
        await fixturePool.query(
          `DELETE FROM portal.extraction WHERE "companyId"=$1 AND "intakeId"=ANY($2::text[])`,
          [companyId, ids]
        );
        await fixturePool.query(
          `DELETE FROM portal.intake WHERE "companyId"=$1 AND id=ANY($2::text[])`,
          [companyId, ids]
        );
      }
      await fixturePool.query(
        `DELETE FROM portal."grant" WHERE "companyId"=$1 AND id IN ('e2e-bob-read','e2e-bob-publish')`,
        [companyId]
      );
      if (bobRevoked)
        await fixturePool.query(
