/**
 * Loopback-only Drive connector fixture for `apps/portal/tests/drive-source.spec.ts`.
 *
 ...
 * `PORTAL_E2E_QUERY_PORT` (query, 4302 by default), plus test controls:
...
import { parseStoredExtraction } from "@carbon/portal/intake/contracts";
import {
  getDriveEnrollment,
...
function principal(actor: "bob" | "alice"): VerifiedWorkforceIdentity {
  return {
    ...
    allowedOperations: ["portal.query", "portal.read"],
...
function browserOrigin(): string {
  const value =
    process.env.PORTAL_WEB_ORIGIN?.trim() ?? "https://localhost:4200";
...
async function main() {
  ...
  const databaseUrl = process.env.PORTAL_E2E_DATABASE_URL?.trim();
  if (!databaseUrl) throw new Error("PORTAL_E2E_DATABASE_URL is required");
  ...
  const cleanup = async () => {
    ...
    await admin.query(
      `DELETE FROM portal."identityBinding" WHERE "companyId"=$1 AND id='e2e-drive-bob-binding'`,
      [localCompanyId]
    );
  };
  ...
  const seed = async () => {
    ...
    await admin.query(
      `INSERT INTO portal."driveEnrollment" ("companyId","createdBy","sourceId",corpora,"driveId","rootFolderIds","oauthScope","credentialSecretRef")
    ...
    await admin.query(
      `INSERT INTO portal."identityBinding" (id,"companyId","createdBy",issuer,subject,"canonicalUserId",active,"revocationVersion",capabilities)
     VALUES ('e2e-drive-bob-binding',$1,'bob','https://cloud.google.com/iap','subject-b','bob',true,1,ARRAY['portal.read']::text[])
  ...
  const query = createReadHandler({
    ...
    cacheStore: {
      ...
      set: async () => {
        ...
        // proven by the packages/portal integration suite.
      }
    },
  ...
  const queryServer = startLocalHttpServer({
    port: loopbackPort("PORTAL_E2E_QUERY_PORT", 4302),
    maximumBytes: 32_768,
    handler: async (request) => {
      const url = new URL(request.url);
      if (request.method === "GET" && url.pathname === "/health")
