...
export const localCallerId = "portal-e2e-loopback";
export const localBucket = "portal-e2e";
...
// contrib/deploying/portal/local-stack-fixture.sql enrolls.
export const actorSubjects = {
...
export async function cleanCapturedIntakes(
  ...
  await pool.query(
    `DELETE FROM portal.audit WHERE "companyId"=$1
       AND ("targetRefs"->>'intakeId'=ANY($2::text[])
         OR "targetRefs"->>'documentId' IN (
           SELECT id FROM portal.document WHERE "companyId"=$1
  ...
  await pool.query(
    `DELETE FROM portal.outbox WHERE "companyId"=$1
       AND ("entityId"=ANY($2::text[]) OR "entityId"=ANY($3::text[]))`,
    [localCompanyId, intakeIds, documentIds]
  );
  if (documentIds.length) {
    await pool.query(
      `UPDATE portal.document SET "currentVersionId"=NULL,version=version+1
       WHERE "companyId"=$1 AND id=ANY($2::text[])`,
      [localCompanyId, documentIds]
    );
    await pool.query(
      `DELETE FROM portal.chunk WHERE "companyId"=$1
       AND "documentId"=ANY($2::text[])`,
      [localCompanyId, documentIds]
    );
    await pool.query(
      `DELETE FROM portal."documentVersion" WHERE "companyId"=$1
       AND "documentId"=ANY($2::text[])`,
      [localCompanyId, documentIds]
    );
    await pool.query(
    ...
    await pool.query(
      `DELETE FROM portal.document WHERE "companyId"=$1
       AND id=ANY($2::text[])`,
      [localCompanyId, documentIds]
    );
  }
  await pool.query(
    `DELETE FROM portal.extraction WHERE "companyId"=$1
       AND "intakeId"=ANY($2::text[])`,
    [localCompanyId, intakeIds]
  );
  await pool.query(
    `DELETE FROM portal.intake WHERE "companyId"=$1
       AND id=ANY($2::text[])`,
    [localCompanyId, intakeIds]
  );
}
...
export const syntheticItemSourceFetch: typeof fetch = async (input, init) => {
  ...
  if (url.pathname !== "/api/v1/portal/resolveItems" || init?.method !== "POST")
