import { assertExecutableTicketCommandBrowser } from "@carbon/portal/commands/ticket.browser";
...
export async function action({ request }: { request: Request }) {
  const actionsUrl = process.env.PORTAL_ACTIONS_URL;
  const actionsAudience = process.env.PORTAL_ACTIONS_AUDIENCE;
  ...
  return forwardTicketCommand(request, {
    actionsUrl,
    verifyWorkforce: verifyPortalBrowserRequest,
