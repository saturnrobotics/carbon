 ...
 * caller media, then relays the unchanged multipart bytes to portal-query.
 */
...
export async function action({ request }: { request: Request }) {
  ...
  // from browser input. portal-query still proves the resulting membership.
  ...
  try {
    return await forwardTranscription(request, {
      queryUrl,
      queryAudience,
      verifyBrowser: verifyPortalBrowserRequest,
