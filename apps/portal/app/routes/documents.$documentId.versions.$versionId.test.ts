 ...
 */
const environment = {
  PORTAL_COMPANY_ID: "company_synthetic",
  PORTAL_WORKER_URL: "https://worker.example.test",
  PORTAL_WORKER_AUDIENCE: "https://worker.example.test",
  PORTAL_WEB_IAP_AUDIENCE: "/projects/000/global/backendServices/portal"
};

function download() {
  return new Request(
    "https://portal.example.test/documents/doc/versions/version"
  );
}
...
describe("a document version download", () => {
  ...
  it("keeps a misconfiguration an outage rather than a denial", async () => {
    ...
    expect(body).not.toContain("Portal company selection is not configured");
  });
});
