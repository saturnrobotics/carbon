...
function assertLabelledDisposable(): void {
  ...
  if (inspection[0]?.Config?.Labels?.["portal.disposable"] !== "true")
    throw new Error(
...
export function revokeLibraryGrant(): void {
  fixtureSql(
    `UPDATE portal."grant" SET "revokedAt"=now(),version=version+1 WHERE id='${libraryGrant}' AND "companyId"='company-b' AND "revokedAt" IS NULL`
  );
}

export function restoreLibraryGrant(): void {
  fixtureSql(
    `UPDATE portal."grant" SET "revokedAt"=NULL,version=version+1 WHERE id='${libraryGrant}' AND "companyId"='company-b'`
  );
}
