...
const port = Number(process.env.PORTAL_E2E_PORTAL_PORT ?? "4200");
if (!Number.isInteger(port) || port < 1024 || port > 65_535)
  throw new Error("PORTAL_E2E_PORTAL_PORT must be an unprivileged port");
const browserOrigin = new URL(
  process.env.PORTAL_WEB_ORIGIN ?? `https://localhost:${port}`
);
if (
...
function loopbackIdentityOnly(): Plugin {
  return {
    name: "portal-e2e-loopback-identity-only",
    enforce: "pre",
...
export default defineConfig({
  ...
  server: {
    ...
    host: process.env.PORTAL_E2E_DOCKER === "1" ? "0.0.0.0" : "127.0.0.1",
    port,
    strictPort: true,
    https: {
