"""Optional trusted-caller registry for the ERP knowledge (workforce) receiver.
...
CONFIG_KEYS = {"KNOWLEDGE_RECEIVER_AUDIENCE"}
SECRET_KEYS = {"KNOWLEDGE_TRUSTED_CALLERS_JSON"}
SECRET_NAME = "knowledge_trusted_callers_json"
...
def _validate_registry(registry):
    ...
    if not isinstance(registry, dict) or set(registry) != {"version", "receiver", "callers"} or registry["version"] != 1:
        raise ValueError("KNOWLEDGE_TRUSTED_CALLERS_JSON must be a version 1 registry with receiver and callers")
    ...
    if not isinstance(receiver, dict) or set(receiver) != {"id", "audience"} or not all(_bounded_string(receiver[key]) for key in ("id", "audience")):
        raise ValueError("KNOWLEDGE_TRUSTED_CALLERS_JSON receiver needs an id and an audience")
    callers = registry["callers"]
    if not isinstance(callers, list) or not 1 <= len(callers) <= 100:
        raise ValueError("KNOWLEDGE_TRUSTED_CALLERS_JSON needs between one and one hundred callers")
    subjects = set()
...
def validate(config):
    audience = config.get("KNOWLEDGE_RECEIVER_AUDIENCE", "")
    if not isinstance(audience, str) or (audience and not re.fullmatch(r"https://[a-z0-9]+(?:[.-][a-z0-9]+)*(?:/[A-Za-z0-9._~/-]*)?", audience)):
        raise ValueError("Invalid KNOWLEDGE_RECEIVER_AUDIENCE; use the HTTPS URL callers mint their service tokens for")
    registry = config.get("KNOWLEDGE_TRUSTED_CALLERS_JSON", "")
    if not isinstance(registry, str) or len(registry) > 100_000 or any(ord(c) < 32 for c in registry):
        raise ValueError("Invalid KNOWLEDGE_TRUSTED_CALLERS_JSON; use a single-line private JSON value")
    if not registry:
        return
    if registry.startswith("replace-"):
        raise ValueError("KNOWLEDGE_TRUSTED_CALLERS_JSON is still the example placeholder; supply the registry or remove the key")
    if not audience:
        raise ValueError("Set KNOWLEDGE_RECEIVER_AUDIENCE before configuring KNOWLEDGE_TRUSTED_CALLERS_JSON")
    try:
        parsed = json.loads(registry)
    except ValueError:
        raise ValueError("KNOWLEDGE_TRUSTED_CALLERS_JSON must contain a JSON object") from None
    if _validate_registry(parsed) != audience:
        raise ValueError("KNOWLEDGE_RECEIVER_AUDIENCE must equal the registry's receiver audience")


def configure(config, erp, directory, write_private, *, materialize=True):
    validate(config)
    erp["environment"]["KNOWLEDGE_RECEIVER_AUDIENCE"] = config.get("KNOWLEDGE_RECEIVER_AUDIENCE", "")
    registry = config.get("KNOWLEDGE_TRUSTED_CALLERS_JSON", "")
    if registry:
        ...
        erp["environment"]["KNOWLEDGE_TRUSTED_CALLERS_JSON"] = "__KNOWLEDGE_TRUSTED_CALLERS_JSON__"
    else:
        # Removing the registry must not resurrect an old file: an empty value
        # leaves the receiver refusing every workforce request.
        erp["environment"]["KNOWLEDGE_TRUSTED_CALLERS_JSON"] = ""
