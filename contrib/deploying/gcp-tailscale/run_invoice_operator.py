...
class ReleaseCompatibilityError(ValueError):
    """Public-safe recovery guidance, without private input values."""


def deployed_erp_revision(runtime, stack):
    ops, erp = stack["services"]["ops"], stack["services"]["erp"]
    receipt_path = runtime / "release-manifest.json"
    if receipt_path.exists():
        receipt = json.loads(receipt_path.read_text())
        services = receipt.get("services", {}) if isinstance(receipt, dict) else {}
        deployed = services.get("erp", {}) if isinstance(services, dict) else {}
        if not isinstance(deployed, dict):
            raise ReleaseCompatibilityError(
                "The successful ERP release receipt is invalid; verify the deployment before running the operator"
            )
        revision = deployed.get("source_commit", "")
        image = deployed.get("image_digest", "")
        config_digest = deployed.get("config_digest", "")
        if not all(
            isinstance(value, str) for value in (revision, image, config_digest)
        ) or not (
            re.fullmatch(r"[a-f0-9]{40}", revision)
            and re.fullmatch(r"sha256:[a-f0-9]{64}", image)
            and re.fullmatch(r"sha256:[a-f0-9]{64}", config_digest)
        ):
            raise ReleaseCompatibilityError(
                "The successful ERP release receipt is incomplete; verify the deployment before running the operator"
            )
        if (
            erp["image"] != image
            or erp.get("labels", {}).get("com.carbon.release.config-digest")
            != config_digest
        ):
            raise ReleaseCompatibilityError(
                "Runtime ERP image or configuration differs from its successful receipt; verify the deployment before running the operator"
            )
    else:
        # Preserve legacy support only when no receipt exists. An invalid or
        # unfinished receipt must never be replaced by the old global marker.
        revision = (runtime / "revision").read_text().strip()
        if (
            not re.fullmatch(r"[a-f0-9]{40}", revision)
            or erp["image"] != "carbon/erp:" + revision
        ):
            raise ReleaseCompatibilityError(
                "Legacy ERP image does not match the deployed revision"
            )
    if ops["image"] != "carbon/ops:" + revision:
        raise ReleaseCompatibilityError(
            "Ops source must match the verified ERP source; prepare a reviewed Ops-only maintenance rebuild from that ERP revision before running the operator"
        )
    return revision
...
def prepare(directory, mode, runtime=RUNTIME, private_root=PRIVATE_ROOT):
    ...
    revision = deployed_erp_revision(runtime, stack)
    if not re.fullmatch(r"[a-f0-9]{40}", config.get("requiredRevision", "")):
        raise ValueError(
            "Runtime and manifest revisions must be full commit identities"
        )
    ...
    ops["environment"] = {
        **erp["environment"],
        **ops["environment"],
        "INVOICE_OPERATOR_REVISION": revision,
        "INVOICE_OPERATOR_MODE": mode,
        "INVOICE_OPERATOR_SCHEDULER_PAUSED": "true" if mode != "check" else "false",
    }
    ops["secrets"] = list(
        dict.fromkeys([*erp.get("secrets", []), *ops.get("secrets", [])])
    )
    ops["volumes"].append(
        {"type": "bind", "source": str(directory), "target": "/operator"}
    )
...
def pin_images(stack, base):
    ...
    ops = captured(
        [
            ...
            "--format",
            "{{.Id}}",
            stack["services"]["ops"]["image"],
        ]
    )
    if running != erp or not all(
        re.fullmatch(r"sha256:[a-f0-9]{64}", value) for value in (erp, ops)
    ):
...
def run(
    directory, mode="check", execute=False, runtime=RUNTIME, private_root=PRIVATE_ROOT
):
    stack = prepare(directory, mode, runtime, private_root)
    print(
        "Prepared maintained invoice operator; no database or model operation has run.",
        flush=True,
    )
    if not execute:
        return
    ...
    with (private_root / "operator.lock").open("a") as lock:
        ...
        scheduler = (
            private_read(scheduler_file)
            if scheduler_file.exists()
            else {"restoreRequired": False, "wasRunning": False}
        )
        ...
        with log.open("w") as output:
            ...
            if mode != "check":
                if not scheduler["restoreRequired"]:
                    running = captured(
                        [*base, "ps", "--status", "running", "--services", "inngest"]
                    )
                    scheduler = {
                        "restoreRequired": True,
                        "wasRunning": "inngest" in running.splitlines(),
                    }
            ...
            if mode != "check":
                ...
                pending = checkpoint_file.exists() and private_read(
                    checkpoint_file
                ).get("restoreRequired", True)
                if pending:
                    raise ValueError(
                        "Company settings need recovery; scheduler remains paused. Run recover and inspect private logs"
                    )
            ...
            if result.returncode:
                raise ValueError(
                    "Invoice operation failed; inspect its private log and checkpoint"
                )
...
def main():
    ...
    parser.add_argument(
        "--mode", choices=("check", "apply", "recover"), default="check"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Execute the selected mode; apply may admit explicitly planned model calls",
    )
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(
            "Invoice operator stopped. Inspect private inputs/logs and run recover after an interrupted apply; scheduler recovery may be required."
        )
