...
def read_config(path: Path) -> dict:
    if not path.is_file():
        raise ValueError(
            "Knowledge deployment is not configured. Copy "
            "contrib/deploying/knowledge/deploy.example.json to "
            "contrib/deploying/knowledge/.local/deploy.json and complete the "
            "one-time setup in contrib/deploying/knowledge/README.md."
        )
    return json.loads(path.read_text())


ORDER = ("knowledge-schema", "knowledge-parser", "knowledge-query", "knowledge-ingest", "knowledge-retention", "knowledge-web")
...
def prepare_plan(config: dict, source: str, foundation: dict, migrations: list[str]) -> dict:
    ...
    for name, spec in services.items():
        ...
        if name in release.DATABASE_UNITS:
            ...
            spec["migrations"] = {
                "minimum": migrations[0] if name == "knowledge-schema" else migrations[-1],
    ...
    libraries = {release.canonical_digest(json.loads(spec["environment"]["KNOWLEDGE_MANUAL_SOURCE_JSON"]))
                 for spec in services.values() if "KNOWLEDGE_MANUAL_SOURCE_JSON" in spec["environment"]}
    if len(libraries) > 1:
...
def release_units(candidate: dict, manifest: Path, *, project: str, region: str, adapter, read_ledger, migration_names: list[str]) -> None:
    ...
    for name in ORDER:
        ...
        if current["services"].get(name, {}).get("revision_digest") != release.revision_digest(spec):
            ...
            print(f"Knowledge: releasing {name}", flush=True)
            release.promote(selected, current, project=project, region=region, manifest=manifest, adapter=adapter, ledger=observation)
        if name == "knowledge-schema" and applied != migration_names:
            print("Knowledge: applying pending migrations", flush=True)
...
def build_images(candidate: dict, repository: str, state: Path, archive: Path, adapter) -> None:
    ...
    for name, spec in candidate["services"].items():
        unit = name.removeprefix("knowledge-")
        ...
        if receipt:
            ...
            print(f"Knowledge: building and publishing {name} (linux/amd64)", flush=True)
            ...
            adapter.call(["docker", "buildx", "build", "--platform", "linux/amd64", "--target", "runtime",
                          "--file", f"contrib/deploying/knowledge/Dockerfile.{unit}", "--tag", tag,
...
def live_ledger(config: dict, adapter) -> dict:
    ...
    exists = adapter.call([*args, "--command", "SELECT to_regclass('knowledge_migrations.ledger') IS NOT NULL"], capture=True).strip()
    if exists == "f":
        return {"schema_version": 1, "names": []}
    if exists != "t":
        raise ValueError("Could not observe the live knowledge migration ledger")
    sql = "SELECT json_build_object('schema_version', 1, 'names', coalesce(json_agg(name ORDER BY name), '[]'::json)) FROM knowledge_migrations.ledger"
...
def source_revision(config: dict, adapter) -> str:
    ...
    module_spec = importlib.util.spec_from_file_location("knowledge_verify_source", HERE.parent / "gcp-tailscale/verify_source.py")
    ...
    verify.require_verified(config["source_repo_url"], revision, required=(*verify.REQUIRED_CHECKS,
                            (".github/workflows/knowledge-check.yml", ("foundation", "runtime"))))
    return revision
...
def observe_units(config: dict, current: dict, adapter) -> None:
    ...
    for name in ORDER:
        ...
        if previous:
            ...
            if name != "knowledge-web" or release.observed_digest(name, document):
...
def orchestrate(config: dict, state: Path, *, apply: bool, adapter) -> None:
    print("Knowledge: checking source, configuration and live release state", flush=True)
    ...
    expected = f"/projects/{number}/locations/{config['region']}/services/knowledge-web"
    if not number.isdigit() or release.foundation_audiences(foundation)["knowledge-web"] != expected:
        raise ValueError("Terraform foundation does not match the selected project and region")
    migration_paths = adapter.call(["git", "ls-tree", "-r", "--name-only", source, "--", "packages/knowledge/migrations/"], capture=True).splitlines()
    migrations = sorted(Path(path).stem for path in migration_paths if path.endswith(".sql"))
    if not migrations:
        raise ValueError("No knowledge migrations found in this revision")
    ...
    if not apply:
        print("Knowledge deployment preflight passed. No images published or cloud resources changed.")
        return
    ...
    print("Knowledge deployment completed. Open the configured KNOWLEDGE_WEB_ORIGIN to run the pilot checks.")
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=Path(os.environ.get("KNOWLEDGE_DEPLOY_CONFIG", str(HERE / ".local/deploy.json"))))
    ...
    try:
        ...
        for command in ("git", "docker", "gcloud", "terraform", "psql", "gh", "curl"):
            if not shutil.which(command):
                raise ValueError(f"Required command is missing: {command}; see the knowledge deployment README")
        state = private_state(args.config)
        with (state / "deployment.lock").open("w") as lock:
            try:
                fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
            except BlockingIOError:
                raise ValueError("Another knowledge deployment is running for this private configuration") from None
            log = state / ("deploy-" + uuid4().hex + ".log")
            print(f"Knowledge: private log {log}", flush=True)
        ...
        print("Error: Knowledge deployment command failed; inspect the private log for details. Completed steps are retained for retry.", file=sys.stderr)
        raise SystemExit(1) from None
