...
def invoker_edges(config: Configuration) -> set[tuple[str, str]]:
    ...
    for name, block in config.resources("google_project_iam_member").items():
        ...
        if match is None:
            raise AssertionError(f"{name} run.invoker condition is not an exact knowledge resource name")
...
class ParserTests(unittest.TestCase):
    ...
    def test_invoker_extraction_rejects_an_unconditioned_or_missing_grant(self):
        ...
        partial = Fake('resource "google_project_iam_member" "x" {\n role = "roles/run.invoker"\n member = "serviceAccount:${google_service_account.runtime["web"].email}"\n condition {\n title = "t"\n expression = "resource.name == \'projects/${var.project_id}/locations/${var.region}/services/knowledge-query\'"\n }\n}')
        self.assertEqual(invoker_edges(partial), {("web", "services/knowledge-query")})
        self.assertNotEqual(invoker_edges(partial), FoundationTests.EXPECTED_INVOKER_EDGES)


class FoundationTests(unittest.TestCase):
    EXPECTED_INVOKER_EDGES = {
        ("web", "services/knowledge-query"),
        ("web", "services/knowledge-actions"),
        ("ingest", "services/knowledge-query"),
        ("ingest", "jobs/knowledge-parser"),
        ("maintenance", "jobs/knowledge-retention"),
    }
    ...
    def test_state_backend_is_a_partial_gcs_configuration(self):
        ...
        self.assertIn("prefix=knowledge/", readme)
    ...
    def test_iap_protects_the_real_web_service_entry_and_terraform_owns_only_its_shell(self):
        ...
        self.assertEqual(unquote(web.attrs["name"]), "knowledge-web")
        ...
        self.assertEqual(release.IAP_SERVICES, {"knowledge-web"})
    ...
    def test_audiences_are_outputs_derived_from_the_foundation(self):
        ...
        expected = {receiver for mapping in release.AUDIENCE_ENVIRONMENT.values() for receiver in mapping.values()} - {"knowledge-web"}
    ...
    def test_private_source_path_uses_the_declared_cidrs_over_peering_not_nat(self):
        ...
        self.assertEqual(set(peerings), {"knowledge_to_source", "source_to_knowledge"})
        self.assertEqual(peerings["knowledge_to_source"].attrs["peer_network"], "var.private_source_network")
        self.assertEqual(peerings["source_to_knowledge"].attrs["network"], "var.private_source_network")
    ...
    def test_secret_access_matrix_gives_database_identities_the_listener_ca_and_nothing_to_web_or_parser(self):
        ...
        for identity in ("query", "ingest", "migration", "maintenance"):
            with self.subTest(identity=identity):
                self.assertIn("knowledge-source-database-ca", access[identity])
                self.assertTrue(any(secret.endswith("-db-url") for secret in access[identity]))
        self.assertEqual(access["maintenance"], ["knowledge-maintenance-db-url", "knowledge-source-database-ca"])
        self.assertNotIn("knowledge-parser-key", self.text)
        self.assertNotIn("versions/latest", self.text)

    def test_private_storage_and_networking_have_no_public_data_path(self):
        bucket = self.config.resources("google_storage_bucket")["knowledge"]
        ...
        redis = self.config.resources("google_redis_instance")["knowledge"]
