...
SUBSTITUTION = re.compile(r"\$\{(KNOWLEDGE_LOCAL_[A-Z0-9_]+):-([^}]*)\}")
# `NAME=value` in the README's exports.
DOCUMENTED = re.compile(r"\b(KNOWLEDGE_[A-Z0-9_]+)=(\S+)")
PUBLISHED = re.compile(r'"127\.0\.0\.1:\$\{(KNOWLEDGE_LOCAL_[A-Z0-9_]+):-(\d+)\}')
PORT_SENTENCE = re.compile(r"The runner publishes ports ([^.]+)\.", re.S)


def substituted(path: Path) -> dict[str, str]:
    """Every `${KNOWLEDGE_LOCAL_*:-default}` the file reads, mapped to its default."""
...
class SecondStackDocumentationTests(unittest.TestCase):
    ...
    def test_the_stack_name_prefixes_the_images_the_build_tags(self):
        ...
        self.assertIn("stack=${KNOWLEDGE_LOCAL_STACK:-", build)
        self.assertIn("tag=${KNOWLEDGE_LOCAL_TAG:-", build)
        ...
        for image in images:
            with self.subTest(image=image):
                ...
                if "${" not in image:
                    continue
                self.assertTrue(
                    image.startswith("${KNOWLEDGE_LOCAL_STACK:-"),
