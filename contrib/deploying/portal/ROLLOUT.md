# Portal manual-v1 rollout

How the `manual-v1` portal workload is admitted to real use: what a pilot must
...
(`.fork/plans/2026-09-07-company-portal-platform.md` §2).

## 1. Gates and where each one runs

| Gate | Command | Runs locally | Evidence |
|---|---|---|---|
| Acceptance matrix A01–A15 | `pnpm --filter @carbon/portal evaluate -- suite=recall` and `-- suite=interactive` | yes; database-backed cases need the labelled disposable PostgreSQL (`PORTAL_TEST_DATABASE_URL` + `PORTAL_TEST_DATABASE_DISPOSABLE=1`) and are reported `SKIPPED-NEEDS-DISPOSABLE-DATABASE` without it | JSON report (`--output=`), per-case `PASS`/`FAIL`/`SKIPPED-*` |
| Performance fixture (500 questions, 10 users, 100k logical chunks) | `pnpm --filter @carbon/portal evaluate -- suite=performance` (or `suite=all`) | yes; requires the disposable database | p50/p95/p99, errors, Recall@10 per cold/warm path |
| Security attack fixture (19 attacks, 18 classes) | `pnpm --filter @carbon/portal verify:security` | yes; `S06`–`S09` and `S17` need the disposable database and `PORTAL_TEST_CONTAINER` | pass/fail per attack id |
| Deployment no-op and isolation | `pnpm --filter @carbon/portal verify:deployment` (self-test), `-- --local --dry-run` (Docker stack), `-- --expected <manifest> --observed <observed> [--previous <observed> --deploy <units>]` | yes | drift per unit, untouched/disturbed per neighbour, ledger head, database start time |
| Browser workflow, sign-in boundary, authorization | `contrib/deploying/portal/local-stack.sh test` (runs `manual-workflow`, `workforce-sso` and `authorization` specs) | yes; Docker Compose stack with real parser, PostgreSQL, Redis and Inngest | Playwright report and traces on failure |
...
3. The pilot source set is explicit: one `upload` source for one company, the
   ...
   `portal_metering."requestPolicy"` row. There is no default allowance;
...
|---|---|---|
| Manual search | remove `PORTAL_QUERY_URL`, `PORTAL_QUERY_AUDIENCE` or `PORTAL_COMPANY_ID` from the web unit | `apps/portal/app/routes/api.query.ts` → `503 query_not_configured`; `apps/portal/app/routes/health.ts` reports `not-configured` |
| Query service | remove any of `PORTAL_BUSINESS_TIMEZONE`, `PORTAL_ORIGIN`, `PORTAL_READ_DATABASE_URL`, `PORTAL_REDIS_URL`, `PORTAL_TRUSTED_CALLERS_JSON`, `PORTAL_MANUAL_SOURCE_JSON` | `apps/portal-query/src/index.ts` `isQueryReady` → `503 query_not_configured` on every route and `/health` |
| Intake, review, publish, removal, download | remove `PORTAL_WORKER_URL` or `PORTAL_WORKER_AUDIENCE` from the web unit | `apps/portal/app/modules/intake/intake.service.ts` throws "Portal intake is not configured" before any forward |
| Worker (ingestion) | remove any of the review/read/ingest database URLs, `PORTAL_OBJECT_BUCKET`, `PORTAL_TRUSTED_CALLERS_JSON`, `PORTAL_MACHINE_CALLERS_JSON`, `PORTAL_IDENTITY_URL`, `PORTAL_IDENTITY_AUDIENCE`, `PORTAL_AUTOMATION_USER_ID` | `apps/portal-worker/src/server.ts` `configuredWorkerDependencies` returns `null` → `503 worker_not_configured` |
| Background extraction | remove `INNGEST_SIGNING_KEY` or the `PORTAL_PARSER_*` values | `apps/portal-worker/src/index.ts` registers no Inngest functions and reports `not-configured` |
| Ticket commands | leave `PORTAL_ACTIONS_URL` / `PORTAL_ACTIONS_AUDIENCE` unset (the manual-v1 plan rejects them) | `apps/portal/app/routes/api.commands.ts` → `503 ticket_commands_not_configured`; `apps/portal-query/src/query.server.ts` → `403 manual_library_is_read_only` for command intents under `manualSourceId` |
| Command proposals and voice | leave query configuration unset for the web unit | `apps/portal/app/routes/api.propose-command.ts` → `503 command_proposal_not_configured`; `api.transcribe.ts` follows the same pattern |
...
| Drive connector | leave `PORTAL_DRIVE_TOKEN_BROKER_URL` / `_AUDIENCE` unset | `configuredWorkerDependencies` resolves every Drive token to `null` |
...
| Wrong release profile | `PORTAL_RELEASE_PROFILE` other than `manual-v1` | `packages/portal/src/release-profile.ts` throws on read |

### Per data scope (rows, no deploy)

| Scope | Switch | Effect |
|---|---|---|
| One user | `UPDATE portal."identityBinding" SET active=false` or deactivate the Carbon user / membership | every verified request fails `resolveHuman`; cache hits are re-authorized on delivery (`packages/portal/src/cache/cache.server.ts`) |
| One library | `UPDATE portal.source SET status='inactive'` | source excluded from retrieval, cache policy snapshot denies (`cache/epochs.server.ts`) |
| One grant | set `"revokedAt"` on the `portal."grant"` row | the source ACL epoch advances and warmed results stop (`authorization.spec.ts` proves this in the browser) |
...
| One company's traffic | delete or lower its `portal_metering."requestPolicy"` row | 429 with `retry-after`, durable without Redis (`scripts/test_request_limits.py`) |
...
The initial Portal namespace cutover is a schema boundary: an image from before
that rename cannot run against the renamed database. Follow the snapshot and
coordinated cutover procedure in [the deployment guide](README.md#portal-naming-and-existing-installations).
The revision-only procedure below applies within a compatible migration range.

Rolling back a portal unit changes only that Cloud Run revision; it never
...
3. Prove the rollback was isolated: observe the target before and after and run
   ...
   migration ledger head must be unchanged unless `portal-schema` was
   selected, and the database start time must be identical.
4. Schema is forward-only: the ledger (`portal_migrations.ledger`) refuses a
...
(`.fork/plans/2026-09-11-portal-authorization.md`, Task 11). Local passes do
...
- [ ] `verify:security` output: 19 attacks passed, `PORTAL_TEST_CONTAINER` named
