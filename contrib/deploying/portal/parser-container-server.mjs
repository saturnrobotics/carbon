...
const parserJob =
  process.env.PORTAL_PARSER_JOB_PATH ?? "/app/dist/parser-job.js";
...
function runParser(request) {
  return new Promise((resolve) => {
    const child = spawn(process.execPath, [parserJob], {
      env: {
        ...process.env,
        PORTAL_PARSER_INPUT_JSON: JSON.stringify(request.input),
        PORTAL_PARSER_OUTPUT_JSON: JSON.stringify(request.output)
      },
...
const server = createServer(async (incoming, outgoing) => {
  ...
  if (incoming.method === "GET" && pathname === "/health") {
    outgoing.writeHead(200).end('{"status":"ok","service":"portal-parser"}');
    return;
  }
  if (pathname !== "/parse") {
