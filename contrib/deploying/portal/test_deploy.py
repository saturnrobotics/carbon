...
class DeploymentEntryTests(unittest.TestCase):
    def test_missing_setup_explains_the_private_config_before_running_tools(self):
        ...
        self.assertIn("Portal deployment is not configured", result.stderr)
        self.assertIn("deploy.example.json", result.stderr)
        self.assertNotIn("Traceback", result.stderr)

    def test_make_exposes_apply_and_read_only_check(self):
        for target, flag in (("deploy-portal", "--apply"), ("deploy-portal-check", "--check")):
            with self.subTest(target=target):
                result = subprocess.run(["make", "-n", target], cwd=ROOT, text=True, capture_output=True)
                self.assertEqual(result.returncode, 0, result.stderr)
                self.assertIn("contrib/deploying/portal/deploy.py", result.stdout)
                self.assertIn(flag, result.stdout)


def module():
    sys.path.insert(0, str(HERE))
    spec = importlib.util.spec_from_file_location("portal_deploy", HERE / "deploy.py")
...
class OrchestrationTests(unittest.TestCase):
    ...
    def test_schema_is_observed_again_before_dependency_ordered_promotion(self):
        ...
        self.assertEqual(events, ["read-ledger", "portal-schema", "execute-schema", "read-ledger",
                                  "portal-parser", "portal-query", "portal-ingest", "portal-retention", "portal-web"])

    def test_successful_steps_survive_failure_and_retry_skips_them(self):
        self.assertTrue(callable(getattr(self.deploy, "release_units", None)), "Missing deployment orchestration")
        self.exercise(fail_unit="portal-ingest")
    ...
    def exercise(self, *, schema_failure=False, fail_unit=None, noop=False, stale_ledger=False):
        ...
        services = {"portal-web": plan()["services"]["portal-web"]}
        for name in release.DATABASE_UNITS:
            services.update(database_plan(name)["services"])
        services["portal-parser"] = dict(services["portal-web"], kind="job", environment={}, secrets={})
        ...
        with tempfile.TemporaryDirectory() as directory, patch.object(release, "promote", side_effect=promote):
            ...
            if schema_failure or fail_unit or stale_ledger:
                with self.assertRaises(ValueError):
                    invoke()
                self.assertNotIn("portal-web", events)
                if schema_failure or stale_ledger:
                    self.assertNotIn("portal-query", events)
                if fail_unit:
                    saved = json.loads(manifest.read_text())
                    self.assertIn("portal-query", saved["services"])
                    self.assertNotIn(fail_unit, saved["services"])
                    fail_unit = None
                    events.clear()
                    invoke()
                    self.assertEqual(events, ["read-ledger", "portal-ingest", "portal-retention", "portal-web"])
            else:
                invoke()
                if noop:
    ...
    def test_runtime_requires_current_schema_and_schema_can_bootstrap(self):
        ...
        config = {"services": {**database_plan()["services"], **database_plan("portal-schema")["services"]}}
        candidate = self.deploy.prepare_plan(config, "a" * 40, foundation_outputs(), [FIRST, LAST])
        self.assertEqual(candidate["services"]["portal-query"]["migrations"], {"minimum": LAST, "maximum": LAST})
        self.assertEqual(candidate["services"]["portal-schema"]["migrations"], {"minimum": FIRST, "maximum": LAST})
    ...
    def test_conflicting_manual_libraries_fail_before_build(self):
        ...
        environment = config["services"]["portal-query"]["environment"]
        environment["PORTAL_MANUAL_SOURCE_JSON"] = '{"sourceId":"different","displayName":"Different library"}'
        # Populate synthetic foundation audiences in the existing web fixture.
        for key, receiver in self.deploy.release.AUDIENCE_ENVIRONMENT["portal-web"].items():
            config["services"]["portal-web"]["environment"][key] = foundation_outputs()["service_audiences"]["value"][receiver]
...
class BuildAndPreflightTests(unittest.TestCase):
    ...
    def test_build_receipts_reuse_same_source_and_pin_registry_digest(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            candidate = plan()
            self.deploy.build_images(candidate, "us-east1-docker.pkg.dev/example/portal", state, archive, Adapter())
            self.assertEqual(candidate["services"]["portal-web"]["image"], "us-east1-docker.pkg.dev/example/portal/web@sha256:" + "b" * 64)
            self.assertEqual(candidate["build_receipt"]["portal-web"]["source_commit"], "a" * 40)
            ...
            self.deploy.build_images(plan(), "us-east1-docker.pkg.dev/example/portal", state, archive, Adapter())
