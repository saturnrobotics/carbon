...
DRIVE_VARIABLE = "PORTAL_DRIVE_ENABLED"
...
class ManualImageContracts(unittest.TestCase):
    def test_all_release_images_pin_the_manual_profile(self):
        for unit in UNITS:
            with self.subTest(unit=unit):
                ...
                self.assertIn("ENV PORTAL_RELEASE_PROFILE=manual-v1", dockerfile)

    def test_production_web_route_manifest_excludes_deferred_routes(self):
        routes = (ROOT / "apps/portal/app/routes.ts").read_text()
        for deferred in (
            "api.commands",
            "api.propose-command",
            "api.transcribe",
    ...
    def test_deferred_drive_surface_is_gated_and_no_release_path_enables_it(self):
        ...
        `PORTAL_DRIVE_ENABLED=true`. That variable reaches no release image
        and no deployed revision, so the gate is what keeps the fence real
        rather than the absence of a literal from one file."""
        manifest = (ROOT / "apps/portal/app/routes.ts").read_text()
        gated = (ROOT / "apps/portal/app/routes.deferred.ts").read_text()
    ...
    def test_parser_image_keeps_test_adapter_out_of_final_finite_job(self):
        ...
        adapter = (
            "COPY contrib/deploying/portal/parser-container-server.mjs "
...
class ParserContainerAdapter(unittest.TestCase):
    def setUp(self):
        ...
        self.job.write_text(
            ...
            "writeFileSync(process.env.PORTAL_TEST_RECORD, JSON.stringify({"
            "input: JSON.parse(process.env.PORTAL_PARSER_INPUT_JSON),"
            "output: JSON.parse(process.env.PORTAL_PARSER_OUTPUT_JSON)}));\n"
        )
        ...
        environment = {
            **os.environ,
            ...
            "PORT": str(self.port),
            "PORTAL_PARSER_JOB_PATH": str(self.job),
            "PORTAL_TEST_RECORD": str(self.record),
        }
