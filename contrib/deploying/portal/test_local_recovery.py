...
class RecoveryOwnershipTests(unittest.TestCase):
    project = "portal-owned-recovery-test"
    ...
    def source_info(self, service):
        return {
            ...
            "Mounts": [{"Type": "volume", "Destination": "/data",
                        "Name": f"{self.project}_portal_storage"}],
        }
    ...
    def execute_preflight(self, storage=None, volume_project=None):
        ...
        volume = {"Labels": {
            ...
            "com.docker.compose.volume": "portal_storage",
        }}
        with patch.dict(os.environ, {"PORTAL_LOCAL_STACK": self.project}), \
    ...
    def test_named_stack_uses_its_own_storage_volume(self):
        ...
        self.assertIn(["docker", "volume", "inspect",
                       f"{self.project}_portal_storage"], inspected)
        self.assertFalse(any("portal-manual-local_portal_storage" in call
    ...
    def test_foreign_or_bind_storage_mount_is_rejected_before_start(self):
        for mount in (
            {"Type": "volume", "Destination": "/data", "Name": "another-stack_portal_storage"},
