#!/usr/bin/env python3
"""Prove the portal images stay outside the enterprise license boundary.
...
portal image is a pruned workspace closure, so the static check walks the
...
def image_entries(reference: str) -> tuple[list[str], str]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="portal-license-") as temporary:
