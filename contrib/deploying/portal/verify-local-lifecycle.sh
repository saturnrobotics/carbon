...
temporary=$(mktemp -d "${TMPDIR:-/tmp}/portal-lifecycle.XXXXXX")
base_tag=portal-manual-local-web-e2e:lifecycle-base
update_tag=portal-manual-local-web-e2e:lifecycle-update
compose_tag=portal-manual-local-web-e2e:manual-v1
...
snapshot_ids() {
  ...
  for service in $services; do
    ...
    [ -n "$container_id" ] || {
      echo "Portal service is not running: $service" >&2
      exit 1
    }
...
assert_object_and_routes() {
  ...
  curl --fail --silent --show-error \
    "http://127.0.0.1:59912/storage/v1/b/portal-e2e/o/$encoded_key" \
...
PY
  curl --fail --silent --show-error \
    "http://127.0.0.1:59912/download/storage/v1/b/portal-e2e/o/$encoded_key?alt=media&generation=$object_generation" \
...
for container in "$postgres_container" "$storage_container"; do
  [ "$(docker inspect --format '{{index .Config.Labels "portal.disposable"}}' "$container")" = true ]
  [ "$(docker inspect --format '{{index .Config.Labels "portal.stack"}}' "$container")" = manual-local ]
done
...
$(docker exec -e PGPASSWORD=synthetic-test-only "$postgres_container" \
  psql -X -U supabase_admin -d portal_test -At -F '|' -c \
  "SELECT d.id,v.id,v.\"objectKey\",v.\"objectGeneration\",v.\"contentHash\",v.\"byteCount\",v.\"reviewedMetadata\"->>'partNumber' FROM portal.document d JOIN portal.\"documentVersion\" v ON v.id=d.\"currentVersionId\" AND v.\"companyId\"=d.\"companyId\" WHERE d.\"companyId\"='company-b' AND d.\"sourceId\"='source-b' AND d.status='published' AND d.\"deletedAt\" IS NULL ORDER BY d.\"createdAt\" DESC LIMIT 1")
EOF
...
FROM ${BASE_IMAGE}
ARG PROOF_NONCE
LABEL portal.lifecycle-proof=${PROOF_NONCE}
DOCKERFILE
