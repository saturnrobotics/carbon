...
BOOTSTRAP_DATABASE = "portal_bootstrap"
...
def selected_project():
    return os.environ.get("PORTAL_LOCAL_STACK") or "portal-manual-local"
...
def assert_disposable(info, expected_stack=None):
    ...
    if labels.get("portal.disposable") != "true":
        raise ValueError("recovery target must be labelled portal.disposable=true")
    if expected_stack and labels.get("portal.stack") != expected_stack:
...
def assert_source_storage_volume(info, project):
    ...
    if (
        labels.get("com.docker.compose.project") != project
        or labels.get("com.docker.compose.volume") != "portal_storage"
    ):
...
BEGIN;
SET LOCAL ROLE portal_read;
SET LOCAL portal.actor_id='{actor}';
SET LOCAL portal.company_id='{company}';
SELECT coalesce(jsonb_agg(id ORDER BY id),'[]') FROM portal.document;
ROLLBACK;
"""))
...
JOIN portal."documentVersion" v ON v.id=d."currentVersionId" AND v."companyId"=d."companyId"
...
FROM portal.document WHERE "deletedAt" IS NOT NULL;
""")),
        "objectReferences": int(db_sql(container, """
SELECT count(*) FROM portal."documentVersion"
...
SELECT jsonb_build_object(
  'roles',(
    ...
    FROM (
      ...
      FROM pg_roles WHERE left(rolname,10)='portal_'
    ) r
  ),
  'memberships',(
    ...
    FROM (
      ...
      WHERE left(granted.rolname,10)='portal_'
         OR left(member.rolname,10)='portal_'
    ) m
  ),
  'schemas',(
  ...
  'policies',(
    ...
    FROM pg_policies p WHERE p.schemaname IN ('portal','portal_metering')
  )
);
"""))
...
def object_inventory(origin):
    with urlopen(f"{origin}/storage/v1/b?project=portal-e2e", timeout=5) as response:
...
UPDATE portal.document SET "currentVersionId"={sql_literal(version)},version=version+1 WHERE id={sql_literal(document)} AND "companyId"='company-b';
...
INSERT INTO portal.document(id,"companyId","createdBy","sourceId","sourceItemId",title,"ownerId",kind,status,classification,"deletedAt")
...
UPDATE portal.document SET "currentVersionId"={sql_literal(tombstone_version)},version=version+1 WHERE id={sql_literal(tombstone)} AND "companyId"='company-b';
COMMIT;
""", capture=False)
...
def remove_linked_recovery_rows(container, documents):
    ...
    db_sql(container, f"""
BEGIN;
DELETE FROM portal.outbox WHERE "companyId"='company-b' AND "entityId" IN ({values});
DELETE FROM portal.chunk WHERE "companyId"='company-b' AND "documentId" IN ({values});
UPDATE portal.document SET "currentVersionId"=NULL,version=version+1 WHERE "companyId"='company-b' AND id IN ({values});
DELETE FROM portal."documentVersion" WHERE "companyId"='company-b' AND "documentId" IN ({values});
DELETE FROM portal.document WHERE "companyId"='company-b' AND id IN ({values});
COMMIT;
""", capture=False)
...
def main():
    ...
    storage_volume = f"{project}_portal_storage"
    ...
    target_database = f"portal-recovery-db-{suffix}"
    target_storage = f"portal-recovery-storage-{suffix}"
    target_network = f"portal-recovery-{suffix}"
    target_database_volume = f"portal-recovery-db-{suffix}"
    target_storage_volume = f"portal-recovery-storage-{suffix}"
    linked_documents = ()
    ...
    with tempfile.TemporaryDirectory() as backup_directory_name:
        ...
        database_archive = backup_directory / "portal.dump"
        try:
            ...
            run(["docker", "network", "create", "--label", "portal.disposable=true", target_network])
            for volume in (target_database_volume, target_storage_volume):
                run(["docker", "volume", "create", "--label", "portal.disposable=true", volume])
            run(["docker", "run", "--rm", "--platform", HELPER_PLATFORM,
            ...
            run(["docker", "run", "--detach", "--name", target_database,
                 "--label", "portal.disposable=true", "--network", target_network,
            ...
            run(["docker", "cp", str(database_archive), f"{target_database}:/tmp/portal.dump"])
            run(["docker", "exec", "-e", f"PGPASSWORD={PASSWORD}", target_database,
                 "pg_restore", "-U", "supabase_admin", "-d", DATABASE,
                 "--exit-on-error", "/tmp/portal.dump"])
