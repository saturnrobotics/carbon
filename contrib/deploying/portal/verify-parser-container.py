...
PARSER_IMAGE = os.environ.get(
    "PORTAL_PARSER_TEST_IMAGE", "portal-manual-local-parser:manual-v1"
)
PARSER_E2E_IMAGE = "portal-manual-local-parser-e2e:manual-v1"
...
class StorageApi:
    ...
    def create_bucket(self):
        return self.request(
            "POST", "/storage/v1/b?project=portal-test", json.dumps({"name": BUCKET}).encode()
        )
...
def parser_command(network, reference, output_key):
    environment = {
        ...
        "GOOGLE_CLOUD_PROJECT": "portal-test",
        "PORTAL_PARSER_INPUT_JSON": json.dumps(reference, separators=(",", ":")),
        "PORTAL_PARSER_OUTPUT_JSON": json.dumps(
...
def main():
    ...
    suffix = uuid4().hex[:10]
    network = f"portal-parser-{suffix}"
    storage = f"portal-parser-storage-{suffix}"
    storage_proxy = f"portal-parser-storage-proxy-{suffix}"
    ...
    run("docker", "network", "create", "--label", "portal.disposable=true", network)
    try:
        run(
            "docker", "run", "--detach", "--rm",
            "--name", storage,
            "--label", "portal.disposable=true",
            "--network", network,
