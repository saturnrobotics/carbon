#!/usr/bin/env python3
"""Prove a portal backup in isolated databases inside a disposable container."""
...
SOURCE_DATABASE = "portal_test"
PASSWORD = "synthetic-test-only"


def validate_container_info(info: dict[str, Any]) -> str:
    if (info.get("Config", {}).get("Labels") or {}).get("portal.disposable") != "true":
...
SELECT jsonb_build_object(
 'sources',(SELECT count(*) FROM portal.source),
 'bindings',(SELECT count(*) FROM portal."identityBinding"),
 'grants',(SELECT count(*) FROM portal."grant"),
 'objectReferences',(SELECT count(*) FROM portal."documentVersion" WHERE "objectKey"<>'' AND "objectGeneration"<>'')
)
"""))


def verify_policy_and_rebuild_state(database: str) -> None:
    hidden_exists = sql(database, "SELECT count(*) FROM portal.document WHERE id='doc-hidden'").splitlines()[-1]
    if hidden_exists != "1":
        raise AssertionError("Synthetic denied document was not restored")
    visible = json_line(sql(database, """
BEGIN;
SET LOCAL ROLE portal_read;
SET LOCAL portal.actor_id='alice';
...
SET ROLE portal_maintenance;
SELECT coalesce(jsonb_agg("documentId" ORDER BY "documentId"),'[]')
FROM portal.recovery_index_candidates(500)
"""))
...
def main() -> None:
    ...
    first_database = f"portal_recovery_stage_{suffix}"
    proof_database = f"portal_recovery_proof_{suffix}"
    first_archive = f"/tmp/portal-recovery-{suffix}.dump"
    proof_archive = f"/tmp/portal-recovery-proof-{suffix}.dump"
    try:
...
UPDATE portal.document SET "currentVersionId"='recovery-retired-version',version=version+1 WHERE id='recovery-retired';
