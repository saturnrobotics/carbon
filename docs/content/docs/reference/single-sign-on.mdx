...
SSO is an Enterprise Edition (EE) feature of your own [self-hosted](/docs/platform/self-hosting) deployment (`CARBON_EDITION=enterprise`, which requires a [commercial license](/docs/platform/licensing)). Carbon Cloud sign-in is unaffected. Enable it through [environment variables](/docs/platform/self-hosting/environment-variables):
