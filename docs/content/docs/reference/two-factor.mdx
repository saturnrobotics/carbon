...
<Screenshot label="Account → Security: the Add Authenticator App dialog with QR code, copyable secret, and six-box verify field" caption="Nothing is active until the first code checks out." />
...
Once verified, every sign-in asks for a code — magic link, Google, Outlook, and passkey alike. [Single sign-on](/docs/reference/single-sign-on) is the one exception: an SSO session arrives with your identity provider's MFA already enforced, so Carbon skips its own code screen.
