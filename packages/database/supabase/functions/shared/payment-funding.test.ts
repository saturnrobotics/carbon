...
Deno.test("two invoices at1.10 and1.20 funded by230 at1.15 reconcile to zero net FX", () => {
  ...
  assertEquals(
    result.applications.map((row) => row.appliedAmount),
    [100, 100]
  );
  assertEquals(
    result.applications.map((row) => row.fxGainLossAmount),
    [-4.34783, 4.34783]
  );
  assertEquals(
    round(
...
Deno.test("a prior110 credit releases its original base100 against an invoice carrying88", () => {
  ...
  assertEquals(
    first,
    second,
    "Changing the applying payment rate cannot revalue prior credit"
  );
...
Deno.test("160.01 source units exhaust exactly even when target base rounds to0.01000", () => {
  ...
  assertEquals(
    round(result.newOnAccountDocument + row.sourceAmount, 2),
    160.01
  );
});
...
Deno.test("funding rejects insufficient sources without mutating any source or request", () => {
  ...
  assertThrows(
    () => allocate(current, requests, { priorSources }),
    Error,
    "funding"
  );
...
Deno.test("funding refuses invalid rates, amounts, document precision, and unsafe integer units", () => {
  ...
  assertThrows(
    () => allocate(source("current", 1.001), []),
    Error,
    "precision"
  );
  assertThrows(
    () => allocate(source("current", 1), [request("a", 0.001)]),
    Error,
    "precision"
  );
  assertThrows(
...
Deno.test("settlement effectiveness requires the owning posted parent", () => {
  ...
  assertEquals(
    isEffectiveSettlement({ ...row, paymentStatus: "Posted" }),
    false
  );
...
Deno.test("invoice reducers preserve signed controls and aggregate before rounding", () => {
  for (const isAR of [true, false]) {
    ...
    assertEquals(invoiceRemainingAmounts(invoice, [row], controls, 2, isAR), {
      remainingDocument: 0,
      remainingBase: 0
    });
    assertThrows(
