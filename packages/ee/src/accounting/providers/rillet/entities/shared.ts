  ...
  async pushToAccounting(entityId: string): Promise<SyncResult> {
    ...
    try {
      ...
      if (err instanceof JournalEntrySyncError) {
        logger.error(
          `[${this.constructor.name}] structured push failure {error}`,
          {
            error: {
              entityId,
              ...err.failure
      ...
      logger.error(`[${this.constructor.name}] push failed {error}`, {
        error: {
          entityId,
          err
        }
...
> extends RilletEntitySyncer<TLocal, TRemote, TOmit> {
  ...
  async pushToAccounting(entityId: string): Promise<SyncResult> {
    ...
    try {
      ...
      if (err instanceof JournalEntrySyncError) {
        logger.error(`[${this.constructor.name}] pre-flight failure {error}`, {
          error: {
            entityId,
            ...err.failure
          }
