...
export async function installFixture(
  ...
  try {
    ...
    await client.query(
      'ANALYZE knowledge.document; ANALYZE knowledge."documentVersion"; ANALYZE knowledge."grant"; ANALYZE knowledge.chunk'
    );
...
export async function prepareReadPool(databaseUrl: string): Promise<pg.Pool> {
  ...
  try {
    await Promise.all(
      clients.map((client) => client.query("SET ROLE knowledge_read"))
    );
  } finally {
...
export async function queryOne(
  ...
  try {
    ...
    const rows =
      mode === "lexical"
        ? await withKnowledgeTransaction(pool, principal, "read", (client) =>
...
export async function explainBaseline(pool: pg.Pool, question: Question) {
  ...
  const statements: Record<Mode, { sql: string; values: unknown[] }> = {
    "exact-acl": {
      sql: `SELECT id FROM knowledge.document WHERE "companyId"=$1 AND id=$2 AND "sourceId"=$3`,
      values: [companyId, question.expectedDocumentId, sourceId]
    },
    lexical: {
      sql: "SELECT * FROM knowledge.search_lexical($1,$2::text[],$3,$4)",
      values: [companyId, [sourceId], question.term, 10]
    },
    vector: {
      sql: "SELECT * FROM knowledge.search_vector_exact($1,$2::text[],$3,$4,$5)",
      values: [
        companyId,
        [sourceId],
        vectorProfile,
  ...
  for (const mode of ["exact-acl", "lexical", "vector"] as const) {
    try {
      plans[mode] = await withKnowledgeTransaction(
        pool,
        principal,
        "read",
        async (client) => {
