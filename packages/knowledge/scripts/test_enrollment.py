...
ENROLL = "knowledge.enroll_workforce_identity"
UNBIND = "knowledge.unbind_workforce_identity"
...
def enroll(subject, company, user, capabilities="ARRAY['knowledge.read']"):
...
def as_migrator(*statements):
    ...
    return f"BEGIN; SET LOCAL ROLE knowledge_migrate; {body}; ROLLBACK"
...
class EnrollmentTests(unittest.TestCase):
    ...
    def test_owner_role_is_nologin_and_functions_expose_no_public_execute(self):
        self.assertEqual(
            sql(
                ...
                "FROM pg_roles WHERE rolname='knowledge_enrollment_owner'"
            ),
            "t",
        )
        for signature in (ENROLL_SIGNATURE, UNBIND_SIGNATURE):
            with self.subTest(function=signature):
                ...
                self.assertEqual(
                    sql(f"SELECT has_function_privilege('knowledge_migrate','{signature}','EXECUTE')"),
                    "t",
                )
        self.assertEqual(
            sql("SELECT pg_has_role('knowledge_migrate','knowledge_enrollment_owner','member')"),
            "f",
        )
    ...
    def test_runtime_write_policies_on_bindings_stay_closed(self):
        for role in ("knowledge_ingest", "knowledge_review", "knowledge_actions"):
            with self.subTest(role=role):
                sql(
                    f"BEGIN; SET LOCAL ROLE {role}; INSERT INTO knowledge.\"identityBinding\"(\"companyId\",\"createdBy\",issuer,subject,\"canonicalUserId\") "
                    "VALUES ('company-a','alice','x','accounts.google.com:1','alice'); ROLLBACK",
                    succeeds=False,
                    sqlstate="42501",
                )
        self.assertEqual(
            sql("SELECT count(*) FROM pg_policy WHERE polrelid='knowledge.\"identityBinding\"'::regclass AND polname IN ('INSERT','UPDATE','DELETE') "
    ...
    def test_migrate_role_enrolls_and_identical_rerun_is_a_noop(self):
        ...
        output = sql(as_migrator(
            ...
            enroll(subject, "company-a", "alice"),
            "RESET ROLE",
            f"SELECT count(*)::text FROM knowledge.\"identityBinding\" WHERE issuer='{ISSUER}' AND subject='{subject}'",
            f"SELECT public.knowledge_resolve_workforce_identity('{ISSUER}','{subject}','company-a')",
        ))
        ...
        self.assertEqual(first["capabilities"], ["knowledge.read"])
    ...
    def test_capability_change_updates_the_ceiling_in_place(self):
        ...
        first, second = json_lines(sql(as_migrator(
            enroll(subject, "company-a", "alice"),
            enroll(subject, "company-a", "alice", "ARRAY['source.entities.search','knowledge.read','knowledge.read']"),
        )))
        self.assertEqual(second["id"], first["id"])
        self.assertEqual(second["version"], 2)
        self.assertEqual(second["revocationVersion"], 1)
        self.assertEqual(second["capabilities"], ["knowledge.read", "source.entities.search"])
    ...
    def test_empty_or_malformed_capabilities_are_rejected(self):
        ...
        sql(as_migrator(enroll(subject, "company-a", "alice", "ARRAY['Knowledge.Read']")), succeeds=False, sqlstate="22023")
    ...
    def test_inactive_user_missing_membership_or_unknown_user_is_rejected(self):
        ...
        sql(
            f"BEGIN; UPDATE public.\"user\" SET active=false WHERE id='alice'; SET LOCAL ROLE knowledge_migrate; {enroll(subject, 'company-a', 'alice')}; ROLLBACK",
            succeeds=False,
            sqlstate="P0002",
        )
        sql(
            f"BEGIN; DELETE FROM public.\"userToCompany\" WHERE \"userId\"='alice' AND \"companyId\"='company-a'; SET LOCAL ROLE knowledge_migrate; {enroll(subject, 'company-a', 'alice')}; ROLLBACK",
            succeeds=False,
            sqlstate="P0002",
        )
        ...
        self.assertEqual(
            sql(f"SELECT count(*) FROM knowledge.\"identityBinding\" WHERE subject='{subject}'"),
            "0",
        )
