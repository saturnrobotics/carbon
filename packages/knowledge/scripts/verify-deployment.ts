 ...
 * Docker stack from `contrib/deploying/knowledge/compose.local.yaml`.
 *
...
export function observeLocalStack(): ObservedTarget | null {
  ...
  if (postgres) {
    ...
    const [head, count] = psql(
      "SELECT coalesce(max(name),''), count(*) FROM knowledge_migrations.ledger"
    ).split("|");
...
function synthetic(
  ...
  const base: Record<string, ObservedService> = {
    "knowledge-web": {
      ...
      deployed_revision: "knowledge-web-00001",
    ...
    "knowledge-query": {
      ...
      deployed_revision: "knowledge-query-00001",
...
export function selfTest() {
  const manifest: ReleaseManifest = {
    ...
    services: {
      ...
      "knowledge-schema": { revision_digest: "sha256:schema-1", image_digest: "sha256:img-schema-1", deployed_revision: "3" }
    }
  };
  ...
  const cases = {
    ...
    driftIsReported: (() => {
      const result = compareDeployment(manifest, synthetic({ "knowledge-query": { revision_digest: "sha256:hand-edited" } }));
      return !result.ok && result.services.find((row) => row.name === "knowledge-query")?.status === "drift";
    })(),
    missingIsReported: compareDeployment(manifest, synthetic({}, { services: { "knowledge-web": baseline.services["knowledge-web"]! } })).services.some((row) => row.status === "missing"),
    webOnlyIsIsolated: (() => {
      const result = compareIsolation(baseline, synthetic({ "knowledge-web": { revision_digest: "sha256:web-2", image_digest: "sha256:img-web-2", deployed_revision: "knowledge-web-00002", container_id: "c-web-2", started_at: "2026-09-11T00:00:00Z" } }), ["knowledge-web"]);
      return result.ok && result.services.map((row) => `${row.name}:${row.status}`).join(",") === "knowledge-query:untouched,knowledge-schema:untouched,knowledge-web:changed";
    })(),
    disturbedNeighbourFails: !compareIsolation(baseline, synthetic({ "knowledge-query": { container_id: "c-query-2", started_at: "2026-09-11T00:00:00Z" } }), ["knowledge-web"]).ok,
    schemaChangeWithoutSchemaDeployFails: !compareIsolation(baseline, synthetic({}, { migrations: { head: "20260912000000_new.sql", count: 34 } }), ["knowledge-web"]).ok,
    schemaDeployMayAdvanceLedger: compareIsolation(baseline, synthetic({ "knowledge-schema": { deployed_revision: "4" } }, { migrations: { head: "20260912000000_new.sql", count: 34 } }), ["knowledge-schema"]).ok,
    databaseRestartFails: !compareIsolation(baseline, synthetic({}, { database: { startedAt: "2026-09-11T00:00:00Z" } }), ["knowledge-web"]).ok
  };
...
export function main(): number {
  ...
  if (flags.has("--local")) {
    ...
    if (!observed) {
      ...
      report.note = "no running compose service; start it with contrib/deploying/knowledge/local-stack.sh up";
