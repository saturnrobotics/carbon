...
const workspaceDirectories: Record<string, string> = {
  knowledge: resolve(packageDirectory, "../../apps/knowledge"),
  "knowledge-worker": resolve(packageDirectory, "../../apps/knowledge-worker"),
  "knowledge-query": resolve(packageDirectory, "../../apps/knowledge-query")
};
const commandShapes = [
  {
    pattern:
  ...
  {
    pattern:
      /^corepack pnpm --filter (knowledge|knowledge-worker|knowledge-query) exec vitest run((?: (?:src|app)\/[A-Za-z0-9_./-]+\.test\.tsx?)+)$/,
