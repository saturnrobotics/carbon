...
if (!redisUrl)
  throw new Error("Revocation proof requires KNOWLEDGE_TEST_REDIS_URL");
...
function policyFor(pool: pg.Pool, principal: typeof alice) {
  return () =>
    withKnowledgeTransaction(pool, principal, "read", (client) =>
      ...
      async (client) =>
        (
          await client.query<{ id: string }>(
            `SELECT c.id FROM knowledge.chunk c WHERE c."companyId"=$1 AND c.id=ANY($2::text[])`,
...
async function sourceEpochs() {
  const result = await admin.query<{ contentEpoch: string; aclEpoch: string }>(
    `SELECT "contentEpoch","aclEpoch" FROM knowledge.source WHERE id='source-a' AND "companyId"='company-a'`
  );
...
describe("versioned cache revocation over real Redis and PostgreSQL", () => {
  beforeAll(async () => {
    const binding = await admin.query(
      `SELECT active FROM knowledge."identityBinding" WHERE id='id-alice' AND "companyId"='company-a'`
    );
  ...
  it("serves a warm Redis hit only until the actor's binding is revoked", async () => {
    ...
    await admin.query(
      `UPDATE knowledge."identityBinding" SET active=false,version=version+1 WHERE id='id-alice' AND "companyId"='company-a'`
    );
    try {
      ...
      await admin.query(
        `UPDATE knowledge."identityBinding" SET active=true,version=version+1 WHERE id='id-alice' AND "companyId"='company-a'`
      );
    }
    expect(await h.get()).toEqual(["chunk-doc-a"]);
  });
  ...
  it("a newly matching document invalidates a previously empty result set", async () => {
    ...
    try {
      ...
      await admin.query(
        `UPDATE knowledge.document SET "currentVersionId"='version-'||id,version=version+1 WHERE id=$1 AND "companyId"='company-a'`,
        [documentId]
      );
      await admin.query(
      ...
      await admin.query(
        `UPDATE knowledge.document SET "currentVersionId"=NULL,version=version+1 WHERE id=$1 AND "companyId"='company-a'`,
        [documentId]
      );
      await admin.query(
  ...
  it("applies reordered and duplicated outbox invalidations to the same end state", async () => {
    ...
    await withKnowledgeTransaction(ingestPool, indexer, "write", (client) =>
