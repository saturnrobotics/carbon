 ...
 * knowledge_enrollment_owner. Call them on the dedicated knowledge_migrate
...
export function normalizeCapabilities(
  ...
  if (
    ...
    throw new EnrollmentInputError(
      "capabilities must be dotted lowercase names such as knowledge.read"
    );
  return normalized;
}
...
export async function enrollWorkforceIdentity(
  ...
  const result = await db.query(
    "SELECT knowledge.enroll_workforce_identity($1::text,$2::text,$3::text,$4::text,$5::text[]) AS binding",
...
export async function unbindWorkforceIdentity(
  ...
  const companyId = requireIdentifier(input.companyId, "company id");
  const result = await db.query(
    "SELECT knowledge.unbind_workforce_identity($1::text,$2::text,$3::text) AS binding",
