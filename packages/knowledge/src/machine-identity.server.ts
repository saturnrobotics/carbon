...
/**
 * Machine (source indexer) admission, shared by every receiver that serves a
 * change feed: the knowledge worker's own ingress and Carbon's canonical
 * source-changes route verify the same registration shape with the same rules.
 *
 * A machine caller is a registered service-account subject with an explicit
 * company and source grant. It is never an employee: forwarded user evidence
 * on machine ingress is refused outright rather than ignored.
 */
export type MachineCapability = Extract<
  Principal,
  { kind: "machine" }
>["capabilities"][number];
export type MachinePrincipal = Extract<Principal, { kind: "machine" }>;
...
export async function verifyMachineRequest(options: {
  ...
  try {
    if (
      options.request.headers.has(PORTAL_USER_EVIDENCE_HEADER) ||
      options.request.headers.has(IAP_ASSERTION_HEADER)
    )
      throw unauthorized();
