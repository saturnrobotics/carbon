...
export type RecallMeasurement = {
  ...
  /** knowledge.search_vector_ann against knowledge.search_vector_exact. */
...
/** Identical to the ranking statement inside knowledge.search_vector_ann. */
const annStatement = `SELECT c.id FROM knowledge.chunk c
 ...
 * run through `SET LOCAL ROLE knowledge_read` inside savepoints on the same
...
export async function measureFilteredRecall(
  ...
  try {
    ...
    const queries = await client.query<{ embedding: string }>(
      ...
      [settings.queries, KNOWLEDGE_EMBEDDING_DIMENSIONS]
    );
    ...
    const allowed = await client.query<{ versionId: string }>(
      'SELECT "currentVersionId" AS "versionId" FROM knowledge.search_allowed_documents($1,ARRAY[$2]::text[])',
      [companyId, sourceId]
    );
    ...
    for (const embedding of embeddings) {
      ...
      try {
        ...
        const exact = await client.query<{ id: string }>(
          "SELECT id FROM knowledge.search_vector_exact($1,ARRAY[$2]::text[],$3,$4,$5)",
        ...
        const ann = await client.query<{
          ...
          'SELECT id,"retrievalPath" FROM knowledge.search_vector_ann($1,ARRAY[$2]::text[],$3,$4,$5)',
          [companyId, sourceId, profile, embedding, settings.k]
        );
      ...
      try {
        const exact = await client.query<{ id: string }>(
          `SELECT c.id FROM knowledge.chunk c
...
/** Transaction-local identity of the calibration reader (mirrors withKnowledgeTransaction). */
async function setReader(client: PoolClient) {
  await client.query(
    "SELECT set_config('knowledge.company_id',$1,true),set_config('knowledge.actor_id',$2,true),set_config('knowledge.caller_id','recall-calibration',true)",
    [companyId, readerId]
  );
}
...
export async function installRecallFixture(
  ...
  await client.query(
    `INSERT INTO knowledge."identityBinding"(id,"companyId","createdBy",issuer,subject,"canonicalUserId",active,capabilities)
     VALUES ('recall_binding',$1,$2,'https://identity.example.com','recall-subject',$2,true,ARRAY['knowledge.read']) ON CONFLICT DO NOTHING`,
    [companyId, readerId]
  );
  await client.query(
  ...
  await client.query(
    `UPDATE knowledge.document SET "currentVersionId"='recall_version_'||right(id,5),status='published',version=version+1
  ...
  await client.query(
    `INSERT INTO knowledge.chunk(id,"companyId","createdBy","documentId","documentVersionId",ordinal,text,"tokenCount",embedding,"embeddingProfile","indexGeneration")
     SELECT 'recall_chunk_'||to_char(value,'FM00000'),$1,$2,'recall_doc_'||to_char(value,'FM00000'),'recall_version_'||to_char(value,'FM00000'),0,
       ...
       $3,1
     FROM generate_series(0,$4::integer-1) value`,
    [companyId, readerId, profile, documents, KNOWLEDGE_EMBEDDING_DIMENSIONS]
  );
  await client.query(
    'ANALYZE knowledge.chunk; ANALYZE knowledge.document; ANALYZE knowledge."documentVersion"; ANALYZE knowledge."grant"'
  );
}
