...
import { verifyKnowledgeExtensions } from "../migrations.server";
...
describe("real authorized PostgreSQL retrieval", () => {
  ...
  it("records the installed pgvector version and re-verifies it without drift", async () => {
    ...
    try {
      ...
      const reports = await verifyKnowledgeExtensions(client);
  ...
  it("expands a selected chunk to its parent sections only where the reader may see them", async () => {
    ...
    try {
      ...
      await client.query(
        `INSERT INTO knowledge.chunk(id,"companyId","createdBy","documentId","documentVersionId",ordinal,text,heading,"parentOrdinal","tokenCount","embeddingProfile","indexGeneration")
      ...
      await client.query(
        "SELECT set_config('knowledge.company_id','company-a',true),set_config('knowledge.actor_id','alice',true),set_config('knowledge.caller_id','query',true)"
      );
  ...
  it("keeps filtered approximate recall at the exact authorized baseline under 10% ACL selectivity", async () => {
    ...
    try {
      ...
      const remaining = await client.query(
        "SELECT count(*)::int AS count FROM knowledge.chunk WHERE \"embeddingProfile\"='recall-calibration-768'"
      );
