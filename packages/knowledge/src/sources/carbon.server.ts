 ...
 * knowledge reads (parts, receipts, purchase status), so every row is one the
 * caller's own Carbon permission returned. The change feed is machine-only:
 * it leases Carbon's `knowledgeSourceOutbox` through the source-changes
 * route, never a database connection, and `persistCarbonChangePage` projects
 * what it observed into `knowledge.entity` and the knowledge outbox.
 */

export const CARBON_SOURCE_CHANGES_PATH = "/api/v1/knowledge/source-changes";
...
/** Carbon source table → the `knowledge.entity.entityType` its projection lands on. */
export const CARBON_KNOWLEDGE_ENTITY_TYPES: Record<
  CarbonEntityType,
...
export function createCarbonSourceAdapter(
  ...
  async function readItem(itemId: string) {
    ...
    return itemSchema
      ...
      .nullable()
      .parse(
        await transport.post("/api/v1/knowledge/getItemIdentity", { itemId })
      );
  }
  ...
  return {
    ...
    async searchEntities(
      ...
      try {
        const result = await transport.post("/api/v1/knowledge/resolveItems", {
          search: term,
          limit: search.limit
        });
        const rows = z
...
function knowledgeEntityTypeOf(type: string): string {
  return (
    (CARBON_KNOWLEDGE_ENTITY_TYPES as Record<string, string | undefined>)[
      type
    ] ?? type
  );
}
...
export function planCarbonChanges(
  ...
  for (const change of changes) {
    ...
    const target = {
      entityType: knowledgeEntityTypeOf(
 ...
 * epoch bumps. Tombstones and ACL changes also enqueue knowledge outbox
...
export async function persistCarbonChangePage(
  ...
  return withKnowledgeTransaction(pool, principal, "write", async (client) => {
    const source = await client.query(
      `SELECT 1 FROM knowledge.source WHERE "companyId"=$1 AND id=$2 AND kind='carbon' AND status='active'`,
    ...
    if (input.plan.upserts.length) {
      const result = await client.query(
        `INSERT INTO knowledge.entity ("companyId","createdBy","sourceId","sourceEntityId","entityType","sourceRevision","displayName","exactIdentifiers",metadata,"observedAt")
    ...
    if (input.plan.tombstones.length) {
      const result = await client.query<{ id: string; sourceVersion: string }>(
        `WITH gone AS (
           UPDATE knowledge.entity e SET "deletedAt"=now(),"observedAt"=t."observedAt"::timestamptz,"updatedBy"=$2,"updatedAt"=now(),version=e.version+1
    ...
    if (input.plan.aclChanges.length) {
      const result = await client.query(
        `INSERT INTO knowledge.outbox ("companyId","createdBy","sourceId","entityType","entityId","sourceVersion","eventType",payload)
         ...
         JOIN knowledge.entity e ON e."companyId"=$1 AND e."sourceId"=$3 AND e."entityType"=a."entityType" AND e."sourceEntityId"=a."sourceEntityId"
         ON CONFLICT ("companyId","sourceId","entityType","entityId","sourceVersion","eventType") DO NOTHING`,
        [
          principal.companyId,
          input.automationUserId,
...
export async function getCarbonSweepState(
  ...
  return withKnowledgeTransaction(pool, principal, "read", async (client) => {
    const result = await client.query<{ cursor: unknown }>(
      `SELECT cursor FROM knowledge.source WHERE "companyId"=$1 AND id=$2 AND kind='carbon' AND status='active'`,
...
export async function reconcileCarbonRange(
  ...
  return withKnowledgeTransaction(pool, principal, "write", async (client) => {
    const source = await client.query<{ cursor: unknown }>(
      `SELECT cursor FROM knowledge.source WHERE "companyId"=$1 AND id=$2 AND kind='carbon' AND status='active' FOR UPDATE`,
      [principal.companyId, input.sourceId]
    );
    ...
    const existing = await client.query<{
      ...
      `SELECT "sourceEntityId","sourceRevision","deletedAt"::text AS "deletedAt" FROM knowledge.entity
       WHERE "companyId"=$1 AND "sourceId"=$2 AND "entityType"=$3 AND "sourceEntityId"=ANY($4::text[])`,
      [principal.companyId, input.sourceId, input.knowledgeEntityType, ids]
    );
    ...
    const gone = await client.query<{ id: string }>(
      `WITH gone AS (
         UPDATE knowledge.entity e SET "deletedAt"=now(),"observedAt"=$8::timestamptz,"updatedBy"=$2,"updatedAt"=now(),version=e.version+1
    ...
    const advanced = await client.query(
      `UPDATE knowledge.source SET cursor=COALESCE(cursor,'{}'::jsonb)||jsonb_build_object('carbonSweep',$3::jsonb),"updatedBy"=$2,"updatedAt"=now(),version=version+1
