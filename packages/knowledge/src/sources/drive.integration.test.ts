...
const ingestPool = new pg.Pool({
  ...
  options: "-c role=knowledge_ingest",
  max: 1
});
...
async function publish(
  ...
  const document = await admin.query<{ id: string }>(
    `SELECT id FROM knowledge.document WHERE "companyId"=$1 AND "sourceId"=$2 AND "sourceItemId"=$3`,
...
async function visibleFiles(principal: typeof alice, sourceId: string) {
  return withKnowledgeTransaction(readPool, principal, "read", async (client) =>
    (
      await client.query<{ sourceItemId: string }>(
        `SELECT d."sourceItemId" FROM knowledge.chunk c JOIN knowledge.document d ON d.id=c."documentId" AND d."companyId"=c."companyId"
...
async function chunkIds(sourceId: string, fileIds: string[]) {
  const result = await admin.query<{ id: string }>(
    `SELECT c.id FROM knowledge.chunk c JOIN knowledge.document d ON d.id=c."documentId" AND d."companyId"=c."companyId"
  ...
  return withKnowledgeTransaction(readPool, principal, "read", async (client) =>
    (
      await client.query<{ id: string }>(
        `SELECT c.id FROM knowledge.chunk c WHERE c."companyId"=$1 AND c.id=ANY($2::text[]) ORDER BY c.id`,
...
async function outboxRows(sourceId: string) {
  const result = await admin.query<{
    ...
    `SELECT d."sourceItemId",o."eventType",o."sourceVersion",o."deliveredAt" FROM knowledge.outbox o
     JOIN knowledge.document d ON d.id=o."entityId" AND d."companyId"=o."companyId"
...
async function cleanup() {
  ...
  await admin.query(
    `DELETE FROM knowledge."identityBinding" WHERE "companyId"=$1 AND id='t19-carol-binding'`,
    [company]
  );
  await admin.query(
...
describe("Drive connector over real row policies", () => {
  beforeAll(async () => {
    ...
    await admin.query(
      `INSERT INTO knowledge."identityBinding" (id,"companyId","createdBy",issuer,subject,"canonicalUserId",active,capabilities)
       VALUES ('t19-carol-binding',$1,'alice','https://identity.example.com','subject-t19-carol','t19-carol',true,ARRAY['knowledge.read'])`,
      [company]
    );
    for (const [sourceId, driveId] of [
      ...
      await admin.query(
        `INSERT INTO knowledge."driveEnrollment" ("companyId","createdBy","sourceId",corpora,"driveId","rootFolderIds","oauthScope","credentialSecretRef","notificationChannelId","notificationTokenHash")
  ...
  it("persists a scoped listing once and converges on a repeated reconciliation without duplicates", async () => {
    ...
    const documents = await admin.query<{
      ...
      `SELECT "sourceItemId",status FROM knowledge.document WHERE "companyId"=$1 AND "sourceId"=$2 ORDER BY 1`,
      [company, sourceA]
    );
    ...
    const grants = await admin.query<{
      ...
      `SELECT d."sourceItemId",g."subjectId" FROM knowledge."grant" g JOIN knowledge.document d ON d.id=g."documentId" AND d."companyId"=g."companyId"
    ...
    const payload = await admin.query<{ payload: { driveFileId: string } }>(
      `SELECT o.payload FROM knowledge.outbox o JOIN knowledge.document d ON d.id=o."entityId" AND d."companyId"=o."companyId"
    ...
    const versions = await admin.query<{ version: string }>(
      `SELECT version FROM knowledge.document WHERE "companyId"=$1 AND "sourceId"=$2 AND "sourceItemId"='spec'`,
      [company, sourceA]
    );
  ...
  it("keeps the same PDF in two drives on separate ACLs and never reveals the hidden source", async () => {
    ...
    const byHash = await admin.query<{ count: string }>(
      `SELECT count(DISTINCT "documentId")::text AS count FROM knowledge."documentVersion" WHERE "companyId"=$1 AND "contentHash"=$2`,
      [company, sharedPdf]
    );
  ...
  it("invalidates every descendant, a warm answer and a candidate batch after a parent folder permission change", async () => {
    ...
    const cache = new AuthorizedCache(cacheStore, () =>
      withKnowledgeTransaction(readPool, alice, "read", (client) =>
    ...
    const before = await admin.query<{ aclEpoch: string }>(
      `SELECT "aclEpoch" FROM knowledge.source WHERE "companyId"=$1 AND id=$2`,
      [company, sourceA]
    );
  ...
  it("withdraws deleted, moved-out and inaccessible-shortcut items and re-arms a delivered outbox event", async () => {
    await admin.query(
      `UPDATE knowledge.outbox SET "deliveredAt"=now(),version=version+1 WHERE "companyId"=$1 AND "sourceId"=$2`,
      [company, sourceA]
    );
  ...
  it("changes nothing when a page fails and records the outcome without broadening access", async () => {
    const before = await admin.query<{ count: string }>(
      `SELECT count(*)::text AS count FROM knowledge."grant" WHERE "companyId"=$1 AND "sourceId"=$2 AND "revokedAt" IS NULL`,
      [company, sourceA]
    );
    await expect(
