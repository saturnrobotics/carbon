...
/**
 * Kanban as a knowledge source. Board membership is the Kanban API's own
...
export function createKanbanSourceAdapter(
  ...
  async function readTicket(ticketId: string) {
    ...
    return ticketEnvelopeSchema.parse(
      await transport.get(`/api/knowledge/tickets/${ticketId}`)
    );
  }
      ...
      if (input.kind === "board") {
        try {
          const catalog = catalogSchema.parse(
            await transport.get("/api/knowledge/catalog")
          );
