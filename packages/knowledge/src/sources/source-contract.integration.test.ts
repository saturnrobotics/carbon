...
/**
 * The Carbon projection against the real knowledge schema, through the real
 * `knowledge_ingest` role and row policies: replayed and reordered deliveries
...
async function epochs() {
  const result = await admin.query<{ contentEpoch: string; aclEpoch: string }>(
    `SELECT "contentEpoch","aclEpoch" FROM knowledge.source WHERE "companyId"=$1 AND id=$2`,
...
async function entity(sourceEntityId: string) {
  const result = await admin.query<{
    ...
    `SELECT "sourceRevision","deletedAt"::text AS "deletedAt","displayName","observedAt"::text AS "observedAt"
     FROM knowledge.entity WHERE "companyId"=$1 AND "sourceId"=$2 AND "entityType"='part' AND "sourceEntityId"=$3`,
...
afterAll(async () => {
  ...
  await admin.query(
    `DELETE FROM knowledge.entity WHERE "companyId"=$1 AND "sourceId"=$2`,
    [indexer.companyId, sourceId]
  );
  await admin.query(
...
describe("Carbon source contract against the knowledge schema", () => {
    ...
    expect(
      await withKnowledgeTransaction(
        ...
        async (client) =>
          (
            await client.query(
              `SELECT 1 FROM knowledge.entity WHERE "companyId"=$1 AND "sourceId"=$2 AND "sourceEntityId"='x1'`,
              [indexer.companyId, sourceId]
            )
          ).rows.length
      )
