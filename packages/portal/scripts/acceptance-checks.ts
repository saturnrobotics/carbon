...
async function removeFreshFixture(adminPool: pg.Pool) {
  ...
  await adminPool.query(
    `UPDATE portal.document SET "currentVersionId"=NULL,version=version+1 WHERE "companyId"='company-a' AND id=$1`,
    [freshFixture.documentId]
  );
  await adminPool.query(
...
export const checks: Record<string, Check> = {
  ...
  async "unauthorized-documents-database"(context) {
    ...
    const visible = await readAs(readPool, "alice", "company-a", async (client) =>
      (await client.query<{ id: string }>("SELECT id FROM portal.document ORDER BY id")).rows.map((row) => row.id)
    );
    const otherCompany = await readAs(readPool, "alice", "company-b", async (client) =>
      (await client.query<{ id: string }>("SELECT id FROM portal.document")).rows.length
    );
  ...
  async "deployment-noop-and-isolation"() {
    ...
    const webOnly = observedTarget({
      "portal-web": { revision_digest: "sha256:web-2", image_digest: "sha256:img-web-2", deployed_revision: "portal-web-00002" },
      "portal-query": { revision_digest: "sha256:query-1", image_digest: "sha256:img-query-1", deployed_revision: "portal-query-00001" }
    });
    const isolation = compareIsolation(observed, webOnly, ["portal-web"]);
    ...
    const restartedQuery = observedTarget({
      "portal-web": webOnly.services["portal-web"]!,
      "portal-query": { ...observed.services["portal-query"]!, deployed_revision: "portal-query-00002" }
    });
    const violation = compareIsolation(observed, restartedQuery, ["portal-web"]);
  ...
  async "local-auth-policy-overhead"(context) {
    ...
    for (let index = 0; index < iterations; index += 1) {
      const started = performance.now();
      const identity = await verifyWorkforceRequest({ request: workforceRequest(), operation: "portal.query", configuration, tokenVerifier: verifier(), identityStore: { resolveHuman: async () => binding }, nowEpochSeconds: now });
  ...
  async "no-cross-user-leakage"() {
    ...
    const isolated = Object.entries({ actorId: "usr_synthetic_blair", companyId: "cmp_beta", callerId: "worker", capability: "portal.intake.review", query: "MTR-100 " }).filter(([field, value]) => cacheKey({ ...cacheScope, [field]: value }, policyAllowed) !== key).map(([field]) => field);
  ...
  async "freshness-and-deletion-database"(context) {
    ...
    try {
      ...
      await adminPool.query(`UPDATE portal.document SET status='withdrawn',"deletedAt"=now(),"aclVersion"="aclVersion"+1,version=version+1 WHERE "companyId"='company-a' AND id=$1`, [freshFixture.documentId]);
      ...
      await adminPool.query(`UPDATE portal.document SET status='published',"deletedAt"=NULL,version=version+1 WHERE "companyId"='company-a' AND id=$1`, [freshFixture.documentId]);
      await adminPool.query(`UPDATE portal."grant" SET "revokedAt"=now(),version=version+1 WHERE "companyId"='company-a' AND "documentId"=$1`, [freshFixture.documentId]);
  ...
  async "intake-cannot-post-transactions-database"(context) {
    ...
    for (const role of ["portal_ingest", "portal_review"]) {
      outcomes[`${role}:mutator`] = await rolledBack(adminPool, role, "SELECT public.portal_fixture_mutator()");
  ...
  async "read-role-cannot-mutate-database"(context) {
    const { readPool, adminPool } = database(context);
    const readOnlyTransaction = await sqlState(readAs(readPool, "alice", "company-a", (client) => client.query("INSERT INTO portal.command DEFAULT VALUES")));
    const mutator = await rolledBack(adminPool, "portal_read", "SELECT public.portal_fixture_mutator()");
    const grant = await rolledBack(adminPool, "portal_read", `DELETE FROM portal."grant"`);
  ...
  async "connector-contract"() {
    const identity = { principal, companyGroupId: "group_alpha", allowedOperations: ["portal.query"], accessLevels: [], assurance: { mode: "carbon-mfa" as const } };
    ...
    const registry = createSourceRegistry({ version: 1, sources: [{ id: "carbon", kind: "carbon", origin: "https://carbon.example.test/", audience: "carbon" }, { id: "kanban", kind: "kanban", origin: "https://kanban.example.test/", audience: "kanban" }, { id: "engineering", kind: "engineering", origin: "https://engineering.example.test/", audience: "engineering" }, { id: "crm", kind: "crm", origin: "https://crm.example.test/", audience: "crm" }] }, { request: new Request("https://query.example.test/"), identity, headers: async () => new Headers(), fetch: async (input) => { const path = new URL(String(input)).pathname; if (path === "/api/portal/access/check") return Response.json({ allowedIds: ["expanded"], policyVersion: "1", validUntil: "2026-09-01T00:00:00Z" }); return Response.json(page); } });
