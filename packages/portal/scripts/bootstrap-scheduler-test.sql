...
-- portal migrator and restricted read/ingest roles used by the RLS proofs.
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname='portal_test_scheduler') THEN
    CREATE ROLE portal_test_scheduler LOGIN NOINHERIT NOSUPERUSER BYPASSRLS
