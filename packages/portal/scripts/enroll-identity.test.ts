...
const localUrl =
  "postgresql://schema:synthetic-test-only@127.0.0.1:59930/portal_test";
const remoteUrl = "postgresql://schema:secret@db.example.com:5432/portal";
  ...
  it("accepts space and equals forms and splits capabilities", () => {
    expect(
      parseArguments([
        ...
        "--iap-subject",
        subject,
        "--capabilities=portal.read,portal.intake.capture",
        "--allow-remote"
      ])
    ).toEqual({
      company: "company-b",
...
describe("isLocalDatabaseUrl", () => {
  it.each([
    "postgresql://u:p@127.0.0.1:59930/portal_test",
    "postgres://u:p@localhost:59930/portal_test",
    "postgresql://u:p@[::1]:59930/portal_test"
  ])("accepts %s", (value) => {
...
describe("runEnrollment", () => {
  it("binds the given user id locally, printing the subject only on the confirmation line", async () => {
    ...
    expect(sql).toContain("portal.enroll_workforce_identity(");
    expect(values).toEqual([IAP_ISSUER, subject, "company-b", "bob", ["portal.read"]]);
  ...
  it("refuses an email-shaped subject before connecting", async () => {
    ...
    const code = await runEnrollment(
      ["--company", "company-b", "--user-id", "bob", "--iap-subject", "bob@example.com", "--capabilities", "portal.read"],
      h.dependencies
    );
    expect(code).toBe(2);
