...
try {
  const ledger = await pool.query("SELECT count(*)::int AS count FROM portal_migrations.ledger");
  if (!ledger.rows[0]?.count) throw new Error("Apply portal migrations before generation");
  const { rows } = await pool.query<Column>(`SELECT table_name,column_name,udt_name,is_nullable,column_default,is_generated
    FROM information_schema.columns WHERE table_schema='portal' ORDER BY table_name,ordinal_position`);
  if (rows.length === 0) throw new Error("Portal schema is missing");
  const output = ["// Generated from migration-built local PostgreSQL by scripts/generate-types.ts. Do not edit.",
    "export type Json = string | number | boolean | null | { [key: string]: Json | undefined } | Json[];",
    "export type Database = { portal: { Tables: {"];
  ...
  console.log(`Generated ${new Set(rows.map((row) => row.table_name)).size} portal tables`);
} finally {
  await pool.end();
}
