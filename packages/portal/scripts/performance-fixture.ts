/**
 * Labelled synthetic performance fixture: 500 questions across 10 users over a
 * 100k-chunk corpus in the disposable local PostgreSQL. Shared by the
 * `performance` suite and the database-backed acceptance checks.
 */
import { readFile } from "node:fs/promises";
import pg from "pg";
import { withPortalTransaction } from "../src/database.server";
import {
  type PerformanceMeasurement,
  summarizeMeasurements
} from "../src/evaluation/performance";
...
export type Mode = "exact-acl" | "lexical" | "vector";

export const performanceCompanyId = "company-a";
export const performanceSourceId = "perf_source_100k";
export const vectorProfile = "perf-vector-768-v1";
export const logicalChunksPerDocument = 200;
export const expectedQuestionCount = 500;
export const expectedUserCount = 10;

export function vectorFor(documentIndex: number): number[] {
  return Array.from({ length: 768 }, (_, dimension) =>
    Number(dimension === documentIndex)
  );
}
...
export function parseQuestions(text: string): Question[] {
  ...
  if (
    ...
    new Set(rows.map((row) => row.userId)).size !== expectedUserCount
  )
    throw new Error(
      "performance fixture requires exactly 500 questions across 10 users"
    );
  for (const row of rows) {
    if (
      !/^perf-q-\d{3}$/.test(row.id) ||
      !/^perf_user_\d$/.test(row.userId) ||
      !/^perfword\d{3}$/.test(row.term) ||
      !/^perf_doc_\d{3}$/.test(row.expectedDocumentId)
    )
      throw new Error(
        "performance questions must use the labelled synthetic fixture"
      );
  }
  return rows;
}
...
export async function loadQuestions(path: string): Promise<Question[]> {
  return parseQuestions(await readFile(path, "utf8"));
}

/** The fixture administrator is the only login allowed to install fixture rows. */
export function administratorUrl(databaseUrl: string): string {
...
export async function installFixture(
  ...
  const companyId = performanceCompanyId;
  const sourceId = performanceSourceId;
  ...
  try {
    ...
    await client.query(
      `INSERT INTO portal."identityBinding"(id,"companyId","createdBy",issuer,subject,"canonicalUserId",active,capabilities)
       SELECT 'perf_binding_'||value,$1,'automation','https://identity.example.com','perf-subject-'||value,'perf_user_'||value,true,ARRAY['portal.read']
    ...
    await client.query(
      `UPDATE portal.document SET "currentVersionId"='perf_version_'||right(id,3),status='published',"updatedBy"='automation',"updatedAt"=now(),version=version+1
    ...
    const vectors = questions.map((question, index) => ({
      documentId: question.expectedDocumentId,
      embedding: `[${vectorFor(index).join(",")}]`
    }));
    await client.query(
    ...
    await client.query(
      'ANALYZE portal.document; ANALYZE portal."documentVersion"; ANALYZE portal."grant"; ANALYZE portal.chunk'
    );
...
export async function prepareReadPool(databaseUrl: string): Promise<pg.Pool> {
  const pool = new pg.Pool({
    connectionString: databaseUrl,
    max: expectedUserCount,
    connectionTimeoutMillis: 5_000
  });
  const clients = await Promise.all(
    Array.from({ length: expectedUserCount }, () => pool.connect())
  );
  try {
    await Promise.all(
      clients.map((client) => client.query("SET ROLE portal_read"))
    );
  } finally {
...
export async function queryOne(
  ...
  try {
    const principal = {
      ...
      callerId: "performance-evaluation"
    };
    const rows =
      mode === "lexical"
        ? await withPortalTransaction(pool, principal, "read", (client) =>
            lexicalSearch(client, companyId, [sourceId], question.term, 10)
          )
        : mode === "vector"
          ? await withPortalTransaction(pool, principal, "read", (client) =>
              vectorSearch(
                ...
                companyId,
                [sourceId],
                vectorFor(Number(question.id.slice(-3))),
                vectorProfile,
                10
              )
            )
          : await withPortalTransaction(
              ...
              async (client) =>
                (
                  await client.query<{ documentId: string }>(
                    `SELECT id AS "documentId" FROM portal.document WHERE "companyId"=$1 AND id=$2 AND "sourceId"=$3 AND status='published' AND "deletedAt" IS NULL`,
    ...
    return {
      ...
      ok: true,
      recalled: rows.some(
        (row) => row.documentId === question.expectedDocumentId
      )
    };
  } catch {
...
export async function runPhase(
  ...
  const workers = Array.from({ length: expectedUserCount }, (_, worker) =>
    questions.filter((_, index) => index % expectedUserCount === worker)
  );
  return (
    await Promise.all(
      workers.map(async (assigned) => {
        const measurements: PerformanceMeasurement[] = [];
        for (const question of assigned)
          measurements.push(await queryOne(pool, question, mode));
        return measurements;
      })
    )
  ).flat();
...
export async function explainBaseline(pool: pg.Pool, question: Question) {
  const companyId = performanceCompanyId;
  const sourceId = performanceSourceId;
  const principal = {
    companyId,
  ...
  const statements: Record<Mode, { sql: string; values: unknown[] }> = {
    "exact-acl": {
      sql: `SELECT id FROM portal.document WHERE "companyId"=$1 AND id=$2 AND "sourceId"=$3`,
      values: [companyId, question.expectedDocumentId, sourceId]
    },
    lexical: {
      sql: "SELECT * FROM portal.search_lexical($1,$2::text[],$3,$4)",
      values: [companyId, [sourceId], question.term, 10]
    },
    vector: {
      sql: "SELECT * FROM portal.search_vector_exact($1,$2::text[],$3,$4,$5)",
      values: [
        companyId,
        [sourceId],
        vectorProfile,
  ...
  for (const mode of ["exact-acl", "lexical", "vector"] as const) {
    try {
      plans[mode] = await withPortalTransaction(
        pool,
        principal,
        "read",
        async (client) => {
          const result = await client.query(
            `EXPLAIN (ANALYZE,BUFFERS,FORMAT JSON) ${statements[mode].sql}`,
            statements[mode].values
          );
...
export type PerformanceReport = {
  ...
  phases: Record<string, ReturnType<typeof summarizeMeasurements>>;
};

/** Installs the fixture through the administrator login, then measures as the read role. */
export async function runPerformanceSuite(
  ...
  try {
    ...
    for (const temperature of ["cold", "warm"] as const) {
      for (const mode of ["exact-acl", "lexical", "vector"] as const) {
        phases[`${temperature}:${mode}`] = summarizeMeasurements(
          await runPhase(pool, questions, mode)
        );
      }
    }
    return {
      schemaVersion: 1,
      scope:
        "local disposable PostgreSQL retrieval and RLS only; excludes IAP, HTTP, model synthesis, and true OS cache eviction",
      fixture: {
        questions: questions.length,
        concurrentUsers: expectedUserCount,
        documents: questions.length,
        logicalChunks: questions.length * logicalChunksPerDocument,
        chunkRowsIncludingLexicalAndVectorProfiles:
          questions.length * logicalChunksPerDocument * 2,
        aclSelectivityPerUser: 0.1
      },
      coldDefinition:
        "first measured pass in a new evaluator process after fixture verification and ANALYZE; operating-system page cache is not evicted",
