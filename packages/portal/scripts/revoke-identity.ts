 ...
 *
 * Each binding goes through portal.unbind_workforce_identity, which sets it
 ...
 * PORTAL_MIGRATION_DATABASE_URL (the portal_migrate schema login);
 * non-local URLs need --allow-remote and an interactive confirmation. Listing a
 * user's bindings requires SET ROLE portal_migrate, which the schema login
...
async function listUserBindings(
  ...
  try {
    await db.query("SET ROLE portal_migrate");
  } catch {
    throw new Error(
      "listing a user's bindings needs a connection that can SET ROLE portal_migrate; pass --iap-subject with --company instead"
    );
  }
  try {
    const result = await db.query(
      'SELECT issuer, subject, "companyId" FROM portal."identityBinding" WHERE "canonicalUserId"=$1::text AND ($2::text IS NULL OR "companyId"=$2::text) ORDER BY "companyId", issuer, subject',
...
export async function runRevocation(
  ...
  if (!databaseUrl) {
    dependencies.stderr(
      `${DATABASE_URL_VARIABLE} is required: the portal_migrate schema login`
    );
    return 2;
  }
