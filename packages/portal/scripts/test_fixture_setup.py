...
class FixtureSetupTests(unittest.TestCase):
    def test_repeated_setup_preserves_source_tables_and_separate_worker_role(self):
        ...
        tables = (
            "SELECT 'public.\"portalCommandReceipt\"'::regclass::oid, "
            "'public.\"portalProcurementSchedule\"'::regclass::oid"
        )
        ...
        self.assertEqual(
            sql(
                ...
                "FROM pg_roles WHERE rolname='portal_test_scheduler'"
            ),
            "t",
        )
        self.assertEqual(
            sql(
                ...
                "WHERE rolname='portal_test_migrator'"
            ),
            "f",
        )
        self.assertEqual(
            sql(
                "SELECT has_schema_privilege('portal_test_scheduler','portal','USAGE')"
            ),
            "f",
        )
        self.assertEqual(
            sql(
                "SELECT has_table_privilege('portal_test_scheduler',"
                "'public.\"portalProcurementSchedule\"','UPDATE')"
            ),
            "t",
        )
