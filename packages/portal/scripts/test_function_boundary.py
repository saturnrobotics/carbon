...
class FunctionBoundaryTests(unittest.TestCase):
 def test_query_role_cannot_execute_public_business_mutators(self):
  sql("BEGIN; SET LOCAL ROLE portal_read; SELECT public.portal_fixture_mutator(); ROLLBACK",succeeds=False)
 def test_write_roles_can_generate_ids_but_not_call_business_mutators(self):
  for role in ['portal_ingest','portal_review','portal_actions']:
   with self.subTest(role=role):
    ...
    sql("BEGIN; SET LOCAL ROLE " + role + "; SELECT public.portal_fixture_mutator(); ROLLBACK",succeeds=False)
 def test_existing_authenticated_execution_is_preserved(self):
  result=sql("BEGIN; SET LOCAL ROLE authenticated; SELECT public.portal_fixture_mutator(); RESET ROLE; SELECT active FROM public.\"user\" WHERE id='alice'; ROLLBACK")
  self.assertEqual(result.splitlines()[-2],'f')
 def test_query_role_still_executes_only_the_explicit_identity_rpc(self):
  result=sql("SET ROLE portal_read; SELECT public.portal_resolve_workforce_identity('https://identity.example.com','subject-a','company-a')->>'actorId'")
