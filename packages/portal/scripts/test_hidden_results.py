...
class HiddenResultTests(unittest.TestCase):
    ...
    def test_matching_hidden_manual_changes_no_counts_or_snippets(self):
        search = "SELECT id FROM portal.search_lexical('company-a',ARRAY['source-a'],'{term}',10) ORDER BY id"
        ...
        self.assertEqual(as_reader("alice", "company-a", "SELECT count(*) FROM portal.chunk WHERE \"documentId\" IN ('doc-a','doc-hidden','doc-b')"), ["1"])
        self.assertEqual(
            as_reader("alice", "company-a", "SELECT count(*) FROM portal.document WHERE id IN ('doc-a','doc-hidden','doc-b') AND title ILIKE '%manual%'"),
            ["1"],
        )
        self.assertEqual(
            as_reader("alice", "company-a", "SELECT count(*) FROM portal.chunk WHERE \"documentId\" IN ('doc-a','doc-hidden','doc-b') AND text ILIKE '%hidden%'"),
            ["0"],
        )

    def test_citation_to_an_inaccessible_version_resolves_nothing(self):
        version = "SELECT id FROM portal.\"documentVersion\" WHERE id='{id}'"
        ...
        self.assertEqual(
            as_reader("alice", "company-a", "SELECT portal.can_access('company-a','source-a','doc-hidden',NULL,'read')"),
            ["f"],
        )
        self.assertEqual(
            as_reader("alice", "company-a", "SELECT portal.can_access('company-a','source-a','doc-a',NULL,'read')"),
            ["t"],
        )
