...
affected portal."identityBinding" (active=false, revocationVersion+1) so a
...
TRIGGER_FUNCTION = "public.portal_propagate_identity_revocation()"
TRIGGERS = (
    ("public.\"user\"", "portal_identity_revocation_on_user_deactivation"),
    ("public.\"userToCompany\"", "portal_identity_revocation_on_membership_removal"),
)
RUNTIME_ROLES = (
    "portal_read",
    "portal_ingest",
...
READER_SNAPSHOT = (
    ...
    "FROM portal.\"identityBinding\" "
    "WHERE \"companyId\"=portal.company_id() AND \"canonicalUserId\"=portal.actor_id() AND active"
)
...
def enroll(subject, company, user):
    ...
    return (
        f"SELECT portal.enroll_workforce_identity('{ISSUER}','{subject}','{company}','{user}',"
        "ARRAY['portal.read']::text[])"
    )


def resolve(subject, company):
    return f"SELECT public.portal_resolve_workforce_identity('{ISSUER}','{subject}','{company}')"


def bindings_of(user):
    return f"SELECT to_jsonb(b) FROM portal.\"identityBinding\" b WHERE \"canonicalUserId\"='{user}' ORDER BY id"


def as_reader(actor, company):
    return (
        f"SET LOCAL ROLE portal_read; SET LOCAL portal.actor_id='{actor}'; "
        f"SET LOCAL portal.company_id='{company}'; {READER_SNAPSHOT}; RESET ROLE"
    )


def as_migrator(*statements):
    return "SET LOCAL ROLE portal_migrate; " + "; ".join(statements) + "; RESET ROLE"
...
class RevocationTests(unittest.TestCase):
    ...
    def test_trigger_function_is_owner_definer_with_no_grantee(self):
        ...
        self.assertEqual(
            sql("SELECT count(*) FROM pg_auth_members WHERE roleid='portal_enrollment_owner'::regrole"),
            "0",
        )
        self.assertEqual(
            sql("SELECT has_schema_privilege('portal_enrollment_owner','public','CREATE')"),
            "f",
        )
    ...
    def test_deactivation_by_authenticated_role_fires_without_an_execute_grant(self):
        output = transaction(
            ...
            "SELECT public.portal_fixture_mutator()",
            "RESET ROLE",
            bindings_of("alice"),
        )
    ...
    def test_permission_edit_changes_permissions_version_but_not_revocation(self):
        ...
        output = transaction(
            ...
            "INSERT INTO public.\"userPermission\"(id,permissions) VALUES ('alice','{\"portal_view\":[\"company-a\"]}') "
    ...
    def test_unbind_denies_the_reader_snapshot_like_the_triggers_do(self):
        ...
        output = transaction(
            ...
            as_migrator(f"SELECT portal.unbind_workforce_identity('{ISSUER}','{subject}','company-a')"),
            as_reader("revoked", "company-a"),
        )
