...
export const PORTAL_CONTRACT_VERSION = "portal.v1" as const;
...
const opaqueIdSchema = z
  .string()
  .min(1)
  .max(PORTAL_LIMITS.idCharacters)
  .regex(/^\S+$/, "IDs cannot contain whitespace");
const boundedIdentifierSchema = z
  .string()
  .trim()
  .min(1)
  .max(PORTAL_LIMITS.identifierCharacters);
const titleSchema = z.string().trim().min(1).max(PORTAL_LIMITS.titleCharacters);
...
export const jsonValueSchema: z.ZodType<JsonValue> = z.lazy(() =>
  z.union([
    ...
    z.boolean(),
    z.number().finite(),
    z.string().max(PORTAL_LIMITS.descriptionCharacters),
    z.array(jsonValueSchema).max(PORTAL_LIMITS.pageItems),
    z
      .record(z.string().min(1).max(200), jsonValueSchema)
      .refine(
        (value) => Object.keys(value).length <= PORTAL_LIMITS.pageItems,
        `JSON objects may contain at most ${PORTAL_LIMITS.pageItems} keys`
      )
  ])
);
...
const boundedJsonObjectSchema = z
  ...
  .refine(
    (value) =>
      ...
      PORTAL_LIMITS.jsonBytes,
    `JSON payload exceeds ${PORTAL_LIMITS.jsonBytes} bytes`
  );
...
const sourceIdentitySchema = z
  .object({
    issuer: z.string().url().max(PORTAL_LIMITS.uriCharacters),
 ...
 * the portal services verified themselves: an IAP assertion proves
...
const humanPrincipalSchema = z
  .object({
    ...
    capabilities: z
      ...
      .max(PORTAL_LIMITS.capabilities),
...
const machinePrincipalSchema = z
  .object({
    ...
    callerId: boundedIdentifierSchema,
    companyId: opaqueIdSchema,
    sourceIds: z.array(opaqueIdSchema).min(1).max(PORTAL_LIMITS.sources),
    policyVersion: boundedIdentifierSchema,
    capabilities: z
...
export const queryRequestSchema = z
  .object({
    ...
    text: z.string().trim().min(1).max(PORTAL_LIMITS.queryCharacters),
...
export const sourceEntitySchema = z
  .object({
    ...
    title: titleSchema,
    description: z.string().max(PORTAL_LIMITS.descriptionCharacters).nullable(),
    fields: z
...
export const itemSearchRequestSchema = z
  .object({
    search: z.string().trim().min(1).max(PORTAL_LIMITS.idCharacters),
...
export const itemCandidateSchema = z
  .object({
    id: opaqueIdSchema,
    readableId: z.string().max(PORTAL_LIMITS.idCharacters),
    name: z.string().max(PORTAL_LIMITS.titleCharacters),
    revision: z.string().max(PORTAL_LIMITS.idCharacters).nullable(),
    mpn: z.string().max(PORTAL_LIMITS.idCharacters).nullable(),
    sourceId: opaqueIdSchema
  })
  .strict();
    ...
    sources: z
      .array(
        z
          .object({
            sourceId: opaqueIdSchema,
            displayName: z.string().max(PORTAL_LIMITS.titleCharacters),
            classification: z.string().max(PORTAL_LIMITS.idCharacters)
          })
          .strict()
      )
      .max(20)
...
export const evidenceSchema = z
  .object({
    ...
    sourceRevision: boundedIdentifierSchema,
    title: titleSchema,
    excerpt: z.string().max(PORTAL_LIMITS.excerptCharacters).optional(),
    page: z.number().int().positive().optional(),
    section: z.string().trim().min(1).max(500).optional(),
    sourceUri: z.string().url().max(PORTAL_LIMITS.uriCharacters),
    observedAt: timestampSchema,
...
export function createSourcePageSchema<T extends z.ZodType>(itemSchema: T) {
  return z
    .object({
      items: z.array(itemSchema).max(PORTAL_LIMITS.pageItems),
...
export const intakeProposalSchema = z
  .object({
    ...
    provenance: z
      ...
      .refine(
        (value) => Object.keys(value).length <= PORTAL_LIMITS.pageItems,
        `Provenance may contain at most ${PORTAL_LIMITS.pageItems} fields`
      ),
    unresolved: z
      .array(unresolvedFieldSchema)
      .max(PORTAL_LIMITS.unresolvedFields),
    reviewDecisions: z
      .array(reviewDecisionSchema)
      .max(PORTAL_LIMITS.reviewDecisions)
  })
  .strict();
...
const clarificationSchema = z
  .object({
    ...
    choices: z
      ...
      .min(1)
      .max(PORTAL_LIMITS.clarificationChoices)
  })
  .strict();
...
export const ticketCommandPayloadSchema = z
  .object({
    ...
    title: z.string().trim().min(1).max(PORTAL_LIMITS.ticketTitleCharacters),
    description: z.string().max(PORTAL_LIMITS.descriptionCharacters),
...
export const portalJsonSchemas = {
...
export const portalOpenApiDocument = {
  openapi: "3.1.0",
  info: {
    title: "Carbon Portal Contracts",
