  ...
  return withPortalTransaction(pool, principal, "read", async (client) => {
    const result = await client.query<AuthorizedDocumentVersion>(
      `SELECT d.id AS "documentId",v.id AS "documentVersionId",d."sourceId",s.kind AS "sourceKind",d."sourceItemId",d.title,
       ...
       FROM portal.document d JOIN portal."documentVersion" v
         ON v."companyId"=d."companyId" AND v."documentId"=d.id
       JOIN portal.source s ON s."companyId"=d."companyId" AND s.id=d."sourceId"
...
export async function tombstoneManualDocument(
  ...
  return withPortalTransaction(pool, principal, "write", async (client) => {
    ...
    const replay = await client.query<{ targetRefs: unknown }>(
      `SELECT "targetRefs" FROM portal.audit WHERE "companyId"=$1 AND "actorId"=$2
       AND "callerId"=$3 AND "requestId"=$4 AND action='portal.document.delete' LIMIT 1`,
      [
        principal.companyId,
        principal.actorId,
        principal.callerId,
    ...
    const target = await client.query<{ sourceRevision: string }>(
      `SELECT v."sourceRevision" FROM portal.document d
       JOIN portal."documentVersion" v ON v."companyId"=d."companyId" AND v.id=d."currentVersionId"
       WHERE d."companyId"=$1 AND d.id=$2 AND d."sourceId"=$3 AND d.kind='manual'
         AND d."deletedAt" IS NULL AND d.status='published'
         AND portal.can_access(d."companyId",d."sourceId",d.id,NULL,'publish')
       FOR UPDATE OF d`,
      [principal.companyId, input.documentId, input.sourceId]
    );
    if (!target.rows[0]) throw new Error("manual cannot be deleted");
    const updated = await client.query(
      `UPDATE portal.document d SET status='withdrawn',"deletedAt"=now(),"aclVersion"=d."aclVersion"+1,
    ...
    await client.query(
      `INSERT INTO portal.audit("companyId","createdBy","actorId","callerId","requestId",action,"targetRefs",decision,"policyVersion",metadata)
       VALUES ($1,$2,$2,$3,$4,'portal.document.delete',jsonb_build_array(jsonb_build_object('documentId',$5::text)),'allow','manual-v1','{}')`,
      [
        principal.companyId,
        principal.actorId,
        principal.callerId,
