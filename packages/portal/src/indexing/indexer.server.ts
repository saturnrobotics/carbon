...
import type { PortalChunk } from "./chunks";

export type EmbeddedChunk = PortalChunk & {
...
export async function indexDocumentVersionInTransaction(
  ...
  const result = await client.query(
    `INSERT INTO portal.chunk ("companyId","createdBy","documentId","documentVersionId",ordinal,text,heading,page,bounds,"parentOrdinal","tokenCount",embedding,"embeddingProfile","indexGeneration")
...
export async function embedCurrentDocumentVersion(
  ...
  const pending = await withPortalTransaction(
    pool,
    principal,
    "read",
    async (client) => {
      const result = await client.query<{
       ...
       FROM portal.document d JOIN portal.source s ON s."companyId"=d."companyId" AND s.id=d."sourceId"
       JOIN portal.chunk c ON c."companyId"=d."companyId" AND c."documentId"=d.id AND c."documentVersionId"=d."currentVersionId" AND c."embeddingProfile"='lexical-v1'
       WHERE d."companyId"=$1 AND d.id=$2 AND d.status='published' AND d."deletedAt" IS NULL
        AND NOT EXISTS (SELECT 1 FROM portal.chunk embedded WHERE embedded."companyId"=c."companyId" AND embedded."documentVersionId"=c."documentVersionId" AND embedded.ordinal=c.ordinal AND embedded."embeddingProfile"=$3)
