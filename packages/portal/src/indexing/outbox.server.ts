  ...
  return withPortalTransaction(pool, principal, "write", async (client) => {
    ...
    const result = await client.query<LeasedOutboxEvent & { ordinal: string }>(
      `WITH locked AS (
        ...
        FROM portal.outbox WHERE "companyId"=$1 AND "deliveredAt" IS NULL AND "availableAt"<=now()
...
export async function acknowledgeOutbox(
  ...
  await withPortalTransaction(pool, principal, "write", async (client) => {
    const result = await client.query(
      `UPDATE portal.outbox SET "deliveredAt"=now(),"leaseOwner"=NULL,"leaseUntil"=NULL,version=version+1 WHERE "companyId"=$1 AND "leaseOwner"=$2 AND id = ANY($3::text[])`,
  ...
  return withPortalTransaction(pool, principal, "read", async (client) => {
    const result = await client.query<{
      ...
      `SELECT count(*)::int AS pending,
       ...
       FROM portal.outbox WHERE "companyId"=$1 AND "deliveredAt" IS NULL`,
      [principal.companyId]
    );
...
export async function confirmOutboxApplied(
  ...
  await withPortalTransaction(pool, principal, "read", async (client) => {
    if (event.entityType === "intake") {
      const result = await client.query(
        `SELECT 1 FROM portal.intake WHERE "companyId"=$1 AND id=$2 AND state IN ('needs-review','ready')`,
    ...
    if (event.entityType !== "document")
      throw new Error("unsupported portal outbox entity type");
    if (event.eventType === "delete") {
      const result = await client.query(
        `SELECT 1 FROM portal.document WHERE "companyId"=$1 AND id=$2 AND ("deletedAt" IS NOT NULL OR status='withdrawn')`,
        [principal.companyId, event.entityId]
      );
      if (!result.rows[0])
    ...
    if (event.eventType === "acl-change") {
      const result = await client.query(
        `SELECT 1 FROM portal.document WHERE "companyId"=$1 AND id=$2`,
        [principal.companyId, event.entityId]
      );
      if (!result.rows[0])
    ...
    if (embeddingProfile === "manual-v1") {
      const result = await client.query(
        `SELECT 1 FROM portal.document d WHERE d."companyId"=$1 AND d.id=$2
          AND (d."deletedAt" IS NOT NULL OR d.status='withdrawn' OR (
            d.status='published' AND EXISTS (
              SELECT 1 FROM portal.chunk c WHERE c."companyId"=d."companyId"
    ...
    const result = await client.query(
      `SELECT 1 AS ready FROM portal.document d
       WHERE d."companyId"=$1 AND d.id=$2 AND d.status='published' AND d."deletedAt" IS NULL
        AND EXISTS (
          SELECT 1 FROM portal.chunk lexical WHERE lexical."companyId"=d."companyId" AND lexical."documentId"=d.id
        ...
        AND NOT EXISTS (
          SELECT 1 FROM portal.chunk lexical WHERE lexical."companyId"=d."companyId" AND lexical."documentId"=d.id
           AND lexical."documentVersionId"=d."currentVersionId" AND lexical."embeddingProfile"='lexical-v1'
           AND NOT EXISTS (
             SELECT 1 FROM portal.chunk embedded WHERE embedded."companyId"=lexical."companyId"
