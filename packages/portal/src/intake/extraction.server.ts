...
export async function getCapturedIntakeForExtraction(
  ...
  return withPortalTransaction(pool, principal, "read", async (client) => {
    const result = await client.query<{
      ...
      `SELECT id,generation,version,state,"inputRefs","reviewDecisions",extraction FROM portal.intake WHERE "companyId"=$1 AND id=$2`,
...
export async function attachCapturedObject(
  ...
  await withPortalTransaction(pool, principal, "write", async (client) => {
    const result = await client.query(
      `UPDATE portal.intake SET "inputRefs"="inputRefs" || $4::jsonb,"updatedBy"=$2,"updatedAt"=now(),version=version+1
...
export async function persistExtractionGeneration(
  ...
  return withPortalTransaction(pool, principal, "write", async (client) => {
    const existing = await client.query<{ output: Extraction }>(
      `SELECT output FROM portal.extraction WHERE "companyId"=$1 AND "intakeId"=$2 AND generation=$3`,
    ...
    const updated = await client.query<{ id: string }>(
      `UPDATE portal.intake SET generation=$3, extraction=$4::jsonb, unresolved=$5::jsonb, state='needs-review', "updatedBy"=$2, "updatedAt"=now(),version=version+1
...
export async function markReviewedIntakeReady(
  ...
  return withPortalTransaction(pool, principal, "write", async (client) => {
    const result = await client.query<{ id: string }>(
      `UPDATE portal.intake SET "reviewDecisions"=$4::jsonb,state='ready',"updatedBy"=$2,"updatedAt"=now(),version=version+1
