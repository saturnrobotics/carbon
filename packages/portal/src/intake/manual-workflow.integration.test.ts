...
async function rolePool(
  role: "portal_review" | "portal_ingest" | "portal_read",
...
describe("real PostgreSQL manual workflow", () => {
  ...
  beforeAll(async () => {
    ...
    await admin.query(
      `INSERT INTO portal.source(id,"companyId","createdBy",kind,"externalId","displayName","ownerId",classification,"providerPolicy")
       ...
       ON CONFLICT (id,"companyId") DO UPDATE SET "providerPolicy"=EXCLUDED."providerPolicy",version=portal.source.version+1`,
      [
        sourceId,
        companyId,
        actorId,
    ...
    await admin.query(
      `DELETE FROM portal."grant" WHERE "companyId"=$1 AND "sourceId"=$2 AND id='manual-workflow-admin'`,
      [companyId, sourceId]
    );
    await admin.query(
    ...
    await admin.query(
      `UPDATE portal.document SET kind='manual',version=version+1 WHERE "companyId"=$1 AND "sourceId"=$2 AND kind<>'manual'`,
      [companyId, sourceId]
    );
    await admin.end();
    reviewPool = await rolePool("portal_review", adminUrl.toString());
    ingestPool = await rolePool("portal_ingest", adminUrl.toString());
    readPool = await rolePool("portal_read", adminUrl.toString());
  });
  afterAll(async () => {
    ...
    try {
      ...
      await cleanup.query(
        `UPDATE portal.document SET status='withdrawn',"deletedAt"=now(),version=version+1 WHERE "companyId"=$1 AND "sourceId"=$2 AND "deletedAt" IS NULL`,
        [companyId, sourceId]
      );
    } finally {
      await cleanup.end();
  ...
  it("publishes searchable immutable metadata and tombstones without resurrection", async () => {
    ...
    ).withPortalTransaction(reviewPool, principal, "read", (client) =>
      client.query(
        `SELECT id FROM portal."grant" WHERE "companyId"=$1 AND "sourceId"=$2 AND "documentId"=$3
    ...
    const matches = await (
      await import("../database.server")
    ).withPortalTransaction(readPool, principal, "read", (client) =>
