  ...
  return withPortalTransaction(pool, principal, "read", async (client) => {
    const result = await client.query(
      `SELECT i.id,i.version,i.generation,i.state,i."sourceId",i."ownerId",i."inputRefs",i.extraction,i."reviewDecisions",i.unresolved,
        e.output AS "extractionOutput" FROM portal.intake i
       LEFT JOIN portal.extraction e ON e."companyId"=i."companyId" AND e."intakeId"=i.id AND e.generation=i.generation
...
export async function publishReviewedIntake(
  ...
  return withPortalTransaction(pool, principal, "write", async (client) => {
    ...
    const replay = await client.query<{ targetRefs: unknown }>(
      `SELECT "targetRefs" FROM portal.audit
       WHERE "companyId"=$1 AND "actorId"=$2 AND "callerId"=$3 AND "requestId"=$4
         AND action='portal.intake.publish' LIMIT 1`,
      [
        principal.companyId,
        principal.actorId,
        principal.callerId,
    ...
    >(
      `SELECT i.id AS "intakeId",i.version,i.generation,i.state,i."sourceId",i."ownerId",i."inputRefs",i.extraction,i."reviewDecisions",i.unresolved,
        e.output AS "extractionOutput" FROM portal.intake i
       JOIN portal.extraction e ON e."companyId"=i."companyId" AND e."intakeId"=i.id AND e.generation=i.generation
    ...
    const existingDocument = (
      await client.query<{
        ...
        `SELECT id,status,"deletedAt" FROM portal.document WHERE "companyId"=$1 AND "sourceId"=$2 AND "sourceItemId"=$3`,
        [
          principal.companyId,
          intake.sourceId,
          publication.document.sourceItemId
    ...
    const insertedVersion = await client.query<{ id: string }>(
      `INSERT INTO portal."documentVersion" ("companyId","createdBy","documentId","sourceRevision","contentHash","objectKey","objectGeneration","MIME","byteCount","observedAt","parserVersion","extractionStatus","reviewedMetadata")
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,now(),'portal-worker-v1','ready',$10::jsonb)
    ...
    const existingVersion =
      ...
      (
        await client.query<{ id: string }>(
          `SELECT id FROM portal."documentVersion" WHERE "companyId"=$1 AND "documentId"=$2 AND "sourceRevision"=$3`,
          [
            principal.companyId,
            existingDocument.id,
            publication.version.sourceRevision
    ...
    await client.query(
      `INSERT INTO portal.chunk ("companyId","createdBy","documentId","documentVersionId",ordinal,text,heading,page,bounds,"tokenCount","embeddingProfile","indexGeneration")
    ...
    await client.query(
      `UPDATE portal.document SET "currentVersionId"=$3,status='published',title=$4,classification=$5,"updatedBy"=$2,"updatedAt"=now(),version=version+1
    ...
    const finalized = await client.query(
      `UPDATE portal.intake SET state='ready',
    ...
    await client.query(
      `INSERT INTO portal.audit ("companyId","createdBy","actorId","callerId","requestId",action,"targetRefs",decision,"policyVersion",metadata)
       VALUES ($1,$2,$2,$3,$4,'portal.intake.publish',jsonb_build_array(jsonb_build_object(
