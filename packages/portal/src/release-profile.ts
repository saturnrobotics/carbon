...
export function readManualSourceConfiguration(
  ...
  if (
    environment.PORTAL_RELEASE_PROFILE &&
    environment.PORTAL_RELEASE_PROFILE !== MANUAL_RELEASE_PROFILE
  )
    throw Error("Unsupported portal release profile");
  if (!environment.PORTAL_MANUAL_SOURCE_JSON)
    throw Error("Manual source configuration is required");
  return manualSourceConfigurationSchema.parse(
    JSON.parse(environment.PORTAL_MANUAL_SOURCE_JSON)
  );
}
