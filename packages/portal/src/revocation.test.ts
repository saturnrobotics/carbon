...
/**
 * The revocation contract between Carbon and the portal runtime, driven
...
const sourceAudience =
  "/projects/123456789/locations/us-central1/services/portal-web";
  ...
  callers: [
    {
      ...
      operations: ["portal_getItemIdentity"],
...
function bindingTable() {
  ...
  const client = {
    async query(text: string) {
      if (text.includes('portal."identityBinding"'))
        return {
          rows: row.active
            ? [{ id: row.id, revocationVersion: String(row.revocationVersion) }]
            : []
        };
      if (text.includes("portal.source"))
        return {
  ...
  return {
    ...
    client,
    identityStore,
    /** public.portal_propagate_identity_revocation on UPDATE OF active. */
    deactivateUser() {
      row.userActive = false;
      revoke();
    },
    /** public.portal_propagate_identity_revocation on DELETE of membership. */
    removeMembership() {
      row.membershipActive = false;
      revoke();
    },
    /** portal.unbind_workforce_identity (the identity:revoke script). */
    unbind: revoke
  };
}
...
function request() {
  return new Request(
    "https://carbon-api.example.com/api/v1/portal/getItemIdentity",
    {
      method: "POST",
      headers: {
