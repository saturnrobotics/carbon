...
function publishedPortalOperations(): string[] {
  let raw: string;
  try {
...
/** Carbon publishes `portal_getItemIdentity` at `/api/v1/portal/getItemIdentity`. */
...
describe("Carbon portal operations reach the transport", () => {
  ...
  it("registers or deliberately excludes every operation Carbon publishes", () => {
    ...
    expect(
      unreachable,
      `These portal operations are published by Carbon but neither registered in the transport allowlist nor listed in EXCLUDED_CARBON_OPERATIONS with a reason, so calling one returns an unregistered-operation refusal: ${unreachable.join(", ")}`
    ).toEqual([]);
  });
  ...
  it("carries the supplier pricing read, the operation this test was written for", () => {
    expect(published).toContain("portal_getItemSupplierPricing");
    expect(
      REGISTERED_SOURCE_OPERATIONS.has("/api/v1/portal/getItemSupplierPricing")
    ).toBe(true);
  });
});
