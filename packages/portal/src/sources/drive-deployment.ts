 ...
 *    (`apps/portal/app/routes.deferred.ts`). Every release image is built
 ...
 *    without one.
 * 2. `contrib/deploying/portal/release.py` rejects any environment key
 ...
 * 3. The same file pins `portal-web` to `manual-v1`, so the profile that
 *    defers this surface is the only one a release can carry.
 *
 * `contrib/deploying/portal/test_images.py` pins locks 1 and 2 by asserting
 * that no release Dockerfile and no `release.py` environment set names the
 * variable.
 */
export const DRIVE_SURFACE_ENABLED_VARIABLE = "PORTAL_DRIVE_ENABLED";
