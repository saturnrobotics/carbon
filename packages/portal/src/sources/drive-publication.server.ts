  ...
  return withPortalTransaction(pool, principal, "read", async (client) => {
    const result = await client.query<DrivePublicationTarget>(
      `SELECT d.id AS "documentId",d."sourceId",d."sourceItemId",d.title,
       ...
       FROM portal.document d JOIN portal.source s ON s."companyId"=d."companyId" AND s.id=d."sourceId" AND s.kind='drive'
...
export async function publishDriveDocumentVersion(
  ...
  return withPortalTransaction(pool, principal, "write", async (client) => {
    const document = await client.query<{
      ...
      `SELECT id,"currentVersionId" FROM portal.document
    ...
    const inserted = await client.query<{ id: string }>(
      `INSERT INTO portal."documentVersion" ("companyId","createdBy","documentId","sourceRevision","contentHash","objectKey","objectGeneration","MIME","byteCount","observedAt","parserVersion","extractionStatus")
    ...
    const version =
      ...
      (
        await client.query<{ id: string; contentHash: string }>(
          `SELECT id,"contentHash" FROM portal."documentVersion" WHERE "companyId"=$1 AND "documentId"=$2 AND "sourceRevision"=$3`,
    ...
    await client.query(
      `UPDATE portal.document SET "currentVersionId"=$3,status='published',"updatedBy"=$2,"updatedAt"=now(),version=version+1
