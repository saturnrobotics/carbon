...
export async function persistDriveChangePage(
  ...
  return withPortalTransaction(pool, principal, "write", async (client) => {
    const source = await client.query<{ cursor: unknown }>(
      `SELECT cursor FROM portal.source WHERE "companyId"=$1 AND id=$2 AND kind='drive' AND status='active' FOR UPDATE`,
    ...
    const enrollment = await client.query<{ rootFolderIds: string[] }>(
      `SELECT "rootFolderIds" FROM portal."driveEnrollment" WHERE "companyId"=$1 AND "sourceId"=$2`,
      [principal.companyId, input.sourceId]
    );
    ...
    if (ordered.length)
      await client.query(
        `INSERT INTO portal."driveItem" ("companyId","createdBy","sourceId","fileId","driveId","mimeType",name,"parentIds","shortcutTargetId",revision,"contentHash",trashed,"aclEvaluated",permissions,"observedAt","removedAt")
    ...
    const scopeChanges = await client.query<{
      ...
      `WITH RECURSIVE reachable AS (
         SELECT i."fileId" FROM portal."driveItem" i
         ...
         UNION
         SELECT c."fileId" FROM portal."driveItem" c JOIN reachable r ON c."parentIds" && ARRAY[r."fileId"]
         ...
         FROM portal."driveItem" i WHERE i."companyId"=$1 AND i."sourceId"=$2
       )
       UPDATE portal."driveItem" i SET "inScope"=c.scoped,"updatedBy"=$4,"updatedAt"=now(),version=i.version+1
    ...
    const items = await client.query<{
      ...
      `SELECT i."fileId",i.name,i."mimeType",i."shortcutTargetId",i.revision,i.trashed,i."inScope",i."aclEvaluated",
       ...
       FROM portal."driveItem" i
       LEFT JOIN portal.document d ON d."companyId"=i."companyId" AND d."sourceId"=i."sourceId" AND d."sourceItemId"=i."fileId"
       LEFT JOIN portal."documentVersion" v ON v."companyId"=d."companyId" AND v.id=d."currentVersionId"
    ...
    if (active.length) {
      const inserted = await client.query(
        `INSERT INTO portal.document ("companyId","createdBy","sourceId","sourceItemId",title,"ownerId",kind,status,classification)
    ...
    if (deleted.length) {
      const result = await client.query(
        `UPDATE portal.document SET status='withdrawn',"deletedAt"=now(),"aclVersion"="aclVersion"+1,"updatedBy"=$2,"updatedAt"=now(),version=version+1
    ...
    if (touched.length)
      await client.query(
        `UPDATE portal."grant" g SET "revokedAt"=now(),"updatedBy"=$2,"updatedAt"=now(),version=g.version+1
         FROM portal.document d WHERE g."companyId"=$1 AND g."sourceId"=$3 AND g."documentId"=d.id AND d."companyId"=g."companyId"
    ...
    if (evaluated.length)
      await client.query(
        `INSERT INTO portal."grant" ("companyId","createdBy","sourceId","documentId","subjectKind","subjectId",capability,origin,"sourcePermissionId","policyVersion")
         ...
         JOIN portal.document d ON d."companyId"=i."companyId" AND d."sourceId"=i."sourceId" AND d."sourceItemId"=i."fileId"
         LEFT JOIN portal."sourceUserBinding" b ON b."companyId"=$1 AND b."sourceId"=$3 AND b.active AND b."sourceUserId"=p."principalId"
    ...
    if (events.length)
      await client.query(
        `INSERT INTO portal.outbox ("companyId","createdBy","sourceId","entityType","entityId","sourceVersion","eventType",payload)
         ...
         JOIN portal.document d ON d."companyId"=$1 AND d."sourceId"=$3 AND d."sourceItemId"=e."sourceItemId"
    ...
    const advanced = await client.query(
      `UPDATE portal.source SET cursor=jsonb_build_object('pageToken',$3::text),"updatedBy"=$2,"updatedAt"=now(),version=version+1
...
export async function getDriveSyncState(
  ...
  return withPortalTransaction(pool, principal, "read", async (client) => {
    const result = await client.query<{ externalId: string; cursor: unknown }>(
      `SELECT "externalId" AS "externalId",cursor FROM portal.source WHERE "companyId"=$1 AND id=$2 AND kind='drive' AND status='active'`,
...
function readEnrollment(
  ...
  return client.query<DriveEnrollment>(
    `SELECT ${enrollmentProjection} FROM portal."driveEnrollment" e
     JOIN portal.source s ON s."companyId"=e."companyId" AND s.id=e."sourceId" AND s.kind='drive' AND s.status='active'
  ...
  return withPortalTransaction(pool, principal, "read", async (client) => {
    ...
    >(
      `SELECT e."sourceId",s."displayName",s."ownerId",s.classification,e.corpora,e."driveId",e."rootFolderIds",
         ...
         to_char(e."lastSyncAt" AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS"Z"') AS "lastSyncAt",
         e."lastSyncStatus",
         (SELECT count(*) FROM portal.document d WHERE d."companyId"=e."companyId" AND d."sourceId"=e."sourceId" AND d.status='published' AND d."deletedAt" IS NULL)::text AS "documentCount"
       FROM portal."driveEnrollment" e
       JOIN portal.source s ON s."companyId"=e."companyId" AND s.id=e."sourceId" AND s.kind='drive'
...
export async function recordDriveSyncOutcome(
  ...
  await withPortalTransaction(pool, principal, "write", async (client) => {
    const result = await client.query(
      `UPDATE portal."driveEnrollment" SET "lastSyncAt"=now(),"lastSyncStatus"=$3,"lastSyncError"=$4,
...
export async function listDriveItems(
  ...
  return withPortalTransaction(pool, principal, "read", async (client) => {
    const result = await client.query(
      `SELECT ${ledgerProjection} FROM portal."driveItem" WHERE "companyId"=$1 AND "sourceId"=$2 AND "fileId"=ANY($3::text[])`,
...
export async function listDriveLedger(
  ...
  return withPortalTransaction(pool, principal, "read", async (client) => {
    const result = await client.query(
      `SELECT ${ledgerProjection} FROM portal."driveItem" WHERE "companyId"=$1 AND "sourceId"=$2 AND NOT trashed ORDER BY "fileId" LIMIT ${DRIVE_DESCENDANT_LIMIT + 1}`,
      [principal.companyId, sourceId]
    );
  ...
  return withPortalTransaction(pool, principal, "read", async (client) => {
    const result = await client.query<{ fileId: string }>(
      `WITH RECURSIVE affected AS (
         SELECT i."fileId",1 AS depth FROM portal."driveItem" i
         WHERE i."companyId"=$1 AND i."sourceId"=$2 AND NOT i.trashed
           AND (i."parentIds" && $3::text[] OR i."shortcutTargetId"=ANY($3::text[]))
         UNION
         SELECT c."fileId",a.depth+1 FROM portal."driveItem" c JOIN affected a
...
export async function getDriveItemForAccess(
  ...
  return withPortalTransaction(pool, principal, "read", async (client) => {
    const result = await client.query<{
      ...
      `SELECT "fileId","shortcutTargetId" FROM portal."driveItem"
...
export async function matchesDriveNotificationChannel(
  ...
  return withPortalTransaction(pool, principal, "read", async (client) => {
    const result = await client.query(
      `SELECT 1 FROM portal."driveEnrollment"
