 ...
 * CRM application publishes exactly these routes; the portal side reads
...
export function createSyntheticProducer(options: SyntheticProducerOptions) {
  ...
  const fetchImpl: typeof fetch = async (input, init) => {
    ...
    if (url.pathname === "/api/portal/changes") {
    ...
    const entityPath =
      url.pathname === "/api/portal/entities/search"
        ? null
        : /^\/api\/portal\/entities\/([A-Za-z0-9_-]{1,256})$/.exec(
            url.pathname
          );
    if (entityPath?.[1]) {
      if (method !== "GET") return json({ error: "method" }, 405);
    ...
    if (url.pathname === "/api/portal/entities/search") {
    ...
    if (url.pathname === "/api/portal/facts/query") {
    ...
    if (url.pathname === "/api/portal/documents/references") {
    ...
    if (url.pathname === "/api/portal/access/check") {
      const input = parseBody(
